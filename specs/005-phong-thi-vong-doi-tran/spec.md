...
- Q: Khi admin bấm Chốt trận, có ≥2 người hoà ở vị trí Nhất (đủ điều kiện kích hoạt `TIE_BREAK` theo `GR-022` C1/C2), nhưng không đủ 3 câu khả dụng cho vòng Câu hỏi phụ — hệ thống làm gì? → A: Trận **ở lại `LOBBY`** — Chốt trận coi như bị chặn cứng giống mọi vòng thiếu câu khác (nhất quán `INV-014` "cửa vào vòng thiếu câu"); admin phải bổ sung câu qua US-008 rồi bấm lại Chốt trận. Trận **không** chuyển sang `TIE_BREAK` trước rồi mới chặn.
- Q: `PRD-REQ-027`/FR-001 nói "chỉ có một trận đang chạy tại một thời điểm" — "đang chạy" có tính một trận vừa tạo, còn ở `LOBBY`, chưa bấm bắt đầu vòng nào hay không? → A: **Có** — mọi trận chưa `FINISHED` (kể cả `LOBBY` chưa bấm bắt đầu) đều tính là "đang chạy". Admin chỉ tạo được trận mới sau khi trận trước đã `FINISHED` — tuần tự nghiêm ngặt, không có mô hình pre-tạo nhiều trận cùng lúc trong một contest.
- Q: Đặt chỗ câu Câu hỏi phụ (`PRD-REQ-091`/US-009) thuộc phạm vi TRẬN hay CONTEST — khi Huỷ trận hoặc tạo trận mới, danh sách đặt chỗ có bị xoá/reset hay giữ nguyên? → A: Thuộc phạm vi **TRẬN**. Trận mới (do Chốt trận xong hoặc do Huỷ trận rồi tạo lại) luôn bắt đầu với danh sách đặt chỗ **trống** — nhất quán với việc điểm và nhật ký sự kiện cũng thuộc phạm vi trận (FR-002).
- Q: Có giới hạn trên cho số câu được đặt chỗ làm Câu hỏi phụ hay không (`PRD-REQ-091`/`GR-023` chỉ nêu ngưỡng dưới — thiếu thì bù)? → A: **Không giới hạn trên**. Hệ thống chấp nhận đặt chỗ nhiều hơn 3 câu; vòng Câu hỏi phụ khi mở chỉ dùng 3 câu đầu tiên theo thứ tự chỉ định, phần dư không dùng vẫn giữ nguyên chỉ định (không tự gỡ).
- Q (phiên đối chiếu PRD 2026-07-30): FR-017 và `specs/004-contest-builder-luat` FR-008 đọc khác nhau về cùng một `PRD-REQ-105` — "mô hình dữ liệu vẫn nhận danh sách nhiều vị trí" là **đường ghi hợp lệ** hay chỉ là **sức chứa schema**? → A: **Sức chứa schema/kiểu dữ liệu**, không phải đường ghi. Server MUST từ chối mọi request ghi `tieBreakPositions ≠ [1]` cho contest v1 — cách đọc "chỉ khoá ở giao diện" biến một khoá cứng thành hàng rào UI-only, trái `CLAUDE.md` §Nguyên tắc code ("validate FE chỉ là UX"). FR-017 đã đồng bộ theo spec 004; `PRD-REQ-025` và `PRD-REQ-105` cần một `Note` làm rõ cùng cách đọc.
- Q: `SC-004` ("một vòng bị chặn vì thiếu câu luôn có đường xử lý trong vòng dưới 1 phút thao tác của admin") dùng một mốc thời gian không có nguồn xác nhận — giữ, sửa, hay bỏ? → A: **Thay bằng tiêu chí cấu trúc, không định thời gian** — bỏ SLA "1 phút" tự suy diễn, đổi thành "100% thông điệp báo thiếu đề có đường dẫn trực tiếp sang màn sửa danh sách câu, không cần rời `LOBBY` sang một màn hình khác".
...
- **Trận vừa tạo, còn ở `LOBBY`, chưa bấm bắt đầu vòng nào**: vẫn tính là "đang chạy" đối với ràng buộc "một trận đang chạy mỗi contest" (§Clarifications 2026-07-30) — hệ thống không cho tạo trận thứ hai trong cùng contest chừng nào trận này chưa `FINISHED`, kể cả khi chưa vòng nào của nó từng mở.
- **Đặt chỗ Câu hỏi phụ khi trận kết thúc (`FINISHED` hoặc "bỏ dở") rồi tạo trận mới**: danh sách đặt chỗ không mang sang trận mới — thuộc phạm vi trận, không phải phạm vi contest (§Clarifications 2026-07-30).

## Requirements *(mandatory)*

### Functional Requirements

- **FR-001**: Hệ thống MUST cho một contest chứa được nhiều trận, và MUST đảm bảo tại một thời điểm chỉ có đúng **một** trận của contest đó ở trạng thái đang chạy. "Đang chạy" MUST được hiểu là **mọi trận chưa `FINISHED`**, kể cả một trận vừa tạo còn ở `LOBBY` và chưa bấm bắt đầu vòng nào — hệ thống MUST NOT cho tạo trận thứ hai trong cùng contest khi trận hiện tại chưa `FINISHED` (§Clarifications 2026-07-30). *(US-001 · PRD-REQ-027 · GR-031 · AC-001, AC-032)*
- **FR-002**: Hệ thống MUST giữ ở cấp **contest**: mã phòng, kho đề đã gán, cấu hình gốc, và phạm vi không-lặp-câu; và MUST giữ ở cấp **trận**: điểm, nhật ký sự kiện, ghế đã gán, biên bản, cấu hình đã đóng băng. *(US-001 · PRD-REQ-027 · AC-001, AC-002)*
...
- **SC-004**: 100% thông điệp báo một vòng bị chặn vì thiếu câu có đường dẫn trực tiếp sang màn sửa danh sách câu — admin xử lý được (bổ sung câu, quay lại mở vòng) mà không cần rời `LOBBY` sang một màn hình khác để tự tìm đường sửa (§Clarifications 2026-07-30).
...
Không còn `NEEDS CLARIFICATION` nào treo. Điểm duy nhất từng treo — hành vi Chốt trận khi hoà đủ điều kiện `TIE_BREAK` nhưng thiếu câu khả dụng — đã được phân xử ở §Clarifications 2026-07-30 (xem FR-018) và tích hợp vào FR-018, Edge Cases, AC-032.
...
**AC-032** — *Related user story*: US-001, US-005 · *Related FR*: FR-001, FR-018 · *Related game rules*: GR-022 (§Clarifications 2026-07-30)
