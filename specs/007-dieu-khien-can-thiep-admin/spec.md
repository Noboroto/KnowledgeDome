...
- Q: Quy tắc **cửa sổ giữ bản tới muộn** thuộc spec nào — EPIC-006 hay EPIC-007? → A: **EPIC-006 sở hữu quy tắc engine** *(nút gửi tắt lúc nào · server giữ hay từ chối một bản · biên trên · Khởi động không biên trên · bỏ mốc cắt)*; **EPIC-007 chỉ sở hữu mốc mở nút chấm** trên màn điều khiển. `FR-032a` và `FR-032b` đã **gỡ khỏi spec này** và chuyển thành `FR-070a`, `FR-070b` của `specs/006-game-engine-luat-thi-dau`; `FR-070` ở đó cũng đã neo lại vào `hạn chót`. Ranh giới này khớp §Out of Scope mà **cả hai spec** đã tự khai.
- Q: Gọi cơ chế đó là gì? → A: **"Cửa sổ giữ bản tới muộn"**, **không** gọi là *"dung sai"*. Chữ *dung sai* nghe như hệ thống **khoan nhượng** cho bản tới muộn — như thể nó thành hợp lệ. Không phải: bản đó **vẫn là bản quá hạn**, vẫn tô đỏ, và số phận của nó vẫn do **admin phán quyết Đúng/Sai**. Thứ cửa sổ này bảo vệ là **quyền phán quyết**, không phải quyền của thí sinh (`QĐ-109`).
- Q: `OQ-001` — Điều chỉnh điểm thủ công dùng dialog **hạng phá huỷ** (`STATE-035`) hay **hạng Yes/No thường** (`PRD-REQ-056`, `GR-029`)? → A: **Không phải hạng nào trong hai — tách hai trục (Option C)**. Hệ thống có **ba tầng dialog** phân biệt bằng **cách thoát**, và *"bắt nhập lý do"* là một trục **độc lập** với tầng: **(1) hạng phá huỷ** — `Esc` **không** đóng, click ra ngoài **không** đóng, bắt lý do; **(2) điều chỉnh điểm** — tầng **riêng, ở giữa**: `Esc` **đóng được** và khi đó **không lưu thay đổi**, click ra ngoài **không** đóng, bắt lý do; **(3) Yes/No thường** — `Esc` **và** click ra ngoài đều đóng và đều hiểu là **No**, không bắt lý do. Hệ quả tài liệu: cần một `QĐ` mới trong `docs/decisions.md`, rồi sửa **cả** `PRD-REQ-056` *(nay mô tả hai trục thay vì một danh sách bốn thao tác)* lẫn `STATE-035` *(gỡ điều chỉnh điểm khỏi hạng phá huỷ, khai tầng riêng của nó)*. Xem `OQ-001`.
- Q: Phím tắt `Y`/`N` áp cho tầng dialog nào? → A: **Yes/No thường và điều chỉnh điểm — KHÔNG áp cho hạng phá huỷ.** Hạng phá huỷ chỉ nhận click chuột, cùng tinh thần với `GR-034` *(chuông không gán phím nào để tránh bấm nhầm khi tay đang trên bàn phím)*; ở hạng đó admin vốn đang gõ lý do nên một phím lỡ tay sẽ là một cú bỏ vòng.
- Q: `OQ-002` — **Dialog cảnh báo lệch luật** (`STATE-036`) là hạng thứ tư phân biệt được bằng mắt, hay là một biến thể của hạng Yes/No trong `PRD-REQ-056`? → A: **Cùng tầng Yes/No thường (Option B)** — nó **không** là một tầng riêng và **không** đòi khác biệt thị giác với dialog xác nhận. Nó dùng lại **y hệt** cách thoát của tầng Yes/No thường: `Esc` và click ra ngoài đều đóng và đều tương đương **No**, có phím tắt `Y`/`N`, **không** bắt nhập lý do. Khác biệt duy nhất là **nội dung chữ** — dialog cảnh báo nêu điểm lệch luật, còn dialog xác nhận nêu hậu quả không hoàn tác được. Hệ quả tài liệu: sửa `PRD-REQ-056` để nói rõ cảnh báo lệch luật **dùng lại** tầng Yes/No thay vì là hạng thứ tư. Xem `OQ-002`.
- Q: `OQ-003` — Ở Tăng tốc, một bản gửi quá hạn tới **sau** cú bấm *chốt câu* thì admin còn công nhận được không? → A: **Tình huống bất khả thi về cấu trúc — không cần nhánh xử lý.** Cơ chế là một **cửa sổ giữ bản tới muộn phía server**, **không** phải ân hạn thao tác: nút gửi phía thí sinh tắt **đúng tại `hạn chót`** ở mọi vòng; server còn chấp nhận bản tới trong `(hạn chót, hạn chót + padding]` — bản phát ra kịp nhưng tới muộn do mạng — và tô **đỏ** để admin có cơ sở phán quyết; sau `hạn chót + padding` server từ chối. Nút chấm và nút *chốt câu* ở kênh gõ chỉ mở **từ `hạn chót + padding`**, tức khi cửa sổ giữ bản tới muộn đã đóng. Hai điều này cộng lại làm mọi bản **chấm được** luôn tới trước cú bấm *chốt câu*. `GR-015` C6 vì thế đọc là: công nhận một bản quá hạn xảy ra **trước** mốc chốt câu, và *"cả bảng tính lại"* mô tả việc bảng được tính **một lần** tại mốc chốt với dấu đã sửa — không cần cơ chế tính lại nào mà `INV-009` đang cấm.
- Q: Giá trị `padding` là bao nhiêu, và nó tồn tại để làm gì? → A: **RuleConfig cấu hình được, mặc định 5 giây**, không hard-code — đúng `CLAUDE.md` §Quy ước code. **Mục đích là bảo vệ tính đúng đắn trước độ trễ mạng và cho admin cơ sở xử lý**, MUST NOT đọc thành một ân hạn thao tác cho thí sinh; đọc sai theo hướng đó sẽ chỏi `GR-033` §Điều kiện *(mốc tuyệt đối, không cửa sổ ân hạn)*.
- Q: `OQ-004` — Ở Khởi động mode nhập liệu, mốc cắt *"công bố đáp án"* đứng ở đâu so với cú bấm phán quyết? → A: **Bỏ hẳn khái niệm mốc cắt riêng (Option D).** `hạn chót` của câu là hạn nhận bài **duy nhất** ở vòng này; `EVENT-016` không còn là một mốc thứ ba mà **gộp vào** cú bấm mở đáp án chung của FR-065. Bản được ghi nhận là bản hợp lệ **cuối cùng trước `hạn chót`**. Nút chấm mở **từ `hạn chót`**, không chờ thêm.
- Q: Ở Khởi động, *"không có padding"* áp cho vế nào — việc giữ bản tới muộn, hay mốc mở nút chấm? → A: **Cả hai, theo hướng nới chứ không siết (Option C).** Khởi động **không có biên trên** cho việc giữ bản tới muộn: một bản tới sau `hạn chót` **luôn** được ghi nhận và tô đỏ, **bất kể muộn bao nhiêu**, và admin chấm được nó — `GR-006` C4 và C5 giữ nguyên hiệu lực. Đồng thời admin **không phải chờ**: nút chấm mở ngay tại `hạn chót`.
- **Ghi chú về một bất đối xứng có chủ đích** *(hệ quả của các quyết định trên, không phải một quyết định riêng)*: vòng chấm theo **lô** — Tăng tốc và câu hàng ngang Vượt chướng ngại vật — cần **biên trên cứng** cho cửa sổ giữ bản tới muộn và cần admin **chờ cửa sổ đóng** mới được chốt, vì chúng có một **bảng điểm chung** chốt một lần; một bản chấm được tới sau cú *chốt câu* sẽ đòi tính lại bảng, thứ `INV-009` cấm. Vòng **Khởi động** chấm **từng người**, không có bảng chung, nên nới cả hai vế **không phá gì**: bản tới muộn bao nhiêu cũng chấm được, và một bản tới sau khi đã chấm chỉ nằm trong lịch sử — nó không lật được phán quyết (`INV-009`), sửa sai vẫn đi qua điều chỉnh điểm thủ công. **Điểm chung của mọi vòng**: nút gửi phía thí sinh tắt đúng tại `hạn chót`, không đâu có thêm giây thao tác nào.
...
- **Related PRD requirements**: EPIC-007 §11 Scope *(bề mặt bấm của admin)* · PRD-REQ-075, PRD-REQ-076 *(các mệnh đề nói về admin — xem §Quy ước truy nguyên điểm 2)*
...
- **Hạng dialog của điều chỉnh điểm thủ công**: `docs/PRD.md` PRD-REQ-056 và `docs/game-state-machine.md` `STATE-035` nói ngược nhau — **đã phân xử** bằng mô hình hai trục, xem `OQ-001` ở §Open Questions và §Clarifications 2026-08-03. Hai tài liệu vẫn cần sửa cho khớp.
- ***"Ba hạng cảnh báo"* của PRD-REQ-056 vs bốn lớp phủ phản hồi của máy trạng thái**: **đã phân xử** — cảnh báo lệch luật dùng lại tầng Yes/No thường, xem `OQ-002`. `PRD-REQ-056` vẫn cần sửa cho khớp.
...
- **Bản gửi quá hạn tới SAU cú bấm *chốt câu* ở Tăng tốc**: **đã phân xử** — tình huống **bất khả thi về cấu trúc** nhờ cửa sổ `padding` và mốc mở nút chấm, xem `OQ-003`. `docs/` vẫn cần bổ sung quy tắc `padding`.
- **Thứ tự giữa mốc cắt *công bố đáp án* (`GR-006`) và cú bấm phán quyết ở Khởi động mode nhập liệu**: **đã phân xử** — bỏ hẳn mốc cắt riêng, `hạn chót` là hạn nhận bài duy nhất; xem `OQ-004`. `GR-006` và `EVENT-016` vẫn cần sửa.
...
- **Cửa sổ giữ bản tới muộn (`padding`)**: Khoảng **`(hạn chót, hạn chót + padding]`** mà server còn **giữ** một bản gửi phát ra kịp nhưng tới muộn do mạng. Bản đó **vẫn là bản quá hạn** — nó chỉ được giữ để còn đi qua **phán quyết Đúng/Sai** của admin, **không** được khoan nhượng thành hợp lệ. Nút gửi phía thí sinh tắt đúng tại `hạn chót` ở mọi vòng. Ở **Khởi động** không có biên trên. **Quy tắc cửa sổ thuộc EPIC-006**; spec này chỉ dùng nó làm mốc mở nút chấm. *(`QĐ-109` · `specs/006` FR-070a, FR-070b)*
...
- **Hạng phản hồi**: Nhóm phân loại của một thông báo từ hệ thống tới admin, xác định bởi thứ nó bảo vệ: toast invalid state *(không ép được)* · dialog Yes/No *(thao tác không hoàn tác được)* · dialog hạng phá huỷ *(bắt lý do, không tắt được)* · dialog cảnh báo lệch luật *(ép được — vị thế xem `OQ-002`)*. *(`PRD §9.4`, `STATE-035` → `STATE-037`)*
...
- **SC-008**: Ba hạng phản hồi phân biệt được bằng mắt ở **100%** số tình huống được thử; **0** dialog hạng phá huỷ đóng được bằng `Esc` hoặc click ra ngoài.
...
> **Cập nhật sau phiên làm rõ 2026-08-03**: **cả bốn mục đã được chủ dự án phân xử** và tích hợp vào các FR/AC tương ứng; chúng giữ lại đây dưới dạng **bản ghi đã đóng** để truy vết. **Không còn `NEEDS CLARIFICATION` hay `CONFLICT` nào tồn đọng.**
>
> **⚠️ Mức chặn còn lại là TRUY NGUYÊN, không phải quyết định.** Bốn phân xử đều phát biểu trong phiên làm rõ, nên theo Nguyên tắc I và Cổng 1 của `.specify/memory/constitution.md` chúng **chưa** là nguồn. `docs/` phải được sửa trước khi các FR liên quan vào task implementation — mỗi mục dưới đây kê rõ những gì cần sửa. Hai mục nặng nhất: `OQ-003` thêm một **quy tắc nghiệp vụ mới** *(cửa sổ `padding`)*, và `OQ-004` **gỡ** một mốc mà `docs/` đang có *(`EVENT-016`)*.
>
> **Tiến độ sửa `docs/` (2026-08-03)**: `docs/decisions.md` **đã xong** — **`QĐ-108`** *(`OQ-001`)*, **`QĐ-109`** *(`OQ-003`)*, **`QĐ-110`** *(`OQ-004`)*. `OQ-002` **không cần `QĐ` mới**: **`QĐ-072` đã khai đúng điều đó từ trước**, spec chỉ tưởng nó là câu hỏi mở vì `decisions.md` không nằm trong danh sách đọc của `/speckit-specify`.
>
> **✅ HOÀN TẤT 2026-08-03**: cả ba tài liệu đã khớp. `docs/PRD.md` — `PRD-REQ-052` *(biên trên cửa sổ giữ bản tới muộn)*, `PRD-REQ-054` *(mốc mở nút chấm = `hạn chót + padding`, ngoại lệ Khởi động; đổi luôn tiêu đề)*, `PRD-REQ-056` *(viết lại theo **hai trục**, bốn loại phản hồi)*, `PRD-REQ-020` *(`padding` vào tập cấu hình luật, mặc định 5 giây)*, version **2.6.0**. `docs/game-rules.md` — `GR-006` §Điều kiện, C2, C3, §Thứ tự đánh giá, §Đồng thời, §Ví dụ, §Nguồn · `GR-015` C5, C6, **C7 mới**, §Đồng thời · `GR-035` C6, **C6b, C6c mới** · bảng §*Kênh trả lời* · bảng §*Bản gửi quá hạn* · nguyên tắc nền **22**. `docs/game-state-machine.md` — `STATE-035` *(bảng ba tầng, §Cấm, §Nguồn)* · `EVENT-016` *(viết lại thành **bí danh**, vai trò mốc cắt đã gỡ)* · `T-036` · `EVENT-012`/`013` §Không hợp lệ ở · §Invalid transitions · `INV-015` · ghi chú Sơ đồ 2. `docs/decisions.md` — `QĐ-027` và `QĐ-030` mang ghi chú bị `QĐ-110`/`QĐ-109` thay một vế.

- **OQ-001 — ✅ ĐÃ PHÂN XỬ (§Clarifications 2026-08-03)**: **Hạng dialog của thao tác điều chỉnh điểm thủ công.** Phân xử của chủ dự án: **không phải hạng nào trong hai** — hệ thống **tách hai trục**. Tầng dialog xác định bằng **cách thoát**, còn *"bắt nhập lý do"* là một trục **độc lập**. Ba tầng: **hạng phá huỷ** *(không `Esc`, không click ngoài, có lý do, không phím tắt)* · **điều chỉnh điểm** *(có `Esc` và khi đó **không lưu thay đổi**, không click ngoài, có lý do, có phím tắt `Y`/`N`)* · **Yes/No thường** *(cả `Esc` lẫn click ngoài đều đóng và đều = **No**, không lý do, có phím tắt `Y`/`N`)*. Đã tích hợp vào **FR-045**, **FR-060**, **FR-061**, **FR-061a** *(mới)*, **FR-063** và **AC-052**, **AC-066**, **AC-067**, **AC-069**.

  <details><summary>Nội dung mâu thuẫn trước khi phân xử</summary>

  **CONFLICT**: Điều chỉnh điểm thủ công thuộc hạng dialog **thường** hay hạng **phá huỷ** — Nguồn A: `docs/PRD.md` §12 `PRD-REQ-056` nói *"**Dialog hạng phá huỷ** cho bỏ vòng, chạy lại vòng, kết thúc sớm, huỷ trận — MUST NOT tắt được bằng `Esc` hay click ra ngoài, và MUST bắt nhập lý do"*, tức liệt kê **đúng bốn** thao tác và **không** có điều chỉnh điểm; Nguồn B: `docs/game-state-machine.md` §F `STATE-035` nói hạng phá huỷ gồm *"(bỏ vòng, chạy lại vòng, kết thúc vòng khẩn cấp, huỷ trận, **điều chỉnh điểm**)"*, tức **năm** thao tác. Nguồn thứ ba `docs/game-rules.md` `GR-029` §Điều kiện chỉ nói *"dialog Yes/No, nút xác nhận **tự tắt sau khi bấm**"* và tách riêng *"**Lý do là bắt buộc**"* — không gán hạng.

  Một bẫy từ ngữ đã được loại khỏi phép phân xử: `docs/permissions.md` gắn ⚠️ cho `PERM-043` `score.adjust` là *"thuộc bảy thao tác phá huỷ"*, nhưng đó là `QĐ-078`, một trục **phân quyền**, và danh sách bảy của nó còn gồm *sửa danh sách đề* — thứ `PRD-REQ-056` rõ ràng không xếp vào hạng dialog phá huỷ. Hai chữ *"phá huỷ"* trong repo chỉ hai trục khác nhau.

  </details>

  **Cổng truy nguyên — ĐÃ GHI `QĐ-108`** *(`docs/decisions.md` §J, ngay sau `QĐ-072`)*. `QĐ-108` khai hạng dialog xác định bằng **cách thoát**, *bắt lý do* là trục riêng, tầng giữa của điều chỉnh điểm, và quy tắc phím tắt `Y`/`N`; nó ghi **Thay cho** vế hình thức của hạng *Dialog Yes/No* trong `QĐ-072`. ⚠️ **Còn hai tài liệu chưa khớp**: **(b)** sửa `PRD-REQ-056` — nay mô tả **hai trục** *(tầng dialog theo cách thoát × có/không bắt lý do)* thay vì một danh sách bốn thao tác, và bổ sung quy tắc phím tắt `Y`/`N`; **(c)** sửa `STATE-035` — gỡ *điều chỉnh điểm* khỏi hạng phá huỷ và khai tầng riêng của nó. Ảnh hưởng: **FR-045**, **FR-060**, **FR-061**, **FR-061a**, **FR-063**. *(`QĐ-108` · `QĐ-072` · `PRD-REQ-056` · `STATE-035` · `GR-029` §Điều kiện, C1 · `QĐ-023` *(vế phím tắt)*)*

- **OQ-002 — ✅ ĐÃ PHÂN XỬ (§Clarifications 2026-08-03)**: **Vị thế của dialog cảnh báo lệch luật trong bộ hạng phản hồi.** Phân xử của chủ dự án: nó **dùng lại tầng Yes/No thường**, **không** là hạng thứ tư, và **không** đòi khác biệt thị giác với dialog xác nhận. Cách thoát y hệt tầng Yes/No thường — `Esc` và click ra ngoài đều = **No**, có phím tắt `Y`/`N`, không bắt lý do. Khác biệt duy nhất là **nội dung chữ**. Khác biệt về **quyền** so với toast invalid state *(cảnh báo ép được, toast không)* vẫn giữ nguyên. Đã tích hợp vào **FR-063**, **FR-063a** *(mới)* và **AC-070**.

  <details><summary>Nội dung câu hỏi trước khi phân xử</summary>

  `PRD-REQ-056` khai **ba** hạng — toast invalid state · dialog Yes/No cho thao tác không hoàn tác được nhưng hợp lệ · dialog hạng phá huỷ — và **không nhắc tới** dialog cảnh báo lệch luật. Nhưng `docs/PRD.md` §9.4 khai một bộ ba **khác**: invalid state · **cảnh báo lệch luật** · chặn cứng; và `docs/game-state-machine.md` tách `STATE-035`, `STATE-036`, `STATE-037` thành ba trạng thái riêng — tức bốn lớp phân biệt được.

  Cảnh báo lệch luật xuất hiện ở ít nhất bốn chỗ của epic này: chọn vòng lệch khuyến nghị (**FR-056**), đặt trạng thái ô chữ của câu đang hỏi dở (**FR-082**), kích hoạt tay lệch khuyến nghị (**FR-096**), và chọn người thi Về đích lệch thứ tự.

  </details>

  **Cổng truy nguyên — ĐÃ THOẢ, và KHÔNG cần `QĐ` mới.** Phát hiện khi cập nhật `docs/decisions.md` (2026-08-03): **`QĐ-072` đã khai đúng điều này từ trước** — *"Cảnh báo lệch luật là dialog Yes/No, không bắt lý do. Nó nêu tên luật đang bị lệch và không tự đóng"*. Spec này tưởng đây là câu hỏi mở vì `docs/decisions.md` **không nằm trong danh sách đọc** của lệnh `/speckit-specify`; phân xử của chủ dự án **trùng khớp** với `QĐ-072`, nên không có mâu thuẫn và không cần quyết định mới. ⚠️ **Còn một tài liệu chưa khớp**: `PRD-REQ-056` liệt kê ba hạng mà **bỏ sót** cảnh báo lệch luật — đó là chỗ `PRD.md` lệch `QĐ-072`, không phải một chỗ trống nghiệp vụ. Sửa cùng lúc với `OQ-001` vì hai thứ chạm cùng một requirement. Ảnh hưởng: **FR-063**, **FR-063a**. *(`QĐ-072` · `PRD-REQ-056` · `PRD §9.4` · `STATE-035` → `STATE-037`)*

- **OQ-003 — ✅ ĐÃ PHÂN XỬ (§Clarifications 2026-08-03)**: **Bản gửi quá hạn tới sau cú bấm *chốt câu* ở Tăng tốc.** Phân xử của chủ dự án: **tình huống bất khả thi về cấu trúc**, nên không cần nhánh xử lý. Cơ chế là một **cửa sổ giữ bản tới muộn phía server**, **không** phải ân hạn thao tác — nút gửi phía thí sinh tắt đúng tại `hạn chót` ở mọi vòng, và bản được giữ **vẫn là bản quá hạn**. Hai ràng buộc cộng lại tạo ra tính bất khả thi: **(a)** server chỉ giữ bản tới trong `(hạn chót, hạn chót + padding]` và từ chối sau mốc đó; **(b)** nút chấm và nút *chốt câu* ở kênh gõ chỉ mở **từ** `hạn chót + padding`. `padding` là **RuleConfig cấu hình được, mặc định 5 giây**. **Vế (a) thuộc EPIC-006** *(`specs/006` FR-070a, FR-070b)*; spec này chỉ sở hữu **vế (b)**. `GR-015` C6 vì thế đọc là: công nhận bản quá hạn xảy ra **trước** mốc chốt câu, và *"cả bảng tính lại"* mô tả việc bảng được tính **một lần** tại mốc chốt với dấu đã sửa — `INV-009` giữ nguyên, không cần ngoại lệ. Đã tích hợp vào **FR-033**, **FR-035** và **AC-038**, **AC-038a** *(mới)*, **AC-041**; vế quy tắc cửa sổ chuyển sang `specs/006` **FR-070a**, **FR-070b** *(§Clarifications 2026-08-03)*.

  <details><summary>Nội dung câu hỏi trước khi phân xử</summary>

  `docs/game-rules.md` `GR-015` C6 nói *"Nếu công nhận thì **cả bảng xếp hạng của câu tính lại**"* và `PRD-REQ-052` lặp lại điều đó ở §Acceptance intent, nhưng **không nguồn nào nêu mốc chót** để công nhận. Ở phía kia, `PRD-REQ-037` và `INV-009` nói sau khi câu khép thì *"không còn đường chấm lại cho bất kỳ ai"* và *"không tính lại thứ hạng"*, còn `GR-013` §Bấm trùng nói *"không có đường đổi phán quyết của một người **sau khi đã chốt câu**"*.

  </details>

  **Cổng truy nguyên — ĐÃ GHI `QĐ-109`** *(`docs/decisions.md` §F, ngay sau `QĐ-031`)*. `QĐ-109` khai **cửa sổ giữ bản tới muộn**, nêu rõ vì sao **không** gọi nó là *"dung sai"* *(chữ đó mời người đọc suy ra một sự khoan nhượng mà quy tắc không cấp — bản được giữ **vẫn quá hạn**)*, và nêu rõ nó **không** nới nút gửi phía thí sinh nên **không** là ngoại lệ của `QĐ-006` / `GR-033` §Điều kiện. **Quy tắc cửa sổ nay do `specs/006` sở hữu** *(FR-070a, FR-070b)*; spec này giữ **mốc mở nút chấm**. ⚠️ **Còn ba tài liệu chưa khớp** — `docs/` vẫn **không giới hạn** thời điểm một bản quá hạn có thể tới *(`GR-035` C6 chỉ nói giữ lại và tô đỏ)*, và bảng §*Kênh trả lời* vẫn nói nút chấm khoá *"tới khi hết giờ"*: **(b)** sửa `GR-035` C6 và `GR-015` để nêu biên trên của cửa sổ giữ bản tới muộn; **(c)** sửa bảng §*Kênh trả lời* để mốc mở nút chấm là `hạn chót + padding`; **(d)** thêm `padding` vào tập RuleConfig của contest builder *(EPIC-004)* với mặc định 5 giây. Ảnh hưởng: **FR-033**, **FR-035** · `specs/006` **FR-070a**, **FR-070b**. *(`QĐ-109` · `QĐ-006`, `QĐ-029` · `GR-015` C6 · `GR-035` C6 · bảng §Kênh trả lời · `PRD-REQ-052`, `PRD-REQ-054` · `INV-009` · `CLAUDE.md` §Quy ước code)*

- **OQ-004 — ✅ ĐÃ PHÂN XỬ (§Clarifications 2026-08-03)**: **Thứ tự giữa mốc cắt *công bố đáp án* và cú bấm phán quyết ở Khởi động mode nhập liệu.** Phân xử của chủ dự án: **bỏ hẳn khái niệm mốc cắt riêng**. `hạn chót` của câu là hạn nhận bài **duy nhất** ở Khởi động; `EVENT-016` gộp vào cú bấm **mở đáp án** chung của FR-065; bản được ghi nhận là bản hợp lệ **cuối cùng trước `hạn chót`**; nút chấm mở **từ `hạn chót`**. Kèm theo: **Khởi động không có biên trên và cũng không chờ** — nút gửi tắt đúng tại `hạn chót` như mọi vòng, nhưng một bản tới sau mốc đó **luôn** được ghi nhận và tô đỏ bất kể muộn bao nhiêu, và nút chấm mở ngay tại `hạn chót`; `GR-006` C4 và C5 vì thế **giữ nguyên hiệu lực**. Nhờ vậy *"bốn mốc bấm của một câu"* ở `EPIC-007 §11 Scope` nay đếm được chính xác: **hiển thị câu · start timer · bắt đầu thực hành *(chỉ câu thực hành)* · phán quyết**. Đã tích hợp vào **FR-009**, **FR-035** và **AC-011**, **AC-011a** *(mới)*, **AC-012**; vế *Khởi động không có biên trên* chuyển sang `specs/006` **FR-070b**.

  <details><summary>Nội dung câu hỏi trước khi phân xử</summary>

  `docs/game-rules.md` `GR-006` §Điều kiện định nghĩa **mốc cắt = admin bấm *"công bố đáp án"*** và dùng nó để xác định bản đáp án được ghi nhận. `docs/game-state-machine.md` `EVENT-016` khai *"Hợp lệ ở: `STATE-019`, `STATE-020` ở vòng Khởi động, mode nhập liệu"* còn `EVENT-012`/`EVENT-013` khai *"Hợp lệ ở: `STATE-019` (chỉ vòng nói) · `STATE-020` (mọi vòng)"* — **hai tập trạng thái giao nhau**, nên máy trạng thái cho phép bấm chấm **trước** mốc cắt. Thêm nữa, `GR-006` §Đồng thời nói *"chấm xong thì nút gửi khoá lại"*, tức phán quyết **cũng** đóng cửa nhận bài — hai mốc cùng đóng một cửa mà không nói mốc nào thắng.

  </details>

  **Cổng truy nguyên — ĐÃ GHI `QĐ-110`** *(`docs/decisions.md` §F, ngay sau `QĐ-109`)*. `QĐ-110` khai việc bỏ mốc cắt, lý do *(ba mốc tranh nhau đóng cùng một cửa nhận bài mà không cái nào thắng)*, và ghi **Thay cho** dòng cuối bảng của `QĐ-027`. ⚠️ **Còn hai tài liệu chưa khớp**: **(b)** sửa `GR-006` §Điều kiện và C3 để neo vào `hạn chót` thay vì mốc cắt; **(c)** **gỡ `EVENT-016`** khỏi `docs/game-state-machine.md` hoặc viết lại nó thành bí danh của cú bấm mở đáp án; **(d)** ghi rõ ở `GR-006` rằng Khởi động **không áp** biên trên của `OQ-003` và **không** chờ trước khi cho chấm, trong khi bản đến trễ do mạng vẫn luôn được ghi nhận và chấm được. Ảnh hưởng: **FR-009**, **FR-035** · `specs/006` **FR-070**, **FR-070b**. *(`QĐ-110` · `QĐ-027`, `QĐ-048` · `GR-006` §Điều kiện, C3, C4, C5, §Đồng thời · `GR-026` · `EVENT-016` · `INV-006` · `EPIC-007 §11 Scope`)*

**Không còn `NEEDS CLARIFICATION` nào tồn đọng.** Bốn chỗ trông như mâu thuẫn — `GR-037` C8 vs C6 ở Về đích, `GR-009` hai lựa chọn vs `PRD-REQ-113` ba lựa chọn, `GR-029` C4 *"điều chỉnh mồ côi"* vs hoàn nguyên của `GR-030`, và mặc định Sai của *chốt câu* vs *Huỷ kết quả* của kết thúc khẩn cấp — đều đã được chính `docs/` giải thích tường minh là **không cùng đối tượng** hoặc **ngoại lệ có chủ ý**; xem §Edge Cases → *Conflicting rule*.
...
|---|---|---|
| `GR-006` | C2, C3 *(bản cuối trước `hạn chót` — **mốc cắt đã bỏ**, `OQ-004`)* · C4 · C5 · §Đồng thời *(nút chấm khoá tới khi cửa sổ giữ bản tới muộn đóng)* | AC-011 · AC-011a, AC-035 · AC-011a, AC-036, AC-022 · AC-041 |
...
| `GR-035` | C6 *(bản quá hạn giữ lại, tô đỏ, không chặn cứng — nay có **biên trên** ở vòng có padding, `OQ-003`)* | AC-035, AC-037, AC-038a, AC-011a |
...
| **`QĐ-108`** *(`OQ-001`)* | ba tầng dialog phân biệt bằng cách thoát × trục *bắt lý do* độc lập; phím tắt `Y`/`N` trừ hạng phá huỷ | AC-052 · AC-066 · AC-067 · AC-069 |
| **`QĐ-072`** *(`OQ-002` — đã có từ trước)* | cảnh báo lệch luật dùng lại tầng Yes/No thường | AC-070 |
| **`QĐ-109`** *(`OQ-003`)* | **mốc mở nút chấm** = `hạn chót + padding` *(quy tắc cửa sổ giữ bản tới muộn thuộc `specs/006`)* | AC-038a · AC-011a |
| **`QĐ-110`** *(`OQ-004`)* | bỏ mốc bấm *công bố đáp án* khỏi vòng đời câu — một câu còn tối đa bốn mốc | AC-011 · AC-012 |
