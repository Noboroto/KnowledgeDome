...
2. **`docs/decisions.md` là nguồn hợp lệ và được dùng.** `CLAUDE.md` §1 khai nó là *"nơi duy nhất ghi vì sao"* và nó nằm trong `docs/`, nên nó thoả Cổng 1 của constitution. Hai quyết định gần đây chi phối trực tiếp epic này: **`QĐ-109`** *(nút gửi phía thí sinh tắt **đúng tại hạn chót** ở mọi vòng)* và **`QĐ-110`** *(Khởi động **không** còn mốc cắt riêng; hạn chót là hạn nhận bài duy nhất)*. `docs/game-rules.md` `GR-006` §Điều kiện **chưa** được sửa theo hai quyết định đó — đây là **độ trễ tài liệu đã biết**, đã ghi ở `specs/007` §Open Questions `OQ-003`, `OQ-004`, **không** phải một mâu thuẫn mới do spec này phát hiện.
3. **Chuẩn tắc từ ngữ**: *"hết giờ"* trong `PRD-REQ-065` và *"mốc cắt"* trong `GR-006` đều được đọc là **`hạn chót` của câu**, theo `QĐ-109` và `QĐ-110`. Spec này dùng chữ **`hạn chót`** ở mọi FR để khỏi tái tạo ba tên gọi cho một mốc.
...
- Q: `CONFLICT-001` — Phạm vi khoá của nút *"Mở chướng ngại vật"*: một CÂU hay một VÒNG? → A: **Không phải trục nào trong hai — khoá gắn với PHÁN QUYẾT.** Nút chỉ bị khoá **vĩnh viễn trong lần chạy vòng** sau khi tín hiệu của ghế đó đã đi qua một phán quyết **Đúng** hoặc **Sai**. Admin chấm **Huỷ kết quả** ⇒ *"không có gì xảy ra"* với ghế đó ⇒ nút **mở lại** và ghế bấm lại được trong cùng vòng. Cùng hành vi khi admin bấm **No** *(`GR-009` C8 đã hứa "thí sinh không mất lượt")*. Đã tích hợp vào **FR-003**, **FR-004**, **FR-004a** *(mới)*, **FR-004b** *(mới)* và **AC-009a**, **AC-009b** *(mới)*.
- Q: Vậy `QĐ-023` *(nút chuông tự khoá ngay khi bấm, trước khi gửi)* có bị phá không? → A: **Không.** Khoá **vẫn xảy ra ngay tại cú bấm**; chỉ **điều kiện gỡ khoá** đổi. `QĐ-023` tồn tại để chống **một cú bấm run tay thành hai tín hiệu**, không phải để đánh dấu *"ghế đã tiêu lượt"*. Hai mục đích trước nay bị gộp vào một cơ chế; phán quyết này tách chúng ra.
- Q: Vậy *"một lần đoán, sai thì loại"* của luật gốc còn giữ được không? → A: **Còn, và không cần khoá để cưỡng chế.** *"Một lần"* đếm theo **phán quyết**, không theo **cú bấm**: chấm **Đúng** ⇒ vòng kết thúc, nút biến mất cho mọi ghế (`GR-009` C10); chấm **Sai** ⇒ ghế mang cờ *bị loại*, máy ghế đó không hiển thị gì (`GR-009` C9, `STATE-022`). Khoá vĩnh viễn chỉ thừa ra ở đúng nhánh **Huỷ kết quả** — nhánh mà `GR-009` C6b nói thẳng là *"thí sinh KHÔNG bị loại"*.
- Q: Huỷ kết quả có tiêu **tín hiệu** không, hay không tiêu gì cả? → A: **Tiêu tín hiệu, KHÔNG tiêu quyền của ghế.** `GR-009` C6b khai *"tín hiệu sang **đã xử lý**"*, và `INV-006` đòi mọi tín hiệu đã tới server đều có outcome và ở lại lịch sử. *"Không có gì xảy ra"* nói về **ghế**, không nói về **tín hiệu**.
- Q: `CONFLICT-003` — Người cướp quyền Về đích tính **bản đầu** hay **bản cuối**? → A: **Bản CUỐI (Option B)**, theo một **nguyên tắc hai trục** của trò chơi mà chủ dự án phát biểu: **giành lượt thì luôn lấy người ĐẦU TIÊN · đáp án thì luôn lấy bản CUỐI CÙNG**. `GR-020` viết *"người cướp chỉ tính bản đầu tiên"* là lấy quy tắc của trục **giành quyền** áp nhầm sang trục **nội dung** — đó là lỗi trục, không phải một ngoại lệ có chủ ý. Đã tích hợp vào **FR-011**, **FR-013**, **FR-013a** *(mới)*, **FR-021**, **FR-021a** *(mới)* và **AC-024a** *(mới)*.
- Q: Nguyên tắc hai trục có đứng vững trên toàn bộ luật không? → A: **Có — `GR-020` là ngoại lệ DUY NHẤT trên 37 rule.** Trục **giành quyền** lấy đầu tiên: `GR-003` *(tín hiệu chuông sớm nhất)*, `GR-007` *(hàng đợi FIFO theo thứ tự tới)*. Trục **nội dung** lấy bản cuối: `GR-006` *(Khởi động)*, `GR-015` *(Tăng tốc)*, `GR-017` *(chọn gói, last-wins)*, `GR-018` *(Về đích người thi chính)*. Chứng cứ mạnh nhất là `GR-015`: Tăng tốc **xếp hạng theo tốc độ** — nơi có lý do mạnh nhất để lấy bản đầu — mà nguồn vẫn lấy **mốc của bản cuối**.
- Q: Bỏ *"bản đầu tiên"* thì nút gửi của người cướp tắt lúc nào, khi vòng này **không có `hạn chót` riêng**? → A: **Tại cú bấm chấm của admin** — quy tắc đã có sẵn ở `GR-006` §Đồng thời *("chấm xong thì nút gửi khoá lại")*, áp chung cho mọi ghế. Người cướp **không** cần một mốc riêng và **không** đẻ thêm knob cấu hình nào.
- Q: Vậy người cướp có được lợi thế thời gian so với người thi chính không? → A: **Bất đối xứng đó đã tồn tại sẵn, ở đúng mode mà luật được đặc tả theo.** Ở **mode sân khấu**, người cướp **nói** tự do cho tới khi admin bấm chấm — không đồng hồ nào chặn. Phán quyết này chỉ làm mode nhập liệu **giống** mode sân khấu. Thêm nữa, Về đích **không xếp hạng theo tốc độ** (`GR-018` C5) nên mốc thời gian của người cướp **không đi vào phép tính điểm nào**.
- Q: `CONFLICT-002` — Nút bị khoá theo luật chơi thì **ẩn** hay **mờ kèm nhãn**? → A: **Mờ kèm nhãn (Option B)**, và tiêu chí phân định là **control đó có nghĩa ở pha hiện tại hay không**. *Không có nghĩa ở pha này* ⇒ **ẩn** *(ngoài cửa sổ chuông · Câu hỏi phụ trước start timer · ghế bị loại · ghế bị vô hiệu hoá · mode sân khấu với ba nút chỉ-admin · hàng ngang đã mở · Ngôi sao hy vọng sau mốc hiển thị câu)*. *Có nghĩa nhưng ghế này bị luật cấm* ⇒ **render vô hiệu hoá kèm nhãn** *(đã bấm & bị chấm Sai · Ngôi sao hy vọng đã dùng · lượt chọn đã dùng · chưa tới lượt · đang chờ admin duyệt)*. Cả hai kết cục vẫn cho ra *"không tín hiệu nào được tạo"*. Đã tích hợp vào **FR-079**, **FR-086** và **AC-089**, **AC-089a** *(mới)*, **AC-096**.
- Q: Vì sao không chọn *"ẩn hết"* cho gọn? → A: **`PRD-REQ-072` đòi *"đánh dấu rõ đó là khoá theo luật chơi"*, mà một nút đã ẩn thì không đánh dấu được gì.** Chọn ẩn hết làm requirement đó mất đối tượng ở đúng những ca nó được viết ra để phục vụ. Nó cũng chỏi mục tiêu epic *"thí sinh luôn biết mình đang ở đâu"*: ghế bị chấm Sai thấy nút biến mất sẽ không phân biệt được **luật cấm** với **máy hỏng**.
- Q: *"Chưa tới lượt"* thuộc nhóm nào? → A: **Nhóm mờ kèm nhãn.** `GR-034` C5 đã xếp nó cạnh *đã trả lời sai* và *đã dùng hết quyền* trong cùng một dòng *"khoá theo luật chơi"*; tách nó ra sẽ chẻ chính danh sách của nguồn.
- Q: `OQ-002` — Máy thí sinh có báo gì khi tín hiệu của chính ghế đó thành **TRƠ**? → A: **Có — báo bằng nhãn trên chính nút chuông (Option B)**, tái dùng nhóm *mờ kèm nhãn* vừa chốt ở `CONFLICT-002` thay vì đẻ một bề mặt mới. Nhãn MUST phân biệt được *"bạn đã giành quyền"* với *"đã có người giành quyền trước"*, để một ghế bấm chuông không còn phải đoán giữa **thắng tốc độ**, **thua tốc độ** và **mất kết nối**. Phán quyết này **không** đụng luật: tín hiệu trơ vẫn là trơ, vẫn không đổi người giữ quyền và không mở lại chuông. Đã tích hợp vào **FR-005a** *(mới)*, **FR-086** và **AC-005a** *(mới)*.
- Q: Vì sao không dùng một lớp phủ hay toast riêng cho việc này? → A: **Thêm một lớp phủ là thêm một thứ che màn thi đấu đúng lúc căng nhất**, và nó phình `STATE-033`…`STATE-040` thêm một trạng thái chỉ để nói một câu mà cái nhãn đã nói được. Nút chuông lúc đó **vốn đã** phải mang nhãn theo `CONFLICT-002`; việc còn lại chỉ là bắt nhãn nói đúng lý do.
- Q: `OQ-001` — Sau khi admin **chọn hộ** gói câu ở mode nhập liệu, thí sinh còn đổi được không? → A: **Còn (Option A)** — cú chọn hộ là một **giá trị khởi tạo**, không phải một cú chốt. Thí sinh vẫn đổi được theo **last-wins** cho tới mốc **admin bấm hiển thị câu đầu tiên**, và nút chọn trên máy thí sinh **vẫn sống** sau cú chọn hộ. `GR-017` chỉ khai **một** mốc khoá duy nhất; đọc theo hướng ngược lại là đẻ ra một mốc khoá **thứ hai** mà nguồn chưa từng khai. `PRD-REQ-070` gọi nó là *"fallback"* — đường lui để **lượt thi không tắc**, không phải quyền chốt thay. Đã tích hợp vào **FR-057**, **FR-058** và **AC-063**.
- **Ghi chú về một hệ quả có sẵn, không phải một quyết định riêng**: `GR-009` C7 chốt băng điểm Chướng ngại vật theo **số hàng ngang đã hỏi tại mốc admin xác nhận**. Một ghế bấm lại sau cú Huỷ kết quả sẽ được xác nhận ở một mốc **muộn hơn**, nên băng của lần bấm thứ hai có thể **thấp hơn** lần đầu *(60 → 50 → 40…)*. Đây là hệ quả **đúng và tự nhất quán** của `GR-009`, không phải luật mới — nêu ra để không ai đọc băng tụt thành lỗi.
- Q: `OQ-003` — Dưới **banner tạm dừng**, bảng điểm và đồng hồ trên máy thí sinh còn cập nhật hay đóng băng? → A: **Banner CHE TOÀN BỘ màn thí sinh (Option C)** — thí sinh **không thấy** bảng điểm, **không thấy** đồng hồ, **không thấy** câu hỏi, nên câu hỏi *"còn cập nhật không"* không có chủ ngữ ở tầng hiển thị. Đây là **ngoại lệ tường minh và duy nhất** của `PRD-REQ-067`, ghi thành **`QĐ-115`**. Đã tích hợp vào **FR-029**, **FR-076a** *(mới)* và **AC-090a** *(mới)*.
- Q: Dữ liệu bên dưới có ngừng chảy không? → A: **Không — che ≠ đổi.** Server vẫn đẩy, client vẫn nhận, banner chỉ phủ lên trên. Tắt banner ⇒ thí sinh thấy **trạng thái hiện tại**, kể cả điểm đã đổi trong lúc bị che *(admin vẫn điều chỉnh điểm, bỏ vòng, hoàn nguyên được — `STATE-040` §Cho phép)*. Nhờ vậy `INV-021` **không** bị đụng và **không** phát sinh bài toán đồng bộ lại khi tắt banner.
- Q: Vì sao che kín chứ không chỉ chặn thao tác? → A: **Banner tồn tại để trận dừng THẬT.** Nếu thí sinh vẫn thấy câu hỏi đang mở và vẫn theo dõi bảng điểm để tính toán thì thứ đang dừng chỉ là **đồng hồ**, không phải **trận**. Nó cũng khớp `QĐ-050`: banner **KHÔNG CHỮ**, cố ý không truyền đạt gì — một tấm che như vậy mà vẫn để lộ bảng điểm bên dưới là mâu thuẫn nội tại.
- **Sửa một lỗi trong bản trước của spec này**: bản `AC-090a` đầu tiên mở kịch bản bằng *"một câu Tăng tốc đang đếm giờ, admin bật banner ở giây thứ 5"* — **trạng thái đó không dựng được**. `STATE-040` §Vào khai *"không có cửa sổ thời gian nào đang đếm"* là **điều kiện cứng**, và §Invalid transitions khai banner với đồng hồ **loại trừ lẫn nhau**. `QĐ-050` nói thẳng: *"câu hỏi banner có đóng băng đồng hồ không **không có chủ ngữ**"*. `AC-090a` đã viết lại trên một trạng thái hợp lệ, và vế **đồng hồ** bị gỡ khỏi cả `FR-076a` — dưới banner **không có đồng hồ nào** để chạy hay để che.
- Q: **Audit chéo 8 spec × PRD** — các epic hiện tại có mâu thuẫn với nhau không? → A: **Có hai mâu thuẫn cứng, và cả hai do chính phiên làm rõ này tạo ra.** `XCONF-1`: `specs/006` **FR-097** khai người cướp tính **bản đầu tiên**, ngược với phán quyết `CONFLICT-003` ở đây. `XCONF-2`: `specs/006` **FR-077** khai *"mỗi ghế phát tối đa **một** tín hiệu cho cả vòng"*, ngược với phán quyết `CONFLICT-001`. Phân xử của chủ dự án: **lan phán quyết sang `specs/006`** — spec 006 không sai về quy trình, nó chép đúng `GR-020` và `GR-009`; chỗ sai nằm ở `docs/`, và cả hai spec đều là tầng dưới phải sửa theo. `specs/006` **FR-077**, **FR-097**, **AC-118**, **AC-170** đã sửa; **AC-118a** mới; ba `XCONF` ghi vào §Open Questions của `specs/006`.
- Q: Nguyên tắc hai trục có ủng hộ `specs/006` FR-077 *("một tín hiệu cho cả vòng")* không, vì *"Mở chướng ngại vật"* là tín hiệu **giành quyền**? → A: **Không — đó là hai mệnh đề khác nhau.** *"Giành lượt lấy người đầu tiên"* là quy tắc **phân xử giữa các ghế đang đua**; nó **không** nói *"mỗi ghế chỉ được đua một lần"*. Cái FR-077 phát biểu là một **hạn mức**, đến từ luật gốc *"một lần đoán, sai thì loại"* — không thuộc trục nào trong hai. Sau một cú *Huỷ kết quả*, `GR-032` §Kích hoạt tay vẫn **khuyến nghị tín hiệu sớm nhất chưa xử lý**, tức *"người đầu tiên"* áp trên tập còn hiệu lực; ghế bấm lại chỉ **thêm một tín hiệu vào tập đang đua**. Nguyên tắc giữ nguyên hiệu lực, không bị nới.
- Q: `XCONF-3` — sau phán quyết `CONFLICT-001`, chuỗi kích hoạt tay ở VCNV còn điều kiện dừng không? → A: **Không phải quyết định mới; chỉ cần sửa câu chữ.** `GR-032` §Kích hoạt tay nói chuỗi *"dừng khi hàng đợi cạn"*, nhưng `PRD-REQ-113` **đã** khai hạn mức là *"một lần cho mỗi phán quyết Huỷ kết quả ở VCNV và **không có trần tổng**"*. Nguồn vốn đã chấp nhận chuỗi không trần, và mỗi vòng lặp đòi **một cú bấm *Huỷ kết quả* của admin** nên nó bị chặn bởi ý chí của người — đúng `QĐ-001`. Câu *"dừng khi hàng đợi cạn"* là mô tả một **kết cục thường gặp**, không phải một bất biến; cần viết lại cho khỏi đọc thành bảo đảm.
- **Chứng cứ trong nguồn cho phán quyết này** *(không phải một quyết định mới, mà là làm rõ ý đồ đã có)*: `GR-032` §Kích hoạt tay khai tập ứng viên gồm *"ghế **vừa bị Huỷ kết quả** ở chính câu/vòng đó *(được đưa lại, tức **nhận lượt thứ hai**)*"* — nguồn **đã** cho ghế bị Huỷ kết quả một lượt thứ hai, chỉ khác là qua đường admin kích hoạt tay. Phán quyết này cho ghế đó **tự bấm lại**; cùng nguyên tắc `QĐ-061` *(Huỷ kết quả = "lượt này coi như không xảy ra")*, khác đường vào.
...
- **`CONFLICT-001`** — phạm vi khoá của nút *"Mở chướng ngại vật"*: **đã phân xử** — khoá gắn với **phán quyết**, không gắn với câu cũng không gắn với vòng; xem §Clarifications 2026-08-03 và §Open Questions. Bốn tài liệu vẫn cần sửa cho khớp.
- **`CONFLICT-002`** — nút bị khoá theo luật chơi thì ẩn hay mờ: **đã phân xử** — tiêu chí là **control có nghĩa ở pha hiện tại hay không**; mất nghĩa ⇒ ẩn, bị luật cấm ⇒ mờ kèm nhãn. Xem §Clarifications 2026-08-03 và §Open Questions. Bốn tài liệu vẫn cần sửa cho khớp.
- **`CONFLICT-003`** — bản được ghi nhận cho **người cướp quyền Về đích**: **đã phân xử** — **bản cuối**, theo nguyên tắc hai trục *(giành lượt lấy người đầu tiên · đáp án lấy bản cuối cùng)*; `GR-020` áp nhầm quy tắc của trục giành quyền sang trục nội dung. Xem §Clarifications 2026-08-03 và §Open Questions. Bốn tài liệu vẫn cần sửa cho khớp.
- **`PRD-REQ-069` *"cùng quy tắc một-đường-vào áp cho Ngôi sao hy vọng"* vs `GR-021` *"tín hiệu có hiệu lực NGAY, không chờ duyệt"* — không phải conflict.** Mệnh đề của `PRD-REQ-069` chỉ đích danh *"quy tắc **một-đường-vào**"*, không nói *"ba trạng thái"*; ba trạng thái là hệ quả của **hàng đợi chặn**, mà Về đích là vòng hàng đợi **không** chặn (`GR-032` §Điều kiện). Hai câu nói về hai thứ khác nhau.
- **`GR-034` §Điều kiện *"`Esc` xoá ô nhập"* vs `PRD-REQ-071` *"mode sân khấu `Esc` không làm gì"* — không phải conflict.** Ở mode sân khấu **không tồn tại** ô nhập ở Khởi động / Về đích / Câu hỏi phụ, nên vế *"xoá ô nhập"* là vô đối tượng chứ không bị phủ định. Ở Vượt chướng ngại vật và Tăng tốc — hai vòng **luôn gõ máy** — ô nhập có tồn tại ở cả hai mode và `Esc` xoá nó bình thường.
- **`GR-006` §Điều kiện *"mốc cắt = admin bấm công bố đáp án"* vs `QĐ-110` *"bỏ hẳn mốc cắt"* — không phải conflict mới.** `QĐ-110` là quyết định **sau** và ghi rõ **Thay cho** dòng tương ứng của `QĐ-027`; `GR-006` chỉ **chưa được sửa theo**. Độ trễ tài liệu này đã được `specs/007` `OQ-004` ghi nhận và không phát sinh từ spec này.

**Missing source behavior**

- **`OQ-001`** — thí sinh còn đổi gói sau cú chọn hộ của admin: **đã phân xử** — **còn**; chọn hộ là giá trị khởi tạo, mốc khoá duy nhất vẫn là cú bấm hiển thị câu đầu tiên. Xem §Clarifications 2026-08-03 và §Open Questions.
- **`OQ-002`** — phản hồi khi tín hiệu của chính ghế đó thành **TRƠ**: **đã phân xử** — báo bằng **nhãn trên chính nút vừa bấm**, không dùng lớp phủ riêng. Xem §Clarifications 2026-08-03 và §Open Questions.
- **`OQ-003`** — máy thí sinh dưới **banner tạm dừng**: **đã phân xử** — banner **che toàn bộ màn**, là ngoại lệ tường minh và duy nhất của `PRD-REQ-067` (`QĐ-115`). Xem §Clarifications 2026-08-03 và §Open Questions.
...
- **FR-003**: Nút chuông MUST tự khoá **ngay trong lần bấm đầu, ở giao diện, TRƯỚC khi tín hiệu rời máy**, để một ghế MUST NOT phát được quá một tín hiệu chuông đang **chờ xử lý** tại một thời điểm. Khoá này MUST chỉ phục vụ việc chống một cú bấm thành hai tín hiệu, MUST NOT bị dùng làm cơ chế đánh dấu *"ghế đã tiêu lượt"* — hai mục đích khác nhau *(§Clarifications 2026-08-03)*. *(US-001 · PRD-REQ-064 · GR-034 §Bấm trùng, STATE-028 · AC-004)*
- **FR-004**: Điều kiện **gỡ khoá** MUST theo loại nút. Với **nút chuông thường** *(Khởi động lượt chung · cướp quyền Về đích · Câu hỏi phụ)*: khoá gắn với **một câu**, và khi câu kết thúc hệ thống MUST gỡ khoá cho mọi ghế. Với **nút *"Mở chướng ngại vật"***: khoá MUST chỉ trở thành **vĩnh viễn trong lần chạy vòng** khi tín hiệu của ghế đó đã đi qua một phán quyết **Đúng** hoặc **Sai**; khi tín hiệu bị admin **từ chối** hoặc bị chấm **Huỷ kết quả**, hệ thống MUST **mở lại** nút cho ghế đó và ghế MUST bấm lại được trong cùng vòng *(§Clarifications 2026-08-03)*. *(US-001 · PRD-REQ-064 · GR-034 §Bấm trùng, GR-009 C6b, C8, GR-032 §Kích hoạt tay, STATE-028 §Ra · QĐ-061 · AC-010, AC-009a, AC-009b)*
...
- **FR-013a**: Toàn hệ thống MUST theo **nguyên tắc hai trục** khi phân xử nhiều tín hiệu của cùng một ghế *(§Clarifications 2026-08-03)*:
...
- **FR-066**: Ba ca biên MUST giữ đúng: câu **bị bỏ qua** MUST vẫn được công bố · phán quyết **Huỷ kết quả** MUST NOT tự công bố · đáp án **Chướng ngại vật** MUST NOT đi theo cơ chế này. *(US-009 · PRD-REQ-049 · GR-037 C7, C8, C9 · AC-073, AC-074, AC-075)*
...
- **FR-079**: Máy thí sinh MUST phân biệt **hai** kết cục hiển thị cho một control không bấm được, và tiêu chí phân định MUST là **control đó có nghĩa ở pha hiện tại hay không** *(§Clarifications 2026-08-03)*:
...
- **Nguyên tắc hai trục**: Quy tắc phái sinh dùng cho **mọi** cơ chế có nhiều tín hiệu từ cùng một ghế. Trục **giành quyền** *(ai được làm)* lấy tín hiệu **đầu tiên**; trục **nội dung** *(làm ra cái gì)* lấy bản **cuối cùng**. Gặp một cơ chế mới thì hỏi nó thuộc trục nào rồi suy ra. *(§Clarifications 2026-08-03 · `GR-003`, `GR-006`, `GR-007`, `GR-015`, `GR-017`, `GR-018`, `GR-020`)*
...
- **Khoá theo luật chơi**: Trạng thái vô hiệu hoá **hợp lệ** của một nút **vẫn được render**, kèm nhãn nêu lý do, do trạng thái game gây ra. Đúng năm ca: đã bấm & bị chấm Sai · Ngôi sao hy vọng đã dùng · lượt chọn đã dùng · chưa tới lượt · đang chờ admin duyệt. Khác hẳn **hai** thứ: disable nút để **chống gửi trùng** *(bị cấm)*, và **control mất nghĩa ở pha hiện tại** *(không render)*. *(`GR-034` C5, `PRD` §9.4, `QĐ-060` · §Clarifications 2026-08-03)*
...
> Sáu mục dưới đây được ghi theo Nguyên tắc II và III của `.specify/memory/constitution.md`. Mục nào **chưa** được chủ dự án phân xử thì **giữ nguyên, không tự trả lời**; mục đã phân xử giữ lại dưới dạng **bản ghi đã đóng** để truy vết. Mọi FR mang cảnh báo ⚠️ trỏ tới một mục ở đây MUST NOT vào task implementation trước khi `docs/` được sửa.
>
> **Tiến độ phiên làm rõ 2026-08-03**: **cả sáu mục đã được phân xử** — `CONFLICT-001` *(khoá chuông gắn với **phán quyết**)* · `CONFLICT-002` *(ẩn vs mờ phân định bằng **control có nghĩa ở pha này không**)* · `CONFLICT-003` *(**nguyên tắc hai trục**: giành lượt lấy đầu tiên, đáp án lấy bản cuối)* · `OQ-001` *(chọn hộ gói câu là **giá trị khởi tạo**)* · `OQ-002` *(outcome của tín hiệu **báo về máy phát** bằng nhãn trên nút)* · `OQ-003` *(banner tạm dừng **che toàn bộ màn thí sinh** — ngoại lệ duy nhất của `PRD-REQ-067`)*. **Không còn mục nào chờ phân xử.**
>
> ⚠️ **Mức chặn còn lại là TRUY NGUYÊN, không phải quyết định.** Phán quyết phát biểu trong phiên làm rõ nên chưa là nguồn; `docs/decisions.md` cần **năm** `QĐ` mới — `QĐ-111` → `QĐ-115` *(`OQ-001` chỉ cần một câu bổ sung vào `GR-017` C3)* — và `PRD-REQ-064`, `PRD-REQ-067`, `PRD-REQ-072`, `PRD` §9.4, `GR-003`, `GR-009`, `GR-017`, `GR-018`, `GR-020`, `GR-032`, `GR-034`, `STATE-028`, `STATE-040`, `EVENT-040` cần sửa theo. **Toàn bộ đã hoàn tất ngày 2026-08-03** — xem `checklists/requirements.md`.

### CONFLICT

- **`CONFLICT-001` — ✅ ĐÃ PHÂN XỬ (§Clarifications 2026-08-03)**: **Phạm vi khoá của nút *"Mở chướng ngại vật"*.** Phán quyết của chủ dự án: **không phải trục nào trong hai** — khoá gắn với **PHÁN QUYẾT**, không gắn với câu cũng không gắn với vòng. Nút chỉ khoá **vĩnh viễn trong lần chạy vòng** sau một phán quyết **Đúng** hoặc **Sai**; chấm **Huỷ kết quả** hoặc bấm **No** ⇒ nút **mở lại**. `QĐ-023` không bị phá vì khoá **vẫn** xảy ra ngay tại cú bấm — chỉ điều kiện **gỡ khoá** đổi. *"Một lần đoán, sai thì loại"* của luật gốc đếm theo **phán quyết**, và hai nhánh Đúng/Sai **tự** cưỡng chế nó qua `GR-009` C9, C10 mà không cần khoá. Đã tích hợp vào **FR-003**, **FR-004**, **FR-004a**, **FR-004b** và **AC-009a**, **AC-009b**.

  <details><summary>Nội dung mâu thuẫn trước khi phân xử</summary>

  **CONFLICT**: Nút *"Mở chướng ngại vật"* được xếp là chuông, nhưng phạm vi khoá của nó được hai nguồn khai khác nhau — Nguồn A: `docs/PRD.md` §12 `PRD-REQ-064` nói *"Nút *"Mở chướng ngại vật"* MUST được xếp là chuông"* rồi ngay sau đó nói *"khoá MUST gắn với **một câu** — sang câu mới thì mở lại"*, tức nút này mở lại ở mỗi câu hàng ngang mới; Nguồn B: `docs/game-rules.md` `GR-009` §Điều kiện nói *"Mỗi ghế phát **một** tín hiệu cho cả vòng — cơ chế *"một lần đoán, sai thì loại"* của luật gốc được giữ nguyên"*, tức khoá gắn với **vòng**. Nguồn thứ ba `docs/game-state-machine.md` `STATE-028` mang **cả hai vế trong cùng một mục**: §Mô tả nói *"ở VCNV mỗi ghế chỉ phát một tín hiệu cho cả vòng"*, còn §Ra nói *"câu kết thúc ⇒ gỡ khoá cho mọi ghế"*.

  Phán quyết cho thấy **cả hai phía đều sai một phần**, vì cả hai tranh luận trên trục *"phạm vi thời gian"* trong khi trục đúng là *"đã đi qua phán quyết chưa"*. Nguồn B sai ở chữ **"tín hiệu"** — thứ luật gốc giới hạn là số lần **được chấm**, không phải số lần bấm.

  </details>

  **Cổng truy nguyên — ⚠️ CHƯA THOẢ, cần ghi `QĐ` mới.** Phán quyết phát biểu trong phiên làm rõ nên theo Nguyên tắc I và Cổng 1 của constitution nó **chưa** là nguồn. Cần **(a)** một `QĐ` mới trong `docs/decisions.md` khai *khoá chuông gắn với phán quyết*, tách hai mục đích của `QĐ-023` *(chống bấm trùng vs đánh dấu đã tiêu lượt)*, và khai vế mở lại khi Huỷ kết quả / bấm No; **(b)** sửa `PRD-REQ-064` — *"khoá gắn với một câu"* chỉ đúng cho chuông thường; **(c)** sửa `GR-009` §Điều kiện — *"mỗi ghế phát **một tín hiệu** cho cả vòng"* đọc lại thành *"mỗi ghế đi qua tối đa **một phán quyết Đúng/Sai** cho cả vòng"*, và bổ sung vế **mở lại nút** vào C6b và C8; **(d)** sửa `STATE-028` §Ra — tách hai nhánh gỡ khoá theo loại nút. Ảnh hưởng: **FR-003**, **FR-004**, **FR-004a**, **FR-004b**. *(`PRD-REQ-064` · `GR-009` §Điều kiện, C6, C6b, C8, C9, C10 · `GR-032` §Kích hoạt tay · `GR-034` §Bấm trùng · `STATE-028` §Mô tả, §Ra · `QĐ-023`, `QĐ-061`, `QĐ-104` · `INV-006`, `INV-019`)*

- **`CONFLICT-002` — ✅ ĐÃ PHÂN XỬ (§Clarifications 2026-08-03)**: **Nút bị khoá theo luật chơi: ẩn hay mờ kèm nhãn.** Phán quyết của chủ dự án: **mờ kèm nhãn**, với tiêu chí phân định là **control có nghĩa ở pha hiện tại hay không**. *Không có nghĩa* ⇒ **không render**; *có nghĩa nhưng ghế này bị luật cấm* ⇒ **render vô hiệu hoá kèm nhãn nêu lý do*. Cả hai kết cục vẫn cho ra *"không tín hiệu nào được tạo"*, nên vế mà cả hai nguồn đồng ý **không** bị đụng tới. Đã tích hợp vào **FR-079**, **FR-086** và **AC-089**, **AC-089a**, **AC-096**.

  <details><summary>Nội dung mâu thuẫn trước khi phân xử</summary>

  **CONFLICT**: Cùng một tình huống được hai chỗ khai hai kết cục nhìn thấy được khác nhau — Nguồn A: `docs/game-rules.md` `GR-003` C5 nói *"Ghế đã bấm và bị chấm Sai ở câu này, bấm tiếp ⇒ Nút **không hiển thị**, bấm **không phản hồi** ⇒ **không tín hiệu nào được tạo**"*, và `GR-034` C6 dùng cùng cách nói cho ca ngoài cửa sổ; Nguồn B: `docs/game-rules.md` `GR-034` C5 nói *"Nút bị khoá theo luật chơi — **đã trả lời sai**, đã dùng hết quyền, chưa tới lượt ⇒ **Nút ở trạng thái khoá**; không có tín hiệu nào được tạo"*, và `docs/PRD.md` §12 `PRD-REQ-072` củng cố phía đó: *"Khoá **theo luật chơi** — chuông đã bấm, Ngôi sao hy vọng đã dùng, chưa tới lượt — MUST vẫn vô hiệu hoá bình thường, và hệ thống MUST **đánh dấu rõ** đó là khoá theo luật chơi"*.

  Phán quyết cho thấy **Nguồn A sai ở đúng một ô**: `GR-003` C5 gộp ca *"đã bấm và bị chấm Sai"* — một ca **khoá theo luật chơi** — vào cùng kết cục với ca *"ngoài cửa sổ"* ở C6, vốn là ca **mất nghĩa**. Hai ca đó thuộc hai nhóm khác nhau và không được cho cùng kết cục hiển thị.

  </details>

  **Cổng truy nguyên — ⚠️ CHƯA THOẢ, cần ghi `QĐ` mới.** Cần **(a)** một `QĐ` mới trong `docs/decisions.md` khai tiêu chí *có nghĩa ở pha này hay không* làm ranh giới giữa **ẩn** và **mờ kèm nhãn**, kèm hai danh sách ca; **(b)** sửa `GR-003` C5 — ca *"đã bấm và bị chấm Sai"* chuyển sang kết cục **mờ kèm nhãn**, tách khỏi C6; **(c)** làm rõ ở `PRD` §9.4 rằng hạng *invalid state* nói về **quyền ép được**, không tự nó quy định ẩn hay mờ; **(d)** bổ sung vào `PRD-REQ-072` danh sách năm ca của nhóm mờ, gồm cả ca **đang chờ admin duyệt** mà requirement hiện chưa kể. Ảnh hưởng: **FR-079**, **FR-086**. *(`GR-003` C5 · `GR-034` C5, C6 · `PRD-REQ-071`, `PRD-REQ-072` · `PRD` §9.4 · `STATE-026` → `STATE-029` · `QĐ-004`, `QĐ-060`)*

- **`CONFLICT-003` — ✅ ĐÃ PHÂN XỬ (§Clarifications 2026-08-03)**: **Bản được ghi nhận cho người cướp quyền Về đích.** Phán quyết của chủ dự án: **bản CUỐI**, theo một **nguyên tắc hai trục** của trò chơi — *giành lượt thì luôn lấy người **đầu tiên**; đáp án thì luôn lấy bản **cuối cùng***. `GR-020` lấy quy tắc của trục **giành quyền** áp nhầm sang trục **nội dung**; đây là **lỗi trục**, không phải một ngoại lệ có chủ ý. Mốc đóng của người cướp là **cú bấm chấm của admin** theo `GR-006` §Đồng thời — vòng này **không** cần một `hạn chót` riêng. Đã tích hợp vào **FR-011**, **FR-013**, **FR-013a**, **FR-021**, **FR-021a** và **AC-024a**.

  <details><summary>Nội dung mâu thuẫn trước khi phân xử</summary>

  **CONFLICT**: Nguồn A: `docs/PRD.md` §12 `PRD-REQ-065` phát biểu **không điều kiện** cho toàn EPIC-008 rằng *"hệ thống MUST ghi nhận **bản cuối cùng** trong các bản hợp lệ"*, và trường **Related game rules** của nó chỉ khai `GR-006`, `GR-015`, `GR-034` — **không** khai `GR-020`; Nguồn B: `docs/game-rules.md` `GR-020` §Điều kiện nói *"**Người cướp chỉ tính BẢN ĐẦU TIÊN** — ngược với người thi chính, vốn tính bản cuối"*, có bảng quyết định C4 minh hoạ, và `docs/game-state-machine.md` `EVENT-040` §Ngoại lệ lặp lại: *"người **cướp quyền** ở Về đích chỉ tính **bản ĐẦU TIÊN** — ngược với mọi chỗ khác"*.

  Phán quyết cho thấy **Nguồn A đúng, và việc nó không khai `GR-020` là nhất quán chứ không phải sót**. Đối chiếu toàn bộ 37 rule: trục **giành quyền** lấy đầu tiên ở `GR-003`, `GR-007`; trục **nội dung** lấy bản cuối ở `GR-006`, `GR-015`, `GR-017`, `GR-018`. `GR-020` là **ngoại lệ duy nhất**. Chứng cứ mạnh nhất là `GR-015`: Tăng tốc **xếp hạng theo tốc độ** — nơi có lý do mạnh nhất để lấy bản đầu — mà nguồn vẫn lấy **mốc của bản cuối**.

  </details>

  **Cổng truy nguyên — ⚠️ CHƯA THOẢ, cần ghi `QĐ` mới.** Cần **(a)** một `QĐ` mới trong `docs/decisions.md` khai **nguyên tắc hai trục** như một quy tắc phái sinh dùng cho mọi cơ chế tương lai, và ghi **Thay cho** vế *bản đầu tiên* của `QĐ-058`; **(b)** sửa `GR-020` §Điều kiện — **bỏ** câu *"Người cướp chỉ tính BẢN ĐẦU TIÊN"*; **(c)** sửa `GR-020` **C4** — chuỗi `"Paris"` → `"London"` → `"Berlin"` nay chấm trên **`"Berlin"`**; **(d)** sửa `GR-018` §Điều kiện — bỏ câu chú *"(Người cướp quyền thì ngược lại — chỉ tính bản đầu tiên, xem `GR-020`.)"*; **(e)** **xoá** `EVENT-040` §Ngoại lệ. **KHÔNG** phải sửa `PRD-REQ-065` — đây là phương án duy nhất trong ba phương án để nó nguyên vẹn. Ảnh hưởng: **FR-011**, **FR-013**, **FR-013a**, **FR-021**, **FR-021a**. *(`PRD-REQ-065` · `GR-020` §Điều kiện, C4, §Thứ tự đánh giá · `GR-018` §Điều kiện, C5 · `GR-006` §Đồng thời · `GR-015` · `GR-032` §Kích hoạt tay · `EVENT-040` §Ngoại lệ · `QĐ-029`, `QĐ-058`)*

### Mâu thuẫn CHÉO giữa các spec — phát hiện trong audit 2026-08-03

> Ba mục dưới đây **không** phải mâu thuẫn trong `docs/` mà là mâu thuẫn **giữa hai spec anh em**, phát sinh vì `specs/006` được viết **trước** phiên làm rõ này và đã chép đúng theo `docs/` bản chưa sửa. Cả ba đã được chủ dự án phân xử theo hướng **lan phán quyết**, và `specs/006` đã được sửa.

- **`XCONF-1` — ✅ ĐÃ HỢP NHẤT**: `specs/006` **FR-097** *("Người cướp MUST được tính theo **bản ĐẦU TIÊN**")* vs `CONFLICT-003` ở spec này *(**bản cuối**)*. Đã sửa `specs/006` FR-097 và AC-170. Cùng bộ sửa `docs/` với `CONFLICT-003`.
- **`XCONF-2` — ✅ ĐÃ HỢP NHẤT**: `specs/006` **FR-077** *("mỗi ghế phát tối đa **một** tín hiệu cho cả vòng")* vs `CONFLICT-001` ở spec này *(hạn mức đếm theo **phán quyết**)*. Đã sửa `specs/006` FR-077, AC-118, thêm AC-118a. Cùng bộ sửa `docs/` với `CONFLICT-001`.
- **`XCONF-3` — ✅ KHÔNG PHẢI MÂU THUẪN**: `GR-032` §Kích hoạt tay nói chuỗi kích hoạt ở Vượt chướng ngại vật *"dừng khi hàng đợi cạn"*, nhưng `PRD-REQ-113` đã khai **không có trần tổng** và mỗi vòng lặp đòi một cú bấm *Huỷ kết quả* của admin. ⚠️ Chỉ cần **sửa câu chữ** của `GR-032`, không phải một quyết định.

**Hai lỗ TRUY NGUYÊN phát hiện cùng đợt** *(không phải mâu thuẫn hành vi, ghi lại để đóng)*:

- **`PRD-REQ-074` — ✅ ĐÃ ĐÓNG 2026-08-03**: requirement khai `EPIC-009, EPIC-001` nhưng `specs/001` xếp trọn nó vào EPIC-009 ở §8 Out of scope, bỏ sót **mặt EPIC-001**. Nội dung vốn đã được phủ — `specs/001` **FR-021** và **FR-033** khai đúng *"bề mặt quyền ghi duy nhất của MC"* — chỉ sai neo. Đã sửa: bảng §0 nhận `PRD-REQ-074` kèm giới hạn *"chỉ mệnh đề quyền ghi"*, §8 tách hai mặt, FR-021 và FR-033 thêm neo, FR-033 bổ sung vế **server từ chối mọi sự kiện ghi khác**, AC-048 mở rộng sang ba loại sự kiện, ma trận truy nguyên US-004 và US-005 cập nhật. Cùng đợt đã bổ sung `PRD-REQ-108` → `PRD-REQ-112` vào bảng §0 — chúng ra đời sau bản đầu của `specs/001` nên các FR đã trích mà bảng chưa có.
- **`PRD-REQ-098` — ⏳ CÒN MỞ**: khai `EPIC-003`, `EPIC-010`, `EPIC-012`. `specs/003` đã chủ động xác nhận `X2`/`X4` **không** thuộc nó, nhưng **không nhận** requirement này theo mã; mặt EPIC-003 còn để trống là ràng buộc *"xuất gói contest **không phụ thuộc dịch vụ ngoài hay kết nối Internet**"*. Vế *"cả hai hồ sơ triển khai"* thuộc **EPIC-012 — chưa có spec**, nên phần đó là **nợ đã biết**, không phải lỗ hổng.

  > ⚠️ **Lưu ý cho đợt audit sau**: từ nay `PRD-REQ-098` **có xuất hiện** trong `specs/008` — chính ở dòng này. Phép dò orphan bằng cách đếm mã xuất hiện trong spec sẽ **không** còn bắt được nó. Dò theo *"spec nào có FR neo vào mã đó"* thay vì *"mã đó có được nhắc tới không"*.

Sáu `PRD-REQ` orphan còn lại — `077`, `078`, `080`, `082`, `083`, `084` — đều thuộc **EPIC-009/010/011**, chưa có spec. Đúng dự kiến.

**Đã kiểm và SẠCH**: `PRD-REQ-025`, `PRD-REQ-105`, `PRD-REQ-113`, `PRD-REQ-114` — mỗi cái được ba spec chia theo **facet** rõ ràng, không chồng lấn và không phát biểu ngược nhau. Phán quyết `CONFLICT-002` *(ẩn vs mờ)* **không** chỏi `specs/006` — spec đó không phát biểu gì về ca *"đã bấm và bị chấm Sai"*.

> **Giới hạn của đợt audit**: đã đối chiếu **(a)** `PRD-REQ` orphan, **(b)** `PRD-REQ` bị nhiều spec cùng nhận, **(c)** ba phán quyết của phiên làm rõ với spec anh em. **Chưa** làm diff ngữ nghĩa từng cặp trên toàn bộ ~7.300 dòng của 8 spec.

### NEEDS CLARIFICATION

- **`OQ-001` — ✅ ĐÃ PHÂN XỬ (§Clarifications 2026-08-03)**: **Thí sinh còn đổi gói sau cú chọn hộ của admin.** Phán quyết của chủ dự án: **còn** — cú chọn hộ là **giá trị khởi tạo**, không phải cú chốt; nút chọn của thí sinh **vẫn sống** và bản-cuối-thắng vẫn áp cho tới mốc **admin bấm hiển thị câu đầu tiên**, vốn là **mốc khoá duy nhất**. Đã tích hợp vào **FR-057**, **FR-058** và **AC-063**.

  <details><summary>Nội dung câu hỏi trước khi phân xử</summary>

  `docs/game-rules.md` `GR-017` C3 nói admin *"chọn hộ mặc định 20/20/20, override được trong dialog"* khi thí sinh chưa chọn lúc tới lượt, và §Điều kiện nói *"đổi gói được, cho tới mốc admin bấm hiển thị câu ĐẦU TIÊN — last-wins tới mốc đó rồi khoá"*. Hai câu này **không nói ai** được hưởng last-wins sau cú chọn hộ.

  Phán quyết dựa trên hai điểm đã có trong nguồn: `GR-017` chỉ khai **một** mốc khoá, và `GR-017` §Khác `QĐ-019` nói thẳng lý do fallback tồn tại là *"lượt thi cần một đường thoát để **không tắc**"* — chỉ cần không tắc, không cần tước quyền chọn của thí sinh.

  </details>

  **Cổng truy nguyên — ⚠️ CHƯA THOẢ, cần sửa `docs/`.** Cần bổ sung vào `GR-017` C3 một vế nói rõ cú chọn hộ **không** khoá nút của thí sinh và **không** tạo mốc khoá thứ hai. Mức nhẹ hơn bốn mục trên: đây là **làm rõ một chỗ im lặng**, không đảo ngược mệnh đề nào, nên có thể xử lý bằng một câu bổ sung thay vì một `QĐ` mới — trừ khi chủ dự án muốn ghi `QĐ` cho nhất quán hồ sơ. Ảnh hưởng: **FR-057**, **FR-058**. *(`GR-017` C3, C6, §Điều kiện, §Khác `QĐ-019` · `PRD-REQ-070`)*

- **`OQ-002` — ✅ ĐÃ PHÂN XỬ (§Clarifications 2026-08-03)**: **Phản hồi trên máy thí sinh khi tín hiệu của chính ghế đó thành TRƠ.** Phán quyết của chủ dự án: **có báo**, bằng **nhãn trên chính nút vừa bấm**, tái dùng nhóm *mờ kèm nhãn* của `CONFLICT-002` — **không** dùng lớp phủ hay toast riêng. Nhãn phân biệt ít nhất ba kết cục: *đã giành quyền* · *thua tốc độ, tín hiệu trơ* · *đang chờ admin duyệt*. Không đụng luật: trơ vẫn không đổi người giữ quyền và không mở lại chuông. Đã tích hợp vào **FR-005a** *(mới)*, **FR-086** và **AC-005a** *(mới)*.

  <details><summary>Nội dung câu hỏi trước khi phân xử</summary>

  `docs/game-rules.md` `GR-003` C3 nói tín hiệu thua tốc độ thành **trơ** — *"ghi làm căn cứ cho admin, **không** đổi người giữ quyền, **không** mở lại chuông"* — và `INV-006` bảo đảm mọi tín hiệu đã tới server đều có **outcome**. Nhưng không nguồn nào nói outcome đó có được **báo về máy phát tín hiệu** hay không. Một ghế bấm chuông rồi thấy nút khoá mà không có tín hiệu nào khác sẽ không phân biệt được *"tôi giành được quyền"*, *"tôi thua tốc độ"* và *"máy tôi mất kết nối"*.

  </details>

  **Cổng truy nguyên — ⚠️ CHƯA THOẢ, cần ghi `QĐ` mới.** Cần **(a)** một `QĐ` mới trong `docs/decisions.md` khai rằng `INV-006` *(mọi tín hiệu có outcome)* kéo theo **nghĩa vụ báo outcome về chính máy phát**, và rằng phương tiện là **nhãn trên nút**, không phải lớp phủ; **(b)** bổ sung vào `GR-003` C3 và `GR-032` §Điều kiện một vế về mặt hiển thị phía thí sinh — hai chỗ này hiện chỉ mô tả hệ quả **phía admin**; **(c)** bổ sung tập nhãn vào `PRD-REQ-072` cùng lúc với sửa đổi của `CONFLICT-002`. Ảnh hưởng: **FR-005a**, **FR-086**. *(`GR-003` C3 · `GR-032` §Điều kiện · `STATE-029`, `STATE-032` · `INV-006`, `INV-013` · `PRD-REQ-072`)*

- **`OQ-003` — ✅ ĐÃ PHÂN XỬ (§Clarifications 2026-08-03, lượt hai)**: **Máy thí sinh dưới banner tạm dừng.** Phán quyết của chủ dự án: banner **CHE TOÀN BỘ màn thí sinh** — không thấy bảng điểm, không thấy câu hỏi. Đây là **ngoại lệ tường minh và duy nhất** của `PRD-REQ-067`. Dữ liệu bên dưới **vẫn chảy** *(che ≠ đổi)*, nên tắt banner là thấy trạng thái **hiện tại** và **không** cần bước đồng bộ lại nào. Đã tích hợp vào **FR-029**, **FR-076a** *(mới)* và **AC-090a** *(mới)*.

  <details><summary>Nội dung câu hỏi trước khi phân xử, và một lỗi đã sửa</summary>

  `STATE-040` và §Invalid transitions nói banner *"chặn toàn bộ"* sự kiện của thí sinh; `INV-021` nói lớp phủ **không bao giờ đổi trạng thái bên dưới**; còn `PRD-REQ-067` đòi bảng điểm *"cập nhật theo thời gian thực"* **không kèm điều kiện**. Không nguồn nào nói banner có che nội dung hay chỉ chặn thao tác.

  **Lỗi đã sửa trong bản trước của spec này**: bản `AC-090a` đầu tiên dựng kịch bản trên *"một câu Tăng tốc đang đếm giờ, admin bật banner"* — **trạng thái bất khả thi**. `STATE-040` §Vào khai *"không có cửa sổ thời gian nào đang đếm"* là **điều kiện cứng**, §Invalid transitions chặn cú mở banner khi có đồng hồ chạy, và `QĐ-050` nói thẳng *"câu hỏi banner có đóng băng đồng hồ không **không có chủ ngữ**"*. Vế **đồng hồ** vì thế đã bị gỡ khỏi cả `FR-076a` lẫn `AC-090a`; chỉ còn vế **bảng điểm** là có thật.

  </details>

  **Cổng truy nguyên — ✅ ĐÃ THOẢ**: **`QĐ-115`** đã ghi vào `docs/decisions.md` *(ngay sau `QĐ-050`)*, và đã sửa theo: `STATE-040` §Mô tả, §Ra, §Nguồn cùng ghi chú *che ≠ đổi* · dòng `STATE-040` ở §Invalid transitions · `PRD-REQ-067` §Description và §Source *(mệnh đề ngoại lệ)*. Ảnh hưởng: **FR-029**, **FR-076a**. *(`QĐ-115` · `QĐ-050` · `STATE-040` · §Invalid transitions · `INV-021` · `PRD-REQ-067`, `PRD-REQ-050`)*

> **Cả sáu mục trên đã phân xử. KHÔNG còn `NEEDS CLARIFICATION` hay `CONFLICT` nào tồn đọng trong spec này.** Sáu chỗ trông như mâu thuẫn nhưng không phải — `PRD-REQ-069` vs `GR-021` về ba trạng thái, `GR-034` vs `PRD-REQ-071` về `Esc`, `GR-006` §mốc cắt vs `QĐ-110`, và ba ca biên của `GR-037` — đều được chính `docs/` giải thích tường minh là **không cùng đối tượng**, là **ngoại lệ có chủ ý**, hoặc là **độ trễ tài liệu đã có quyết định sau đè lên**; xem §Edge Cases → *Conflicting rule*.
...
|---|---|---|
| `GR-003` | C1, C2 *(click trong cửa sổ tạo tín hiệu)* · C3 *(tín hiệu trơ — **nay có nhãn báo về máy phát**, `OQ-002` đã phân xử)* · C5 *(đã chấm Sai ở câu này — nay là **mờ kèm nhãn**, `CONFLICT-002` đã phân xử)* · C6 *(ngoài cửa sổ ⇒ **ẩn**)* · §Biên | AC-001 · AC-005 · AC-096 · AC-006, AC-089 · AC-007 |
...
| `GR-009` | C6 *(chấm Sai ⇒ ghế bị loại, máy hết nút)* · **C6b** *(Huỷ kết quả ⇒ không bị loại, nút mở lại)* · C8 *(bấm No ⇒ không mất lượt, nút mở lại)* · C9, C10 *(không hiển thị)* · C14 *(vẫn trả lời hàng ngang khi đang chờ duyệt)* · §Điều kiện *(nút là chuông; **một PHÁN QUYẾT** cho cả vòng — `CONFLICT-001` đã phân xử)* | AC-009a · AC-009b · AC-009b · AC-011, AC-012 · AC-052 · AC-009, AC-009a |
...
| `GR-020` | §Điều kiện *(**nay là bản cuối** — `CONFLICT-003` đã phân xử)* · §Cấm *(không công bố khi cửa sổ cướp mở)* · C4 *(chuỗi ba bản ⇒ chấm trên **bản cuối**)* · §Thứ tự đánh giá *(không có đồng hồ trả lời riêng)* | AC-024a · AC-071 · AC-024a · AC-024a |
...
| `STATE-026`, `STATE-027`, `STATE-028` | ba cờ ghế sinh ra **khoá theo luật chơi**; `STATE-028` §Ra nay tách hai nhánh gỡ khoá theo loại nút | AC-055, AC-056 · AC-045 · AC-004, AC-010, AC-009b, AC-096 |
| `STATE-038` | dialog xác nhận phía thí sinh — ngoại lệ duy nhất | AC-040, AC-045, AC-048 |
| `STATE-040` | banner tạm dừng **che toàn bộ màn thí sinh** và chặn mọi sự kiện; dữ liệu bên dưới vẫn chảy *(`OQ-003` đã phân xử — `QĐ-115`)*; banner và đồng hồ **loại trừ lẫn nhau** *(`QĐ-050`)* | AC-090, AC-090a |
...
| `EVENT-040` §Ngoại lệ | **mục này bị GỠ** — người cướp quyền nay theo quy tắc bản cuối như mọi vai *(`CONFLICT-003` đã phân xử)* | AC-024a |
