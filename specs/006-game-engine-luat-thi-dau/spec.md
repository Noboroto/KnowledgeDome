...
- Q: Kênh **gõ** khoá nút chấm tới mốc nào — *"hết giờ"* như bản trước viết, hay muộn hơn? → A: **Tới khi cửa sổ giữ bản tới muộn đã đóng**, tức `hạn chót + padding` (`QĐ-109`). Nút gửi phía thí sinh vẫn tắt **đúng tại `hạn chót`** — cửa sổ này **không** cấp thêm giây thao tác nào cho thí sinh. Đã tích hợp vào **FR-010**, **FR-070a** *(mới)*, **AC-016**, **AC-098**, **AC-099**, **AC-099a** *(mới)*.
- Q: Cửa sổ giữ bản tới muộn có biên trên ở mọi vòng không? → A: **Không.** Vòng chấm theo **lô** *(Tăng tốc, câu hàng ngang VCNV)* và các vòng gõ khác có biên trên `hạn chót + padding`; **Khởi động không có biên trên** và cũng **không chờ** — nút chấm mở ngay tại `hạn chót`. Lý do bất đối xứng: Khởi động chấm **từng người**, không có bảng điểm chung nào để một bản tới muộn phá (`QĐ-109`).
- Q: Vòng Khởi động còn **mốc cắt** *"công bố đáp án"* không? → A: **Không — đã bỏ** (`QĐ-110`). `hạn chót` của câu là hạn nhận bài **duy nhất**; bản được ghi nhận là bản hợp lệ **cuối cùng trước `hạn chót`**. `EVENT-016` không còn là một mốc riêng mà gộp vào cú bấm **mở đáp án** chung (`QĐ-048`). Đã tích hợp vào **FR-070**, **AC-096** → **AC-099**.
- Q: Ai sở hữu đặc tả của hai quy tắc trên? → A: **EPIC-006 sở hữu quy tắc engine** — nút gửi tắt lúc nào, server giữ hay từ chối một bản, biên trên, và việc bỏ mốc cắt. **EPIC-007 chỉ sở hữu mốc mở nút chấm** trên màn điều khiển. `specs/007` đã bỏ hai FR tương ứng và trỏ sang spec này.
...
- Q (giải `OQ-001`): Câu **ô trung tâm** VCNV có thời lượng suy nghĩ bao nhiêu và lấy từ đâu? → A: **Dùng CÙNG một giá trị với câu hàng ngang** — không phải metadata riêng từng câu, và người soạn đề **không** khai thời lượng riêng cho câu ô trung tâm. **Nhưng con số `15` giây KHÔNG cố định**: nó là **giá trị cấu hình luật cấp vòng VCNV**, preset `O26_DEFAULT@1` đặt mặc định `15`. **SỬA 2026-07-31 (`QĐ-106`)**: vòng có **HAI** knob độc lập chứ không phải một — *(a)* thời gian suy nghĩ mỗi câu, dùng chung cho hàng ngang và ô trung tâm; *(b)* cửa sổ giải Chướng ngại vật sau gợi ý cuối. Nguồn nói hai con số 15 giây ở **hai mệnh đề khác nhau** cho **hai đại lượng khác nhau** *(luật gốc §VCNV: "thời gian suy nghĩ cho mỗi từ hàng ngang là 15 giây" và "các thí sinh sẽ có 15 giây suy nghĩ để đưa ra Chướng ngại vật")* — chúng chỉ trùng giá trị, nên gộp vào một knob là suy diễn vượt nguồn. Điều này khớp `CLAUDE.md` §Quy ước khác *("Mọi timer/điểm là RuleConfig — KHÔNG hard-code luật")*; ngoại lệ *cố định, không cấu hình* vẫn **chỉ** là 3 câu × 15 giây của Câu hỏi phụ (`GR-023` §Biên) và hệ số Ngôi sao hy vọng (`GR-021` §Biên).
...
- Thang điểm Tăng tốc và thứ tự lượt Về đích cho **số ghế TRÊN 4**: nguồn chỉ viết cho đúng 4 người (`GR-013` C5, `GR-014` §Biên, `GR-016` §Biên, `NON-GOAL-012`). Ngoài phạm vi v1 — xem §Open Questions. *(Dưới 4 ghế thì **thuộc** v1: chạy bằng **ghế bỏ thi** — FR-120, `PRD-REQ-114`.)*
...
- **FR-055a**: Khi admin **kích hoạt tay** một tín hiệu kế tiếp theo FR-034a, mốc **câu khép** MUST lùi tới sau khi **người được kích hoạt đã được chấm**, hoặc cửa sổ vừa cấp lại cho người đó đã đóng mà admin đã chốt câu. Trong khoảng từ cú bấm *Huỷ kết quả* tới mốc mới đó, server MUST NOT đẩy đáp án tới thí sinh, khán giả hay lớp phủ — cùng lập luận với `FR-056`: công bố sớm sẽ xoá cơ hội của người được kích hoạt. *(US-007, US-005 · **PRD-REQ-113**, PRD-REQ-088, PRD-REQ-045 · GR-037 C8b, C6, C8, GR-032 §Kích hoạt tay · AC-044c)*
...
- **FR-070**: Ở **mode nhập liệu**, hệ thống MUST ghi nhận **bản hợp lệ cuối cùng trước `hạn chót`**; `hạn chót` MUST là hạn nhận bài **duy nhất** của vòng Khởi động và hệ thống MUST NOT có một **mốc cắt** riêng nào *(`QĐ-110`)*; nút gửi MUST NOT khoá sau khi gửi. Bản **rỗng sau khi cắt khoảng trắng** MUST bị bỏ qua và MUST NOT ghi đè bản hợp lệ đã có. Mọi bản gửi MUST được trim hai đầu. Lịch sử các bản đã gửi MUST NOT bị xoá — chỉ *bản được ghi nhận* thay đổi. *(US-008 · PRD-REQ-036, PRD-REQ-065 *(giao diện thuộc EPIC-008)* · GR-006 C1 → C7 · AC-096 → AC-100)*
- **FR-070a**: Nút gửi trên máy thí sinh MUST tắt **đúng tại `hạn chót`** ở **mọi** vòng gõ; hệ thống MUST NOT cấp thêm giây thao tác nào cho thí sinh, đúng `GR-033` §Điều kiện *(mốc tuyệt đối, không cửa sổ ân hạn)*. Ở các vòng gõ **trừ Khởi động**, server MUST **giữ** một bản tới trong khoảng **`(hạn chót, hạn chót + padding]`** — bản phát ra kịp mà tới muộn do mạng — và MUST đánh dấu nó **quá hạn**, để nó còn đi qua **phán quyết Đúng/Sai** của admin; từ mốc `hạn chót + padding` trở đi server MUST từ chối, và bản ấy MUST NOT còn là đối tượng chấm được. Biên MUST là **biên đóng**: bản tới đúng mốc `hạn chót + padding` vẫn được giữ. `padding` MUST là **RuleConfig cấu hình được**, mặc định **5 giây**, MUST NOT hard-code. Cửa sổ này MUST NOT được đặc tả hay hiện thực như một **ân hạn cho thí sinh**: bản trong cửa sổ **vẫn là bản quá hạn** *(§Clarifications 2026-08-03)*. *(US-001, US-008, US-010 · PRD-REQ-036, PRD-REQ-042 · GR-033 §Điều kiện, GR-035 C6, GR-006 C4, C5, GR-015 C5, C6, INV-005, INV-009, `QĐ-109` · AC-099a)*
- **FR-070b**: Ở **Khởi động**, hệ thống MUST NOT áp biên trên nào: một bản tới server sau `hạn chót` MUST **luôn** được giữ và đánh dấu quá hạn, **bất kể muộn bao nhiêu**, và MUST còn chấm được — `GR-006` C4 và C5 vì thế MUST giữ nguyên hiệu lực. Một bản tới **sau** khi admin đã chấm MUST vào lịch sử nhưng MUST NOT lật được phán quyết đã chốt; sửa sai MUST đi qua điều chỉnh điểm thủ công *(§Clarifications 2026-08-03)*. *(US-008 · PRD-REQ-036 · GR-006 C4, C5, §Đồng thời, GR-035 C6, INV-006, INV-009, `QĐ-109` · AC-099a)*
...
- **FR-077**: Nút *"Mở chướng ngại vật"* MUST được xếp là **chuông**: MUST chỉ nhận click chuột, MUST tự khoá khi bấm, MUST NOT có dialog phía thí sinh. Hạn mức của một ghế trong một lần chạy vòng MUST đếm theo **phán quyết**, không theo cú bấm: mỗi ghế MUST đi qua tối đa **một** phán quyết **Đúng** hoặc **Sai** cho cả vòng; một tín hiệu bị admin **từ chối** hoặc bị chấm **Huỷ kết quả** MUST NOT tiêu hạn mức đó, và ghế MUST phát lại được trong cùng vòng. Hàng đợi cho tín hiệu này MUST **chặn**. *(US-009, US-005 · PRD-REQ-045, PRD-REQ-064 *(giao diện thuộc EPIC-008)* · GR-009 §Điều kiện, C6, C6b, C8, C9, C10, GR-032 §Kích hoạt tay, GR-034 · QĐ-061 · `specs/008` §Clarifications 2026-08-03 `XCONF-2` · AC-118, AC-118a)*
...
- **FR-083**: Gợi ý cuối ở ô trung tâm MUST kích hoạt khi **cả 4 hàng ngang đã được HỎI** — điều kiện MUST đọc theo *hàng ngang đã hỏi*, MUST NOT đọc theo *miếng ghép đã mở*, nên giai đoạn này MUST luôn tới được. Câu ô trung tâm MUST cho **+10** và mở ô khi Đúng, và MUST NOT mở ô khi Sai; băng điểm MUST là **20** trong cả hai trường hợp, không phụ thuộc kết quả câu ô trung tâm. Thời gian suy nghĩ câu ô trung tâm MUST dùng giá trị **(a)** của FR-083a *(chung với câu hàng ngang)*, còn cửa sổ giải Chướng ngại vật sau gợi ý cuối MUST dùng giá trị **(b)** — một knob **riêng**, mặc định cũng **15 giây** — và cho **+20** khi đúng. *(US-009 · PRD §9.2, EPIC-006 §11 · GR-011 C1 → C4, C6 · AC-128 → AC-131)*
- **FR-083a**: Vòng Vượt chướng ngại vật MUST có **đúng hai** giá trị cấu hình thời gian, cả hai ở **cấp vòng** và đổi **độc lập** với nhau: **(a)** *thời gian suy nghĩ mỗi câu* — dùng chung cho **câu hàng ngang** và **câu ô trung tâm**; **(b)** *cửa sổ giải Chướng ngại vật sau gợi ý cuối*. Preset `O26_DEFAULT@1` MUST đặt **15 giây** cho cả hai; đổi (a) MUST NOT làm đổi (b) và ngược lại. Câu VCNV MUST NOT khai thời lượng riêng — người soạn đề không nhập, và nếu dữ liệu có mang thì engine MUST bỏ qua. Hệ thống MUST NOT hard-code con số 15 ở đường xử lý nào. *(US-009 · **`QĐ-106`**, PRD-REQ-008 §Note ngoại lệ, PRD §9.2 `STATE-004`, EPIC-006 §11, GOAL-002 · GR-008 §Điều kiện, GR-011 §Điều kiện · luật gốc §VCNV *(hai mệnh đề 15 giây riêng biệt)* · `CLAUDE.md` §Quy ước khác · AC-131a)*
- **FR-120**: Khi một trận bắt đầu với **dưới 4 ghế**, engine MUST áp **nguyên luật 4 ghế** và MUST coi các ghế thiếu người là **ghế bỏ thi** — vô hiệu hoá từ trước mốc đóng băng, điểm **luôn 0**, không phát được tín hiệu nào. Ghế bỏ thi MUST vào bảng điểm và bảng xếp hạng và MUST được xếp hạng **bình thường như một ghế 0đ**; phép tìm nhóm hoà cần phân định MUST **chỉ xét ghế hoạt động**. Ghế bỏ thi MUST vẫn được cấp lượt ở **Khởi động lượt riêng** và **Về đích**, và MUST chỉ đi tiếp bằng thao tác **bỏ qua của admin**; bỏ qua **trước** mốc hiển thị MUST trả câu về kho theo FR-041. Mọi phép kiểm kho đề MUST giữ mẫu số là **số ghế đã gán**. Engine MUST NOT có đường xử lý nào cho **trên 4 ghế** — hàng rào nằm ở cú bắt đầu trận, thuộc `specs/005` FR-035. *(US-009, US-011, US-012 · **PRD-REQ-114** · GR-036 C5b, GR-020 C6, GR-022, GR-016, GR-017 §Biên · `QĐ-105` · AC-209)*
...
- **FR-097**: Cửa sổ **cướp quyền** MUST là **5 giây**, hàng đợi MUST **không chặn**, và server MUST phân xử ngay theo timestamp. Người cướp MUST được tính theo **bản CUỐI CÙNG**, y như mọi vai khác — theo **nguyên tắc hai trục** *(giành lượt lấy người **đầu tiên**; đáp án lấy bản **cuối cùng**)*; vòng này MUST NOT có `hạn chót` riêng cho người cướp, và mốc đóng MUST là **cú bấm chấm của admin** theo `GR-006` §Đồng thời. Cướp **đúng** ⇒ **transfer**: người thi chính **−giá trị câu**, người cướp **+giá trị câu**. Cướp **sai** ⇒ người cướp **−½ giá trị câu**, và người thi chính MUST NOT được hoàn lại. Người thi chính MUST NOT cướp câu của chính mình. Bấm ngoài cửa sổ MUST NOT tạo tín hiệu nào. *(US-011 · PRD §9.2, EPIC-006 §11, PRD-REQ-045 · GR-020 C1 → C4, §Biên · AC-167 → AC-170)*
...
- **FR-121**: Mỗi lần một câu được **hiển thị** cho thí sinh, nhật ký sự kiện của trận MUST ghi lại **phiên bản nào** của câu đã dùng *(tham chiếu tới bản do kho đề lưu — `PRD-REQ-115`, `specs/002` FR-004b)*, đủ để dựng lại và in lại trận đúng nội dung đã lên sóng kể cả khi câu bị sửa về sau. Việc sửa câu ở kho đề MUST NOT đổi nội dung mà một trận đã ghi. *(US-006 · **PRD-REQ-115**, PRD-REQ-047 · GR-031, INV-001, INV-022 · `QĐ-107` · AC-052)*
...
- **Contest builder**: nút *"Áp dụng luật 2026"*, cấu hình từng vòng, chọn mode trả lời cấp contest, gán ghế và vị trí, chọn danh sách câu **ban đầu**, khoá cứng `rowCount = 4` và `tieBreakPositions = [1]` ở đường ghi cấu hình — thuộc EPIC-004. **Hàng rào số ghế > 4 tại cú bấm bắt đầu trận** và việc **đặt ghế bỏ thi** cho trận dưới 4 ghế thuộc EPIC-005 (`specs/005` FR-035); spec này chỉ đặc tả **hệ quả phía engine** ở FR-120.
- **Nhập/xuất gói contest** (EPIC-003) · **sau trận: biên bản, thống kê** (EPIC-010) · **nhật ký thao tác chung và hạn lưu trữ** (EPIC-011).
- **Luật cho số ghế TRÊN 4** (`NON-GOAL-012`), **thi đội** (`NON-GOAL-013`), **playlist tuỳ ý** (`NON-GOAL-011`), **mọi đường máy tự chấm** (`NON-GOAL-001`) — không thuộc v1 dưới bất kỳ epic nào. Số ghế **dưới** 4 thì thuộc v1 qua FR-120.
...
- **OQ-005 — ✅ ĐÃ THU HẸP 2026-07-31 (`QĐ-105`)**: ca **dưới 4 ghế** nay **thuộc v1** — trận chạy bằng **ghế bỏ thi** theo `PRD-REQ-114` và **FR-120**; ca *"số ghế hoạt động tụt dưới 4 giữa trận"* đi cùng đường. Phần còn treo chỉ là **luật cho số ghế TRÊN 4**, và nó bị chặn cứng ở cú bắt đầu trận (`specs/005` FR-035) nên **không chặn v1**. Nội dung gốc:  **Luật cho số ghế ≠ 4**. Thang điểm Tăng tốc (`GR-013` C5, `GR-014` §Biên), thứ tự lượt Về đích (`GR-016` §Biên), và mệnh đề *"3 thí sinh còn lại"* của cướp quyền (`GR-020` C6) đều chỉ được nguồn viết cho **đúng 4 người**. v1 chỉ hỗ trợ **lưu trữ và giao diện** cho 1-12, không có đường xử lý luật (`NON-GOAL-012`, `QĐ-007`), nên câu hỏi này **không chặn v1**. Riêng ca **số ghế hoạt động tụt dưới 4 giữa trận** đã có phân xử một phần — *admin quyết, hệ thống chỉ cảnh báo* (`GR-020` C6) — nhưng chưa nêu hệ thống cảnh báo **cái gì** và ở mốc nào. *(GR-013 C5, GR-014, GR-016, GR-020 C6 · NON-GOAL-012)*

- **`XCONF-1` — ✅ ĐÃ PHÂN XỬ 2026-08-03**: **Người cướp quyền Về đích tính bản đầu hay bản cuối.** Spec này ban đầu chép theo `GR-020` §Điều kiện *("chỉ tính BẢN ĐẦU TIÊN")*, trong khi `docs/PRD.md` `PRD-REQ-065` phát biểu **không điều kiện** rằng hệ thống luôn ghi nhận **bản cuối** và **không** khai `GR-020` trong *Related game rules*. Mâu thuẫn được phát hiện trong đợt audit chéo 2026-08-03 và chủ dự án phân xử: **bản CUỐI**, theo **nguyên tắc hai trục** — *giành lượt lấy người **đầu tiên**; đáp án lấy bản **cuối cùng***. `GR-020` lấy quy tắc của trục giành quyền áp nhầm sang trục nội dung; đối chiếu toàn bộ 37 rule thì nó là **ngoại lệ duy nhất**. Đã sửa **FR-097** và **AC-170**. ⚠️ `docs/` cần sửa: `GR-020` §Điều kiện và C4 · `GR-018` §Điều kiện · **xoá** `EVENT-040` §Ngoại lệ · một `QĐ` mới khai nguyên tắc hai trục. Xem `specs/008` §Clarifications 2026-08-03. *(`PRD-REQ-065` · `GR-020` §Điều kiện, C4 · `GR-018` §Điều kiện · `GR-006` §Đồng thời · `EVENT-040` §Ngoại lệ · `QĐ-029`, `QĐ-058`)*

- **`XCONF-2` — ✅ ĐÃ PHÂN XỬ 2026-08-03**: **Hạn mức tín hiệu *"Mở chướng ngại vật"* của một ghế trong một vòng.** Spec này ban đầu khai *"mỗi ghế phát tối đa **một** tín hiệu cho cả vòng"* theo `GR-009` §Điều kiện, trong khi `PRD-REQ-064` khai khoá chuông *"gắn với **một câu** — sang câu mới thì mở lại"*. Chủ dự án phân xử: hạn mức đếm theo **PHÁN QUYẾT**, không theo cú bấm — tối đa **một** phán quyết Đúng/Sai cho cả vòng; **Huỷ kết quả** hoặc **bấm No** **không** tiêu hạn mức và ghế **bấm lại được**. Hai nhánh Đúng/Sai tự cưỡng chế *"một lần đoán, sai thì loại"* của luật gốc qua `GR-009` C9, C10 mà **không cần** một khoá riêng. Đã sửa **FR-077** và **AC-118**, thêm **AC-118a**. ⚠️ `docs/` cần sửa: `GR-009` §Điều kiện, C6b, C8 · `PRD-REQ-064` · `STATE-028` §Ra · một `QĐ` mới. Xem `specs/008` §Clarifications 2026-08-03. *(`PRD-REQ-064` · `GR-009` §Điều kiện, C6b, C8 · `GR-032` §Kích hoạt tay · `STATE-028` · `QĐ-023`, `QĐ-061`)*

- **`XCONF-3` — ✅ KHÔNG PHẢI MÂU THUẪN, chỉ cần sửa câu chữ**: `GR-032` §Kích hoạt tay nói chuỗi kích hoạt ở VCNV *"dừng khi hàng đợi tín hiệu Chướng ngại vật của vòng **cạn**"*. Sau `XCONF-2`, ghế bấm lại được nên hàng đợi **nạp lại được** và điều kiện dừng đó không còn chứng minh được. Nhưng `PRD-REQ-113` **đã** khai hạn mức là *"một lần cho mỗi phán quyết Huỷ kết quả ở VCNV và **không có trần tổng**"* — nguồn vốn đã chấp nhận chuỗi không có trần, và mỗi vòng lặp đòi **một cú bấm *Huỷ kết quả* của admin** nên nó bị chặn bởi ý chí của người, đúng `QĐ-001`. ⚠️ `docs/` cần sửa **câu chữ** của `GR-032` §Kích hoạt tay cho khỏi đọc thành một bảo đảm. **Không** phải một quyết định mới. *(`PRD-REQ-113` · `GR-032` §Kích hoạt tay · `QĐ-001`, `QĐ-104`)*

**Không có `CONFLICT` nào KHÁC.** Bốn chỗ trông như mâu thuẫn — *"ghi nhận bản đầu"* vs *"ghi nhận bản cuối"*, fallback của `GR-017` vs lệnh cấm của `GR-007`, *"tối đa 1 lượt chọn"* vs lượt quay lại vị trí 1, và `stripDiacritics` vs mệnh đề chính tả của luật gốc — đều đã được chính `docs/game-rules.md` giải thích tường minh là **không cùng đối tượng** hoặc **ngoại lệ có chủ ý**; xem §Edge Cases → *Conflicting rule*.
...
| US-005 | PRD-REQ-044, PRD-REQ-045, PRD-REQ-046, **PRD-REQ-113** | GR-032 *(gồm §Kích hoạt tay)*, GR-007 C2/C9, GR-009 C8, GR-003 C3, GR-012 §Đồng thời, INV-006, INV-007, INV-008, INV-013 | FR-030 → FR-038 **+ FR-034a → FR-034e** | AC-041 → AC-051 **+ AC-044a → AC-044g** |
| US-006 | PRD-REQ-047, PRD-REQ-048, PRD-REQ-092, PRD-REQ-093, **PRD-REQ-115** | GR-031, GR-005, GR-012 C4, GR-023 §Nguồn đề, INV-011, INV-012, INV-014, INV-022 | FR-039 → FR-050 **+ FR-121** | AC-052 → AC-063 |
| US-007 | PRD-REQ-049, PRD-REQ-088, PRD-REQ-089, **PRD-REQ-113** | GR-037 *(gồm C8b)*, GR-020 §Cấm, GR-008, GR-012, GR-019 §Bảo mật, INV-017 | FR-051 → FR-061 **+ FR-055a, FR-057a** | AC-064 → AC-076 **+ AC-071a** |
...
| US-009 | PRD §8.2, PRD §9.2 (`STATE-004`), EPIC-006 §11 · PRD-REQ-045, PRD-REQ-046, PRD-REQ-058, PRD-REQ-069, PRD-REQ-092, **PRD-REQ-113**, **PRD-REQ-114** | GR-007, GR-008, GR-009, GR-010, GR-011, GR-012, GR-036 C5b, INV-019 | FR-071 → FR-086 **+ FR-083a, FR-120** | AC-102 → AC-139 **+ AC-131a, AC-209** |
...
**Then** câu **nói** chấm được ngay và máy thí sinh không có nút gửi; câu **gõ** có nút chấm **khoá** tới mốc `hạn chót + padding` trong khi nút gửi của thí sinh sống tới `hạn chót` rồi tắt; câu **thực hành** có nút chấm **sống suốt** vì không có ô nhập nào để chờ.
...
**When** ghế A gửi `"Hà Nội"`, rồi sửa thành `"Huế"`, rồi gửi lại `"Huế"` y hệt — tất cả trước `hạn chót`,
...
**Given** ghế A có một bản hợp lệ trước `hạn chót`, và một bản nữa tới server **sau** `hạn chót` — Khởi động không có biên trên nên bản đó luôn được giữ,
...
**Given** ghế A **chỉ** có một bản, và bản đó tới server **sau** `hạn chót`,
**When** admin mở màn chấm,
**Then** bản được giữ và tô **đỏ**, và admin có **ba** lựa chọn Đúng / Sai / **Huỷ kết quả**.

**AC-099a** — *US*: US-001, US-008, US-010 · *FR*: FR-070a, FR-070b, FR-010 · *GR*: `GR-033` §Điều kiện, `GR-035` C6, `GR-006` C4, C5 · `INV-005`, `INV-006`
**Given** một câu Tăng tốc có `hạn chót` ở `13,000` giây và `padding` cấu hình ở **5 giây**; song song một câu Khởi động mode nhập liệu cũng có `hạn chót` ở `13,000` giây,
**When** ở câu Tăng tốc: ghế B có bản tới server lúc `15,000` giây, ghế D có bản tới lúc `18,000` giây, ghế E có bản tới lúc `18,001` giây, và ghế C thử bấm gửi lúc `13,500` giây; còn ở câu Khởi động: ghế F có bản tới lúc `25,000` giây,
**Then** nút gửi của mọi ghế đã tắt tại `13,000` giây nên ghế C **không phát được tín hiệu nào**; bản của ghế B và ghế D được **giữ và tô đỏ** — bản ghế D tới **đúng** mốc `hạn chót + padding` vẫn hợp lệ vì biên là biên **đóng**; bản của ghế E **bị từ chối** và MUST NOT là đối tượng chấm được; bản của ghế F ở câu **Khởi động** **vẫn được giữ và tô đỏ** dù muộn `12` giây, vì vòng đó không có biên trên; và **không** bản nào trong số được giữ trở thành **hợp lệ** — tất cả đều chờ phán quyết Đúng/Sai của admin.
...
**Then** ghế đó nhận lần lượt **+60 · +50 · +40 · +30 · +20** và vòng kết thúc; nút này chỉ nhận **click chuột** *(không phím tắt nào)* và tự khoá khi bấm.

**AC-118a** — *US*: US-009 · *FR*: FR-077 · *GR*: GR-009 C6, C6b, C8, C9, GR-032 §Kích hoạt tay · INV-019
**Given** một vòng VCNV, ghế B đã bấm *"Mở chướng ngại vật"*, admin đã bấm **Yes** cho tín hiệu đó,
**When** admin lần lượt thử ba phán quyết trên ba lần chạy độc lập — **Sai**, **Huỷ kết quả**, và **bấm No** *(chưa chấm)*,
**Then** với **Sai**: ghế B mang cờ *bị loại*, hạn mức đã tiêu, B **không** bấm lại được — nhưng điểm hàng ngang B đã kiếm **giữ nguyên**; với **Huỷ kết quả**: B **không** bị loại, **không ai** được điểm, hạn mức **chưa tiêu**, B **bấm lại được trong cùng vòng**, và tín hiệu cũ vẫn chuyển sang *đã xử lý* rồi ở lại lịch sử; với **bấm No**: y như Huỷ kết quả và **0 tác dụng phụ** phát sinh. Ở cả ba ca, băng điểm của lần bấm sau MUST chốt theo **số hàng ngang đã hỏi tại mốc admin xác nhận lần đó**, nên nó có thể **thấp hơn** lần trước.
...
**When** admin chấm B,
**Then** phán quyết áp trên **bản CUỐI CÙNG** `"Berlin"` — **y như** người thi chính, theo nguyên tắc hai trục; nút gửi của B **không khoá** sau lần gửi đầu và chỉ khoá tại **cú bấm chấm của admin** *(`specs/008` §Clarifications 2026-08-03 `XCONF-1`)*.
...
**AC-209** — *US*: US-009, US-011, US-012 · *FR*: FR-120 · *GR*: GR-036 C5b, GR-022, GR-016 · *PRD*: `PRD-REQ-114`
**Given** một trận bắt đầu với **2 thí sinh thật** — C và D — nên hai ghế còn lại ở trạng thái **bỏ thi** *(0đ, vô hiệu hoá từ đầu)*,
**When** trận chạy trọn bốn vòng rồi admin bấm Chốt trận,
**Then** engine áp **nguyên luật 4 ghế** ở mọi vòng · ở **Tăng tốc**, thang `40/30/20/10` tính **chỉ trên tập người được chấm Đúng** nên hai ghế bỏ thi **không giữ chỗ** trong thang · ở **VCNV**, lượt chọn hàng ngang quay lại vị trí số 1 nên **cả 4 hàng ngang vẫn được hỏi hết** · ở **Khởi động lượt riêng** và **Về đích**, lượt của hai ghế bỏ thi **vẫn được cấp** và admin **bỏ qua bằng tay**; bỏ qua trước mốc hiển thị thì câu **trả lại kho** · pre-flight Về đích vẫn đòi `3 × 4` câu mỗi mức · bảng xếp hạng cuối ghi **đủ bốn ghế** với hai ghế bỏ thi 0đ · phép tìm nhóm hoà **chỉ xét C và D**.
