...
- **Thời lượng suy nghĩ của câu ô trung tâm VCNV**: dùng giá trị cấu hình **(a)** *thời gian suy nghĩ mỗi câu* của vòng VCNV — **chung** với câu hàng ngang, preset `O26_DEFAULT@1` đặt **15 giây**. Cửa sổ giải Chướng ngại vật sau gợi ý cuối là knob **(b)** riêng, đổi độc lập, preset cũng **15 giây**. Người soạn đề **không** khai thời lượng riêng cho câu ô trung tâm; dữ liệu có mang thì engine bỏ qua (FR-083a).
- **Mốc câu khép khi người cướp quyền Về đích bị chấm *Huỷ kết quả***: câu **đã khép** — không ai được trả lời nữa, cửa sổ cướp không mở lại — nhưng server **MUST NOT tự đẩy** đáp án tới thí sinh, khán giả hay lớp phủ, kể cả khi cờ *hiện đáp án sau khi chấm* đang bật. Quy tắc phát biểu theo **loại phán quyết**, không theo vai bị chấm; đáp án chỉ hiện khi admin **mở tay** (FR-057a).
- **Nguồn đề của Câu hỏi phụ trong một trận `practice` chạy trong contest thật**: áp **hai bước cố định** — **(1)** dựng kho khả dụng bằng phép đảo, **chỉ** những câu đã lộ ở các trận thật trước đó; **(2)** trên kho đó mới áp thứ tự ưu tiên ba kho *(Về đích → Khởi động → VCNV)*, loại kho Tăng tốc, loại câu thực hành. Phép kiểm *"đủ 3 câu khả dụng"* đếm trên **tập đã lộ**, và hệ thống **MUST NOT** bù bằng câu chưa lộ (FR-105a).
...
- **FR-057a**: Quy tắc *"**Huỷ kết quả** không tự công bố"* MUST phát biểu theo **loại phán quyết**, MUST NOT theo vai bị chấm hay vị trí trong chuỗi câu. Ở **Về đích**, khi phán quyết **khép câu** là *Huỷ kết quả* cho **người cướp quyền**, server MUST NOT tự đẩy đáp án tới thí sinh, khán giả hay lớp phủ — dù câu đã khép theo `GR-037` C6 và cờ *hiện đáp án sau khi chấm* đang bật. Trong ca này `GR-037` **C8 thắng C6**, và đáp án MUST chỉ hiện qua thao tác **mở tay của admin**. *(US-007, US-011 · PRD-REQ-088 · GR-037 C6, C8, GR-020 · AC-071a)*
...
- **Câu khép**: Mốc mà không còn ai được trả lời câu đó nữa. Khởi động lượt riêng, câu hàng ngang và câu ô trung tâm VCNV, Tăng tốc: khép tại **cú bấm chấm**. Khởi động lượt chung: khép tại cú bấm chấm **hoặc** khi hết cửa sổ chuông 3 giây mà không ai bấm. Về đích: khép khi cửa sổ cướp quyền đã đóng **và** người cướp đã được chấm, hoặc hết 5 giây không ai bấm, hoặc người thi chính được chấm **Đúng**. Khi admin **kích hoạt tay** một tín hiệu sau phán quyết *Huỷ kết quả*, mốc khép **lùi** tới sau khi người được kích hoạt đã được chấm. *(TERM-057)*
...
> Hai mục dưới đây là **missing source behavior nằm ngoài phạm vi v1**, ghi lại để phiên bản sau không lấp bằng suy diễn. Không mục nào chặn v1.

- **OQ-004 — ngoài phạm vi v1, đã có mặc định**: **Băng điểm Chướng ngại vật cho `rowCount` 5-8**. Băng là **mảng cấu hình bắt buộc** dài bằng `rowCount` (`GR-009` C11); mặc định của preset khi mở khoá là `băng(k) = max(30, 70 − 10k)`, tái tạo đúng 60·50·40·30 ở `rowCount = 4` và giữ **20 là sàn** sau gợi ý cuối (`QĐ-158`). v1 khoá cứng `rowCount = 4` (`PRD-REQ-025`) nên mục này **không chặn v1**.

- **OQ-005 — missing source behavior, ngoài phạm vi v1**: **Luật cho số ghế TRÊN 4**. Thang điểm Tăng tốc (`GR-013` §Biên), phép tìm nhóm hoà (`GR-014` §Biên), thứ tự lượt Về đích (`GR-016` §Biên) và mệnh đề *"3 thí sinh còn lại"* của cướp quyền (`GR-020`) đều chỉ được nguồn viết cho **đúng 4 người**. v1 chặn cứng số ghế **trên** 4 tại cú bấm bắt đầu trận (`specs/005` FR-035), nên câu hỏi này **không chặn v1**. Số ghế **dưới** 4 **thuộc v1** — trận chạy bằng **ghế bỏ thi** theo FR-120. *(`GR-013`, `GR-014`, `GR-016`, `GR-020` · NON-GOAL-012)*
...
| `GR-010` | C1 · C2 · C3 · C4 · C5 | AC-125 · AC-103 · AC-127 · AC-127 · AC-135 |
...
| `GR-013` | C1 · C2 · C3 · C4 · C6 · C7 | AC-140 · AC-142 · AC-141 · AC-141 · AC-145 · AC-145 |
...
| `GR-020` | C1 · C2 · C3 · C4 · C5 · C6 · C6b | AC-167 · AC-168 · AC-169 · AC-170 · AC-176 · AC-176 · *(số ghế trên 4 — chặn cứng ở cú bấm bắt đầu trận, `specs/005` FR-035)* |
...
**AC-052a** — *US*: US-006 · *FR*: FR-121 · *GR*: GR-031, INV-001, INV-022
**Given** một câu đã được **hiển thị** cho thí sinh trong một trận đang chạy, và sau trận đó người soạn đề sửa nội dung câu ở kho đề rồi lưu thành một phiên bản mới,
**When** ban tổ chức in lại biên bản của trận đã chạy và dựng lại trận đó từ nhật ký sự kiện,
**Then** nhật ký sự kiện của trận mang **tham chiếu tới đúng phiên bản câu đã lên sóng**, nên biên bản và bản dựng lại hiện **nội dung cũ** chứ không phải nội dung đã sửa · việc sửa câu ở kho đề **không** đổi một dòng nào trong nhật ký trận · **0** sự kiện nào được sinh thêm bởi thao tác sửa đó · điểm của mọi ghế **không đổi**.
...
**Given** một contest vừa bấm *"Áp dụng luật 2026"* nên **cả hai** giá trị cấu hình thời gian của vòng VCNV đang là **15 giây** — **(a)** *thời gian suy nghĩ mỗi câu* và **(b)** *cửa sổ giải Chướng ngại vật sau gợi ý cuối*; sau đó admin đổi **chỉ giá trị (a)** thành **20 giây** rồi bắt đầu một trận mới,
**When** trận chạy trọn vòng VCNV — một câu hàng ngang, câu ô trung tâm, và cửa sổ giải Chướng ngại vật sau gợi ý cuối,
**Then** câu hàng ngang **và** câu ô trung tâm đều dài **20 giây**; cửa sổ giải Chướng ngại vật **vẫn 15 giây** vì **(b)** là knob độc lập và không bị **(a)** kéo theo; người soạn đề không phải khai thời lượng riêng cho câu ô trung tâm ở bất kỳ đâu trong kho đề; và không màn hình hay đường xử lý nào hiện con số 15 như một hằng số cố định.
...
**When** admin chấm B,
**Then** phán quyết áp trên **bản CUỐI CÙNG** `"Berlin"` — **y như** người thi chính, theo nguyên tắc hai trục; nút gửi của B **không khoá** sau lần gửi đầu và chỉ khoá tại **cú bấm chấm của admin** *(`specs/008`)*.
