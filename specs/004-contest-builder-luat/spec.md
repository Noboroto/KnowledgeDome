...
**Lưu ý về đầu vào**: lệnh gọi yêu cầu đọc thêm `docs/rule-traceability.md` và `docs/reviews/prd-review.md` — hai đường dẫn này **không tồn tại** trong repo hiện tại (tương ứng gần nhất là `docs/traceability.md` và `docs/reviews/game-rules-review.md`, đã dùng thay). Đây không phải `CONFLICT` về nội dung nghiệp vụ, chỉ là chênh đường dẫn đầu vào, nên không ghi vào §Open Questions.
...
- Q: Khi contest đã có trận `FINISHED` hoặc trận hiện tại đang ở `LOBBY`, admin còn sửa được RuleConfig cấp contest và danh sách câu đã gán hay không? → A: Sửa được, trừ lúc một vòng đang chạy — khớp quy tắc đã có ở `GR-031` C8 cho danh sách câu, mở rộng cùng nguyên tắc sang RuleConfig, cùng khuôn với `QĐ-075` (mode sửa được kể cả sau trận `FINISHED`, không đụng trận đã chạy).
- Q: Khi admin gán một vị trí đã có ghế khác đang giữ, hệ thống phản hồi ra sao? → A: Không có tranh chấp đồng thời (đúng một admin thao tác tại một thời điểm — `QĐ-008`); khi admin gán lại một vị trí đã bị ghế khác chiếm, hệ thống hiện **dialog xác nhận "thay thế?" (Yes/No)** — Yes thì vị trí chuyển sang ghế mới và ghế cũ mất vị trí đó (thành chưa gán vị trí), No thì giữ nguyên phép gán cũ.
- Q: Khi admin gán số ghế ≠ 4 cho một trận **official**, hệ thống phản hồi bằng hạng nào? → A: ~~Chặn cứng ngay tại bước gán ghế~~ — **ĐÃ SỬA 2026-07-30 (phiên đối chiếu PRD)**: câu trả lời cũ **mâu thuẫn với `docs/PRD.md`**. `PRD-REQ-026` khai *"Mô hình dữ liệu **và giao diện** MUST hỗ trợ 1-12 ghế"*; `NON-GOAL-012` (PRD §5) khai *"v1 chỉ hỗ trợ **lưu trữ và giao diện** cho 1-12; đường xử lý **luật** chỉ có cho đúng 4"*; `QĐ-007` khai *"schema, seat model, `scoringUnit`, RuleConfig dạng mảng **và giao diện** làm cho 1-12 người ngay từ v1"*. Số ghế vì thế **KHÔNG** phải khoá cứng thứ tư của v1 — nó khác hẳn `rowCount`/playlist/`tieBreakPositions`, ba thứ mà PRD nói thẳng *"cửa tạo contest MUST NOT cho chọn giá trị khác"*. **Câu trả lời đúng: giao diện gán ghế MUST nhận 1-12 ghế và server MUST nhận phép gán đó; hàng rào đặt ở CỬA BẮT ĐẦU TRẬN / pre-flight** — trận có số ghế ≠ 4 bị **chặn cứng, không ép được**, vì v1 không có engine-path luật cho số ghế khác. Đây là chỗ hàng rào phải nằm để `QĐ-007` giữ được lời hứa "v1.5 chỉ ship luật + sửa controller, không dựng lại UI".
- Q: Khoá cứng "số ghế = 4" có áp cho contest **practice** không? → A: **Hàng rào ở cửa bắt đầu trận áp như nhau bất kể `contestPurpose`** — `CLAUDE.md` §Lộ trình version xác nhận luật cho 1-12 thí sinh (mọi mục đích trận, kể cả practice) là hạng mục của v1.5, nên ở v1 **không có engine-path đa ghế cho bất kỳ mục đích trận nào**. Bước **gán ghế** thì mở 1-12 cho cả hai, chỉ **bắt đầu trận** mới đòi đúng 4.
- Q: Khi contest builder mở lại một contest có `rowCount`/`tieBreakPositions` ngoài khoá cứng v1, giao diện xử lý ra sao? → A: **Zero-trust — mọi validate phải chạy ở SERVER, không riêng UI** (`CLAUDE.md` §Nguyên tắc code): server MUST từ chối mọi request ghi `rowCount ≠ 4` hoặc `tieBreakPositions ≠ [1]` cho một contest v1, bất kể đến từ UI hay gọi thẳng API — khoá cứng ở giao diện (FR-004, FR-007) chỉ là UX, không phải hàng rào duy nhất. Hệ quả: FR-005/FR-008 được hiểu lại — "vẫn nhận không lỗi cấu trúc" chỉ nói về **sức chứa của schema/kiểu dữ liệu** (chuẩn bị cho v1.5), KHÔNG có nghĩa API cấu hình contest của v1 chấp nhận ghi giá trị đó; đường duy nhất một giá trị "lạ" tồn tại trong dữ liệu là qua nguồn ngoài API v1 (seed/fixture/migration nội bộ) — không qua contest builder. Khi mở lại một contest mang giá trị như vậy, contest builder MUST hiển thị giá trị thật và khoá (disable) trường đó khỏi chỉnh sửa, vì server sẽ từ chối mọi lần ghi lại giá trị ≠ khoá cứng.
- Q: EPIC-004 liệt kê "chủ đề và âm thanh" trong phạm vi nhưng PRD không có mã `PRD-REQ` riêng — ở v1 đây là bắt buộc hay tuỳ chọn? → A: **Tuỳ chọn (SHOULD), không chặn tạo/bắt đầu contest**; ngoài ra hệ thống MUST có **gói chủ đề/âm thanh dựng sẵn (preset)** để admin chọn nguyên gói thay vì cấu hình thủ công từng phần, giảm gánh nhập liệu cho người dựng contest. **Điều hoà với `CLAUDE.md` §Sound** ("slot trống = silent, không có bộ SFX default"): preset là một **lựa chọn admin chủ động bấm chọn** (giống cơ chế "Áp dụng luật 2026"), khác hẳn việc hệ thống tự động điền một bộ mặc định vào slot mà admin không hề thao tác — CLAUDE.md vẫn đúng cho trường hợp admin không chọn preset nào và không tự tải file.
- Q (khái quát hoá, không qua `/speckit-clarify`, theo yêu cầu trực tiếp của chủ dự án): **"Luật cho toàn bộ: zero-trust validate, không phụ thuộc UI-only, UI chỉ là enhancement validate nhanh cho user"** có áp cho MỌI quy tắc chặn/khoá của feature này không, không riêng `rowCount`/`tieBreakPositions`? → A: **Có — khái quát hoá thành nguyên tắc chung `FR-033`.** Mọi quy tắc chặn/khoá/validate trong đặc tả này (ba khoá cứng v1, pre-flight kho đề, khoá sửa theo trạng thái vòng/trận, khoá gỡ câu đã hiển thị, chặn `everPublic`, chặn số ghế, dialog "thay thế?") MUST được **server thực thi**, bất kể request đến từ UI hay gọi thẳng API/socket; giao diện MUST chỉ là lớp tăng cường (ẩn lựa chọn, disable nút, báo lỗi tức thời cho phản hồi nhanh), MUST NOT là hàng rào duy nhất cho bất kỳ quy tắc nào. Đây là áp dụng trực tiếp nguyên tắc zero-trust đã có sẵn ở `CLAUDE.md` §Nguyên tắc code, không phải luật nghiệp vụ mới — các FR liên quan (FR-004, FR-006, FR-007, FR-014, FR-016, FR-018, FR-019, FR-022, FR-028, FR-029) đã được cập nhật để trỏ về `FR-033` thay vì lặp lại cùng một câu "server MUST" ở từng chỗ (giữ DRY theo `CLAUDE.md` §Nguyên tắc code).
...
5. **Given** một request gửi thẳng tới server (bỏ qua UI) với `rowCount = 6` hoặc `tieBreakPositions = [1, 2]` cho một contest v1, **When** server nhận request đó, **Then** server **từ chối** ghi giá trị đó — khoá cứng được **server thực thi (zero-trust)**, không chỉ là UX ẩn lựa chọn ở giao diện (§Clarifications 2026-07-30, `CLAUDE.md` §Nguyên tắc code). Sức chứa "vẫn nhận 5-8 / nhiều phần tử không lỗi cấu trúc" (PRD-REQ-025, PRD-REQ-105) chỉ nói về **kiểu dữ liệu của schema** (chuẩn bị cho v1.5), không phải một đường ghi hợp lệ qua API cấu hình contest của v1.
6. **Given** một contest mang `rowCount` hoặc `tieBreakPositions` ngoài khoá cứng v1 — chỉ có thể tồn tại qua nguồn **ngoài** API v1 (seed/fixture/migration nội bộ, không qua contest builder) — **When** admin mở lại contest đó trong contest builder, **Then** giao diện **hiển thị giá trị thật** và **khoá (disable) trường đó khỏi chỉnh sửa** — vì server sẽ từ chối mọi lần ghi lại giá trị ≠ khoá cứng, cho sửa tiếp cũng vô nghĩa (§Clarifications 2026-07-30).
...
10. **Given** một contest đã có **ít nhất một trận `FINISHED`** và trận hiện tại (nếu có) đang ở `LOBBY`, **When** admin sửa một giá trị RuleConfig của contest, **Then** hệ thống **cho phép**, và thay đổi đó **không đụng** tới bản cấu hình đã đóng băng vào bất kỳ trận nào đã bắt đầu hay đã `FINISHED` — chỉ áp cho trận **kế tiếp** của contest đó (§Clarifications 2026-07-30).
11. **Given** một vòng của trận hiện tại **đang chạy** (không ở `LOBBY`), **When** admin thử sửa một giá trị RuleConfig cấp contest, **Then** thao tác **không thực hiện được** — cùng cửa khoá với việc sửa danh sách câu (`GR-031` C8), và RuleConfig giữ nguyên không đổi.
...
2. **Given** một contest v1 (official hoặc practice), **When** admin gán N thí sinh vào N ghế với 1 ≤ N ≤ 12, N ≠ 4 — qua giao diện hoặc qua request gửi thẳng tới server — **Then** hệ thống **lưu phép gán đó bình thường**: `PRD-REQ-026` và `NON-GOAL-012` đòi **cả mô hình dữ liệu lẫn giao diện** hỗ trợ 1-12 ngay ở v1, nên bước gán ghế **không** có khoá cứng nào (§Clarifications 2026-07-30, đã sửa).
2b. **Given** một contest v1 đã gán N ghế với N ≠ 4, **When** admin bấm **bắt đầu trận**, **Then** server **chặn cứng, không ép được**, kèm thông điệp nêu rõ v1 chỉ có đường xử lý luật cho đúng 4 ghế; phép gán ghế **giữ nguyên** không bị xoá, và admin sửa lại về 4 ghế rồi bắt đầu được ngay (FR-030).
...
4. **Given** một contest đang gán ghế, ghế X đã giữ vị trí 2, **When** admin gán vị trí 2 cho ghế Y (khác ghế X), **Then** hệ thống hiện **dialog xác nhận "thay thế?"** (§Clarifications 2026-07-30); admin bấm **Yes** ⇒ vị trí 2 chuyển sang ghế Y, ghế X trở về trạng thái **chưa gán vị trí**; admin bấm **No** ⇒ phép gán **giữ nguyên như trước khi thao tác**, không ghế nào đổi vị trí.
5. **Given** một contest **practice** đang gán ghế, **When** admin gán số ghế khác 4, **Then** hành vi **giống hệt** contest official — phép gán lưu bình thường ở bước gán ghế, và cú bấm bắt đầu trận mới là chỗ bị chặn (AC US4-2, US4-2b); `contestPurpose` **không** đổi hàng rào nào, vì luật đa ghế cho **mọi** mục đích trận là hạng mục v1.5 (`CLAUDE.md` §Lộ trình version).
...
5. **Given** một contest chưa cấu hình chủ đề/âm thanh gì, **When** admin chọn một **gói dựng sẵn (preset)** từ danh sách gói có sẵn, **Then** hệ thống áp toàn bộ chủ đề và âm thanh của gói đó cho contest bằng một thao tác, và admin vẫn sửa lại từng phần sau đó nếu muốn (§Clarifications 2026-07-30).
6. **Given** một contest chưa cấu hình chủ đề/âm thanh và admin **không** chọn preset nào, **When** contest đó bắt đầu trận, **Then** hệ thống **không tự động điền** một chủ đề hay bộ âm thanh nào — chủ đề trống theo mặc định nền tảng (nếu có) và mọi slot âm thanh **im lặng**, đúng `CLAUDE.md` §Sound; đây không phải một ca lỗi.
...
- **Invalid state — sửa cấu hình luật khi một vòng đang chạy**: RuleConfig và danh sách câu cấp contest sửa được ở `LOBBY` hoặc sau trận `FINISHED`, nhưng **không thực hiện được** trong lúc một vòng đang chạy (§Clarifications 2026-07-30, mở rộng `GR-031` C8) — sửa đổi đó không đụng bản đã đóng băng vào trận đang chạy hoặc đã đóng sổ.
...
- **Stale state — admin mở contest builder trên một contest đã có giá trị `rowCount` hoặc `tieBreakPositions` ngoài khoá cứng v1** (chỉ tồn tại qua nguồn ngoài API v1): hiển thị giá trị thật, khoá trường khỏi chỉnh sửa (§Clarifications 2026-07-30, FR-032, AC US2-6).
- **Duplicate event — gán một vị trí đã bị ghế khác giữ**: không có tranh chấp đồng thời thật (một admin cho mỗi contest); hệ thống hiện dialog xác nhận "thay thế?" (§Clarifications 2026-07-30, FR-029).
...
- **Missing source behavior — chủ đề và âm thanh nằm trong phạm vi liệt kê của EPIC-004 nhưng chưa có `PRD-REQ` số hoá riêng**: đã xác nhận là cấu hình tuỳ chọn (SHOULD) qua §Clarifications 2026-07-30; khoảng trống truy nguyên trong `docs/PRD.md` §12 vẫn còn, nên đề nghị chủ dự án bổ sung mã `PRD-REQ` khi rà lại PRD.

## Requirements *(mandatory)*

### Functional Requirements

- **FR-033**: Với **mọi** quy tắc chặn/khoá/validate trong feature này — cả ba khoá cứng v1 (FR-004, FR-006, FR-007), pre-flight kho đề (FR-016), khoá sửa theo trạng thái vòng/trận (FR-014, FR-018, FR-028), khoá gỡ câu đã hiển thị (FR-019), chặn `everPublic` (FR-022), và chặn số ghế ≠ 4 **tại cửa bắt đầu trận** (FR-030) — server MUST là nơi thực thi quyết định, và MUST từ chối mọi request vi phạm bất kể request đến từ giao diện contest builder hay gọi thẳng API/socket. Giao diện MUST chỉ đóng vai trò **lớp tăng cường (enhancement)** — ẩn lựa chọn không hợp lệ, disable nút, báo lỗi tức thời — để admin có phản hồi nhanh mà không cần chờ round-trip; giao diện **MUST NOT** là hàng rào duy nhất cho bất kỳ quy tắc nào trong danh sách trên. *(Toàn bộ US-002, US-004, US-005, US-007; §Clarifications 2026-07-30; `CLAUDE.md` §Nguyên tắc code (zero-trust: "KHÔNG BAO GIỜ tin client — mọi request/socket event đều verify... bất kể client là ai"))*
...
- **FR-005**: Schema/kiểu dữ liệu của `rowCount` MUST có sức chứa cho giá trị 5-8 (chuẩn bị cho v1.5), nhưng server MUST từ chối — theo nguyên tắc zero-trust, không chỉ ẩn ở UI — mọi request ghi `rowCount ≠ 4` cho một contest v1, bất kể qua UI hay gọi thẳng API. *(US-002; PRD-REQ-025; §Clarifications 2026-07-30; `CLAUDE.md` §Nguyên tắc code (zero-trust); AC US2-5, US2-6)*
- **FR-006**: Giao diện tạo contest MUST khoá playlist ở đúng bốn vòng, đúng thứ tự Khởi động → Vượt chướng ngại vật → Tăng tốc → Về đích — không có thao tác thêm, bớt, lặp, hay đổi thứ tự vòng ở bước thiết kế contest; server MUST từ chối (zero-trust) mọi request cấu hình playlist khác bốn vòng chuẩn, bất kể qua UI hay gọi thẳng API. *(US-002; PRD-REQ-024; `GR-022`, `GR-030`; `QĐ-069`; `CLAUDE.md` §Nguyên tắc code (zero-trust); AC US2-3)*
...
- **FR-008**: Schema/kiểu dữ liệu của `tieBreakPositions` MUST có sức chứa cho mảng nhiều phần tử (chuẩn bị cho phiên bản sau), nhưng server MUST từ chối — theo nguyên tắc zero-trust — mọi request ghi `tieBreakPositions ≠ [1]` cho một contest v1, bất kể qua UI hay gọi thẳng API. *(US-002; PRD-REQ-105; `GR-022`, `GR-025`; §Clarifications 2026-07-30; `CLAUDE.md` §Nguyên tắc code (zero-trust); AC US2-5, US2-6)*
- **FR-032**: Khi một contest mang `rowCount` hoặc `tieBreakPositions` ngoài khoá cứng v1 (chỉ có thể tồn tại qua nguồn ngoài API v1), contest builder MUST hiển thị giá trị thật và MUST khoá (disable) trường đó khỏi chỉnh sửa thay vì từ chối mở toàn bộ contest. *(US-002; §Clarifications 2026-07-30; AC US2-6)*
...
- **FR-029**: Khi admin gán một vị trí đã bị một ghế khác giữ, giao diện MUST hiện dialog xác nhận "thay thế?" như lớp tăng cường; server MUST tự kiểm tra lại xung đột vị trí tại thời điểm ghi (không tin dialog đã hiện phía client) và chỉ thực hiện chuyển vị trí khi request mang xác nhận hợp lệ — xác nhận Yes MUST chuyển vị trí sang ghế mới và đưa ghế cũ về trạng thái chưa gán vị trí; No hoặc đóng dialog MUST giữ nguyên phép gán trước đó, không ghế nào đổi vị trí. *(US-004; §Clarifications 2026-07-30; `TERM-002`; `QĐ-005`, `QĐ-072` (dialog xác nhận thao tác một chiều của admin); FR-033; AC US4-4)*
- **FR-030**: Bước **gán ghế** MUST nhận tổng số ghế từ 1 đến 12 cho **mọi** contest v1 — official lẫn practice: giao diện MUST cho hoàn tất phép gán ở mọi giá trị trong dải đó, và server MUST ghi phép gán đó. Số ghế MUST NOT là một khoá cứng của cửa tạo contest — nó khác `rowCount`, playlist và `tieBreakPositions` ở đúng điểm này. Hàng rào của v1 nằm ở **cửa bắt đầu trận**: server MUST chặn cứng, không ép được, cú bấm bắt đầu một trận có số ghế đã gán khác 4, kèm thông điệp nêu rõ v1 chỉ có đường xử lý luật cho đúng 4 ghế; phép gán ghế MUST giữ nguyên khi cú bắt đầu bị chặn. *(US-004; §Clarifications 2026-07-30 (đã sửa); `PRD-REQ-026`, NON-GOAL-012, `QĐ-007`, `CLAUDE.md` §Lộ trình version, FR-033 (zero-trust); AC US4-1, US4-2, US4-5)*
- **FR-013**: Mô hình dữ liệu **và giao diện** MUST hỗ trợ 1 đến 12 thí sinh — không cần migrate schema **và không cần dựng lại giao diện** khi v1.5 mở khoá luật đa ghế; v1 chỉ chưa có **engine-path luật** cho số ghế khác 4, và chỗ duy nhất khác biệt đó lộ ra là cửa bắt đầu trận (FR-030). *(US-004; PRD-REQ-026 (GOAL-002); NON-GOAL-012; `QĐ-007`; `GR-007`, `GR-016`; `CLAUDE.md` §Lộ trình version; AC US4-2)*
...
- **FR-018**: Server MUST cho phép admin thêm hoặc bớt câu trong danh sách đã gán chỉ khi trận đang ở `LOBBY` (không vòng nào đang chạy) — kiểm tại server, bất kể request đến từ UI hay gọi thẳng API (FR-033) — và MUST chạy lại pre-flight ngay sau mỗi lần sửa. *(US-005; `GR-031` C5, C8; AC US5-5, US5-8)*
...
- **FR-023**: Hệ thống MUST cho phép admin chọn một chủ đề hiển thị cho contest; cấu hình này là **tuỳ chọn (SHOULD)**, MUST NOT chặn tạo hoặc bắt đầu contest nếu admin chưa chọn. *(US-006; không có `PRD-REQ` số hoá riêng — xem §Clarifications 2026-07-30; GOAL-008; AC US6-1, US6-6)*
- **FR-024**: Hệ thống MUST cho phép admin tải lên âm thanh riêng cho từng slot hiệu ứng của contest, trong ngưỡng kích thước đọc từ cấu hình môi trường; cấu hình này là **tuỳ chọn (SHOULD)**, MUST NOT chặn tạo hoặc bắt đầu contest nếu admin chưa cấu hình. *(US-006; không có `PRD-REQ` số hoá riêng — xem §Clarifications 2026-07-30; `CLAUDE.md` §Quy ước khác; AC US6-2, US6-4, US6-6)*
...
- **FR-031**: Hệ thống MUST cung cấp các gói chủ đề/âm thanh dựng sẵn (preset) mà admin chọn nguyên gói bằng một thao tác, thay cho việc phải tự cấu hình từng phần; admin MUST vẫn sửa lại được từng phần sau khi áp preset. *(US-006; §Clarifications 2026-07-30; AC US6-5)*
...
- **NON-GOAL-012** (luật cho số ghế ≠ 4) — **không cùng khuôn với ba khoá cứng trên**: `NON-GOAL-012` chỉ loại **đường xử lý luật**, và khai thẳng rằng v1 vẫn hỗ trợ **lưu trữ và giao diện** cho 1-12. Feature này vì thế **mở** bước gán ghế cho 1-12 (FR-013, FR-030) và chỉ dựng hàng rào ở cửa bắt đầu trận.
- **NON-GOAL-019** (trình hướng dẫn cài đặt lần đầu trên trình duyệt) và mọi cấu hình cấp hệ thống/triển khai (EPIC-012).

## Open Questions

- ~~**Q1**~~ — **Đã trả lời** (§Clarifications 2026-07-30): RuleConfig cấp contest và danh sách câu đã gán sửa được ở `LOBBY` và sau trận `FINISHED`; chỉ khoá trong lúc một vòng đang chạy.
- ~~**Q2**~~ — **Đã trả lời, rồi ĐÃ SỬA 2026-07-30** (§Clarifications): trùng vị trí ⇒ dialog xác nhận "thay thế?" (Yes/No). Vế số ghế: câu trả lời đầu ("chặn cứng ngay tại bước gán ghế") **mâu thuẫn `PRD-REQ-026`/`NON-GOAL-012`/`QĐ-007`** và đã bị thay — bước gán ghế nhận 1-12, hàng rào chuyển sang **cửa bắt đầu trận** (FR-030).
- ~~**Q3**~~ — **Đã trả lời** (§Clarifications 2026-07-30): zero-trust — server từ chối mọi ghi giá trị ngoài khoá cứng cho contest v1, bất kể qua UI hay API thẳng; giá trị "lạ" chỉ có thể tồn tại qua nguồn ngoài API v1 (seed/fixture/migration nội bộ). Khi mở lại một contest như vậy, contest builder hiển thị giá trị thật và khoá (disable) trường đó khỏi chỉnh sửa.
- ~~**Q4**~~ — **Đã trả lời** (§Clarifications 2026-07-30): chủ đề/âm thanh là **tuỳ chọn (SHOULD)** ở v1, không chặn tạo/bắt đầu contest; có preset gói dựng sẵn để giảm gánh cấu hình, đúng đắn hoà hợp với `CLAUDE.md` §Sound (preset là hành động chủ động của admin, không phải auto-fill). Việc PRD thiếu mã `PRD-REQ` số hoá cho mục này vẫn là một khoảng trống truy nguyên trong `docs/PRD.md` §12 — nên được chủ dự án bổ sung vào PRD sau, không phải lỗi của spec này.
- ~~**Q5**~~ — **Đã trả lời** (§Clarifications 2026-07-30): hàng rào "đúng 4 ghế" **tại cửa bắt đầu trận** áp như nhau cho contest official lẫn practice ở v1 — luật đa ghế cho **mọi** mục đích trận (kể cả practice) là hạng mục của v1.5. Bước **gán ghế** thì mở 1-12 cho cả hai.
- **Q6 — MỚI, cần chủ dự án xác nhận nơi đặt FR đối ứng**: FR-030 đặt hàng rào ≠4 ghế ở **cú bấm bắt đầu trận**, mà cú bấm đó thuộc `PRD-REQ-028`/EPIC-005 (`specs/005-phong-thi-vong-doi-tran`). Spec 005 hiện **chưa có FR nào** cho điều kiện tiên quyết này. Cần một trong hai: (a) thêm một FR vào spec 005 khai cú bắt đầu trận kiểm số ghế, hoặc (b) chấp nhận FR-030 của spec 004 là chủ sở hữu duy nhất và spec 005 chỉ trỏ tới. Không phải mâu thuẫn — là một khoảng trống phân công giữa hai spec.
...
| US-006 — Chủ đề và âm thanh | (GOAL-008, phạm vi liệt kê trong EPIC-004 §11; không có `PRD-REQ` số hoá — đã xác nhận SHOULD qua §Clarifications 2026-07-30) | — | FR-023, FR-024, FR-025, FR-031 | US6-1…US6-6 |
