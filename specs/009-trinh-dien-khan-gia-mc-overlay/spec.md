...
- Q: `CONFLICT-001` — Màn MC có bị banner tạm dừng phủ không? → A: **KHÔNG phủ.** Sửa `game-state-machine.md` §F bảng nhóm `F.3` — bỏ **MC** khỏi cột *"Ai thấy"*. Banner tạm dừng tác động lên đúng **hai** loại màn như `PRD-REQ-076` và `STATE-040` §Mô tả đã khai: **máy thí sinh** *(che toàn bộ + chặn toàn bộ thao tác)* và **viewer/overlay** *(báo hiệu thuần thị giác)*. Màn MC và màn admin đều **không bị phủ**. Đã tích hợp vào **FR-055**, **FR-055a** *(mới)* và **AC-050**, **AC-050a** *(mới)*.
- Q: Vậy `OQ-001` *(MC thao tác được gì dưới banner)* còn đối tượng không? → A: **Không — câu hỏi mất chủ ngữ.** Banner không phủ màn MC nên không có lớp nào để prompt `STATE-039` phải nổi lên trên, và MC vẫn đọc câu hỏi cùng đáp án bình thường. `STATE-039` §Cho phép giữ nguyên hiệu lực mà không cần một ngoại lệ mới.
- Q: Vì sao không chọn *"banner phủ cả màn MC"* cho nhất quán với bảng `F.3`? → A: **Vì nó đẻ ra một ca kẹt mà nguồn không có.** Tổ hợp *(banner đang bật × phiên admin giữ quyền mất kết nối × admin khác bấm giành)* dựng được; nếu banner chặn prompt `STATE-039` thì cú giành **không ai duyệt được** và trận kẹt ở đúng tình huống `QĐ-093` sinh ra để cứu. Thêm nữa, màn `/mc` là **read-only** *(trừ đúng một prompt)* nên vế *"chặn thao tác"* của nhóm `F.3` **không có đối tượng** trên màn này — chỉ còn vế *"che nội dung"*, mà che nội dung của MC thì phá đúng lý do màn MC tồn tại: MC là người **đọc câu hỏi** trên sân khấu.
- Q: `CONFLICT-002` — Khe âm thanh cho sự kiện điều khiển va vào lệnh cấm báo khán giả về can thiệp của admin. → A: **Tách theo KHE, không theo nhóm.** Khe *mở màn công bố* **hướng khán giả** — nó là một **mốc trình diễn có chủ đích**, và nó **không** nằm trong danh sách bốn thao tác mà `QĐ-076` cấm. Năm khe còn lại — *hoàn nguyên · bỏ vòng · chạy lại vòng · kết thúc sớm · ép qua cảnh báo* — **chỉ phát trên máy admin**, không bao giờ tới kênh public. `QĐ-076` giữ nguyên và **không cần ngoại lệ nào**. Đã tích hợp vào **FR-074**, **FR-074a** *(mới)*, **FR-065a** *(mới)* và **AC-069**, **AC-069a** *(mới)*.
- Q: Máy nào thực sự phát tiếng cho khán giả? → A: **Nguồn phát là cấu hình được — `admin` hoặc `overlay` — mặc định `overlay`.** Lớp phủ dựng stream là mặc định vì nó **chính là buổi phát sóng** (`QĐ-102`), nên tiếng của nó vào thẳng bản trộn. Admin bật/tắt và đổi nguồn được. Đã tích hợp vào **FR-074b** *(mới)* và **AC-069b** *(mới)*.
- Q: Cấu hình nguồn phát áp cho khe nào? → A: **Chỉ áp cho khe HƯỚNG KHÁN GIẢ** *(các mốc thi đấu + mở màn công bố)*. Năm khe điều khiển **luôn** phát ở máy admin và **không** đi qua cấu hình này — nên mặc định `overlay` MUST NOT đẩy được tiếng *bỏ vòng* vào bản phát sóng. **Màn khán giả im lặng**: nó **không bao giờ** là nguồn phát, vì một hội trường có 200 người xem trên 200 máy sẽ thành 200 nguồn tiếng lệch nhau.
- Q: Trận **không có lớp phủ nào kết nối** mà nguồn phát đang là `overlay` — có phương án lui tự động sang `admin` không? → A: **KHÔNG lui.** Hệ thống MUST NOT tự đổi nguồn phát; admin **tự đổi cấu hình** khi cần. Cùng khuôn với `QĐ-001` — máy không tự quyết thay người — và cùng khuôn với `QĐ-079` *(khe trống là im lặng, không báo lỗi)*: im lặng vì thiếu nguồn phát là một **trạng thái hợp lệ**, không phải một sự cố phải tự cứu. Đã tích hợp vào **FR-074c** *(mới)* và **AC-069c** *(mới)*.
- Q: `OQ-003` — Lớp phủ dựng stream có nhận đáp án **Chướng ngại vật** khi admin bấm công bố theo `GR-012` C2 không? → A: **CÓ.** Lớp phủ nhận **cùng lúc và cùng điều kiện với màn khán giả**. `GR-012` C2 viết *"cho viewer"* là **viết tắt của hai kênh public**, không phải một lệnh cấm riêng với overlay. Đã tích hợp vào **FR-026**, **FR-026a** *(mới)* và **AC-023**.
- Q: Vậy `QĐ-080` vế 4 *(loại Chướng ngại vật khỏi cơ chế công bố)* có bị phá không? → A: **Không — hai trục khác nhau.** Vế 4 loại Chướng ngại vật khỏi **trục THỜI ĐIỂM** *(mốc câu khép + cờ `revealAnswerAfterJudge`)*, chứ **không** loại nó khỏi **trục NGƯỜI NHẬN**. Trục người nhận đã được vế 2 chốt dứt điểm: *"lệnh cấm tuyệt đối với overlay **bị gỡ**"*, và `TERM-007` khai overlay nhận *"cùng lúc và cùng điều kiện với viewer"*. Đáp án Chướng ngại vật vẫn theo **thời điểm** của `GR-012` — lộ khi có người giải đúng, hoặc khi admin bấm công bố — và theo **người nhận** của hai kênh public.
- Q: Vì sao không giữ lệnh cấm riêng cho overlay ở đúng đường này? → A: **Vì nó biến bản phát sóng thành kênh duy nhất không thấy cao trào của vòng.** `QĐ-102` gọi lớp phủ là *"chính buổi phát sóng, không phải khán giả của nó"*. Khoảnh khắc mở toàn bộ miếng ghép và hiện Chướng ngại vật là điểm nhấn của cả vòng VCNV; giấu nó khỏi đúng kênh đông người xem nhất là hỏng sản phẩm ở chỗ khách hàng nhìn thấy, mà **không** bảo vệ được gì — tới mốc đó đề đã lộ cho toàn bộ khán giả tại chỗ.
- Q: `OQ-002` — Lớp nào trên cùng khi banner tạm dừng và lớp công bố kết quả cùng bật? → A: **Không có lớp nào trên cùng — hai lớp LOẠI TRỪ LẪN NHAU.** Không mở được lớp công bố khi banner đang bật, và không mở được banner khi lớp công bố đang bật. Tổ hợp *(banner × lớp công bố)* trở thành **không dựng được**, nên câu hỏi thứ tự chồng lớp **mất chủ ngữ** — cùng khuôn lập luận mà `QĐ-050` đã dùng cho cặp *banner × đồng hồ*. Đã tích hợp vào **FR-044**, **FR-058a** *(mới)* và **AC-041**, **AC-058a** *(mới)*.
- Q: Vậy `INV-021` *(nhiều lớp phủ bật cùng lúc là hợp lệ)* có bị phá không? → A: **Không — thu hẹp, không phủ định.** `INV-021` viết vế đó làm **hệ quả** của lập luận *"đừng nhét lớp phủ vào enum cấp trận"*, và phán quyết này **không** nhét chúng vào enum: nó chỉ thêm **một** guard giữa **hai** lớp cụ thể. Mọi tổ hợp lớp phủ khác vẫn bật cùng lúc được, và mở/đóng lớp phủ vẫn **không** đổi trạng thái bên dưới.
- Q: Đây có phải chỗ **chặn cứng** thứ tư không? → A: **Không — đây là INVALID STATE.** `INV-014` khai đúng **ba** chỗ chặn cứng **của luật chơi**, và cả ba đều là **ngưỡng tài nguyên**. Cặp loại trừ này là một **thao tác không có đối tượng** ở trạng thái hiện tại: nút **không bật**, lệnh lọt tới server thì bị từ chối kèm **toast**, và **không ép được** — y hệt cặp *banner × đồng hồ* đã có.
- Q: Vì sao không chọn thứ tự chồng lớp cố định cho gọn? → A: **Vì loại trừ dùng lại một pattern đã có, còn thứ tự chồng lớp đẻ một trục mới.** Dựng thứ tự z là mở một chiều quy định *"lớp nào thắng lớp nào"* phải duy trì cho **mọi cặp** lớp phủ về sau; loại trừ thì đóng hẳn câu hỏi cho cặp duy nhất dựng được trên kênh public. Nó cũng buộc admin **kết thúc dứt điểm** phần công bố trước khi tạm dừng trận, thay vì để hai thông điệp trình diễn chồng nhau trên sóng.
- Q: `OQ-006` — Mốc câu khép gặp một cú **đóng hiển thị bằng tay** của `PERM-044` thì bên nào thắng? → A: **Cú đóng tay THẮNG.** Engine MUST NOT đẩy đè lên một cú đóng tay còn hiệu lực; đáp án vẫn kín trên hai kênh public tới khi admin **tự mở lại**. Cơ sở: cú đẩy của engine tại mốc câu khép bị gác bởi **chính cờ `revealAnswerAfterJudge`**, mà `permissions.md` đã khai `PERM-044` **"thắng cờ"** đó — nếu engine đẩy đè thì `PERM-044` thắng cờ nhưng thua cái chạy **bằng** cờ, tức nghĩa của nó rỗng đi ở đúng ca người ta cần. Khớp `QĐ-001` *(người độc quyền phán quyết)* và `CLAUDE.md` §Admin *("admin **toàn quyền** mở và đóng đáp án, ô chữ")*. Đã tích hợp vào **FR-027**, **FR-027a** *(mới)* và **AC-024**, **AC-024a** *(mới)*.
- Q: Vậy `GR-037` §Bấm trùng *("công bố là một chiều ở phía engine")* có bị phá không? → A: **Không — nó nói đúng một chiều, và chiều đó vẫn nguyên.** *"Một chiều"* nghĩa là **engine không tự thu lại** thứ nó đã đẩy; nó **không** nói *"cú đẩy của engine đè được lên phán quyết của người"*. Hai mệnh đề độc lập: engine không tự thu, và người thì thu được — đó chính là hình dạng của mô hình ADVISORY. Kết quả: **engine chỉ đi một chiều, admin đi được cả hai.**
- Q: Cú đóng tay giữ hiệu lực trong bao lâu? → A: **Trong phạm vi CÂU**, và chấm dứt bằng đúng một cách — admin **tự mở lại**. Nó MUST NOT là một cờ dính sang câu sau: sang câu mới, hiển thị đáp án quay về hành vi mặc định do cờ cấp trận quyết định. Đây là hệ quả của `NFR-25` *(mỗi vòng bắt đầu sạch)* áp xuống cấp câu, không phải một luật mới.
- **Ghi chú về một hệ quả, không phải một quyết định riêng**: bỏ MC khỏi `F.3` **không** đồng nghĩa MC không biết trận đang tạm dừng. MC là vai **có mặt tại sân khấu** và là người tuyên bố việc tạm dừng bằng **lời** (`QĐ-001` tầng MC) — thông tin đó tới MC qua kênh người, không qua lớp phủ. Đây cùng khuôn với `QĐ-050`: banner **không chữ** vì *"khán giả tự hiểu từ bối cảnh sân khấu"*.

### Session 2026-08-04 (phiên hai)

- Q: `OQ-004` vế 1 — *"chủ đề hiển thị"* gồm những trục nào? → A: **Ba trục TĨNH — bảng màu · logo giải · ảnh nền.** Không có tài sản động. Phông chữ đã bị loại từ trước vì `CLAUDE.md` §Quy ước code khoá cứng *"Be Vietnam Pro"*. Lý do không lấy **hình hiệu động**: nó đẻ thêm một loại media phải preload, phải có ngưỡng dung lượng riêng, và trùng vai với **khe âm thanh** cùng **module animation** — hai trục vốn đã tách riêng theo `QĐ-079` và `CLAUDE.md` §Quy ước code. Đã tích hợp vào **FR-072**, **FR-072a** *(mới)* và **AC-067**.
- Q: `OQ-005` vế còn mở — danh sách đầy đủ các khe của nhóm *mốc thi đấu* xác định thế nào? → A: **Dẫn xuất từ wiki Fandom, lọc hai tầng.** Tập khe = cột **Tên** ở trang [`Âm thanh`](https://duong-len-dinh-olympia.fandom.com/vi/wiki/%C3%82m_thanh) của wiki Fandom — cùng nguồn mà `docs/source/fandom-olympia-26-luat-choi.md` đã lấy luật — **trừ** hai tầng: **(1) biến thể ngoài O26** *(`Ô mạo hiểm` 6 khe; `Bộ 8/12/16 câu hỏi` là cấu trúc Khởi động O23, còn O26 là lượt riêng 6 câu + lượt chung 12 câu)*; **(2) khe truyền hình không ứng với sự kiện engine** *(nhóm ngoài trận — nhạc hiệu mở đầu, giới thiệu cuộc thi, hình hiệu phần thi, Olympedia, Hãy đón xem…; và nhóm tổng kết — trao giải, giải nhất/nhì/ba/tư)*, vì giữ chúng thì `FR-077` mất nghĩa: không sự kiện nào của engine ứng với chúng, và khe *mở màn công bố* đã phủ mốc tổng kết. **Một điều chỉnh bắt buộc**: wiki tách khe theo **độ dài đồng hồ** vì O26 có thời lượng cố định; dự án này đặt `timeSeconds` ở **từng câu** và khai *"mọi timer là RuleConfig"*, nên mọi biến thể độ dài MUST gộp thành **một khe *đếm giờ* mỗi vòng**. Kết quả: **52 khe** nhóm mốc thi đấu. Đã tích hợp vào **FR-074d** *(mới)* và **AC-069d** *(mới)*.
- Q: `OQ-007` — lớp phủ bật mà trạng thái bên dưới không cho nội dung có nghĩa thì hai kênh public hiện gì? → A: **Dựng bình thường với dữ liệu hiện có — không có nhánh "rỗng".** Ca dựng được nhất là mở lớp công bố ở `LOBBY`: mọi ghế `0` ⇒ bảng hiện đủ ghế, điểm `0`, **cả bảng đồng hạng 1** — đúng kết quả mà quy tắc đồng hạng (`GR-025` C5, `QĐ-049`) đã định nghĩa, không phải một ca chưa khai. Một trạng thái rỗng riêng sẽ đẻ thêm một loại nội dung, một chuỗi giao diện, và một ngưỡng *"thế nào là rỗng"* phải định nghĩa cho **từng** lớp phủ. Từ chối cú mở lại phá `QĐ-001` — máy tự phán rằng thứ admin muốn đưa lên sóng là *"không đáng hiển thị"*. Đã tích hợp vào **FR-085a** *(mới)* và **AC-079a** *(mới)*.
- Q: `OQ-004` vế 2 — chủ đề áp cho những màn nào? → A: **Chỉ HAI KÊNH PUBLIC** — màn khán giả và lớp phủ dựng stream. Màn thí sinh, màn MC và màn admin là bề mặt **vận hành**, không phải bề mặt trình diễn; giữ chúng ngoài chủ đề loại hết rủi ro tương phản và khả năng đọc mà chính `OQ-004` nêu, và khớp `FR-082` *(nội dung chính trong một khung nhìn)* cùng *"chữ rất to"* của `PRD-REQ-074`. Đã tích hợp vào **FR-072b** *(mới)* và **AC-067a** *(mới)*.
...
- **Khoá cổng đang bật**: khán giả **mới** không vào được; khán giả **đang xem** không bị ngắt; **lớp phủ vẫn vào được**, kể cả một lần kết nối lại giữa buổi phát sóng (`QĐ-102`, `PRD-REQ-086` §Note). Cơ chế của nút này thuộc EPIC-012.
...
- **Mở lớp công bố khi banner tạm dừng đang bật** — và chiều ngược lại: nút **không bật**, toast invalid state, **không ép được**; tổ hợp *(banner × lớp công bố)* **không dựng được** (`FR-058a`, §Clarifications 2026-08-04).
...
- **`CONFLICT-001`** — **màn MC có bị banner tạm dừng phủ không**: **đã phân xử** — **KHÔNG phủ**; banner tác động lên đúng hai loại màn là máy thí sinh và hai kênh public. Xem §Clarifications 2026-08-04 và §Open Questions. `game-state-machine.md` §F bảng nhóm `F.3` vẫn cần sửa cho khớp.
- **`CONFLICT-002`** — **khe âm thanh cho sự kiện điều khiển vs *"khán giả không được báo về can thiệp của admin"***: **đã phân xử** — tách theo **khe**, không theo nhóm; *mở màn công bố* hướng khán giả, năm khe sự cố chỉ phát trên máy admin. Xem §Clarifications 2026-08-04 và §Open Questions. `QĐ-079` và `PRD-REQ-077` vẫn cần sửa cho khớp.
- **`PRD-REQ-073` *"cả hai vào bằng mã phòng 6 số"* vs `QĐ-096` *"vào bằng ĐÚNG một URL, không bước nhập mã"* — không phải conflict.** `QĐ-096` vế 1 khai *"mã phòng 6 số nằm **TRONG** URL"*, nên hai câu nói về cùng một thứ ở hai mức chi tiết. `PRD-REQ-001` §Note *(dòng 566)* đã ghi nhận `QĐ-096`, và `JOURNEY-004` main flow đã viết đúng dạng *"mở đường dẫn, không tài khoản và không bước nhập mã"*.
- **`PRD-REQ-075` *"lớp phủ công bố áp cho cả máy thí sinh"* vs `GR-037` *"đáp án mật tới mốc câu khép"* — không phải conflict.** `GR-037` §bảng ba loại thông tin tách rạch ròi: **điểm số** là **công khai với mọi vai, luôn luôn**; chỉ **đáp án chuẩn** mới mật. Lớp công bố chỉ chứa tên, điểm và thứ hạng, nên nó không lộ thêm gì (`QĐ-049` §Lớp phủ áp cho MỌI vai, `QĐ-015`).
- **`STATE-040` *"banner che toàn bộ màn thí sinh"* vs `PRD-REQ-067` *"máy thí sinh hiện bảng điểm realtime"* — không phải conflict.** `QĐ-115` khai đây là **ngoại lệ tường minh và duy nhất** của `PRD-REQ-067`, và mặt máy thí sinh của nó đã được `specs/008` FR-076a đặc tả. Spec này chỉ nhận mặt **viewer/overlay**.

**Missing source behavior**

- **`OQ-001`** — thao tác của MC dưới banner tạm dừng: **đã phân xử cùng `CONFLICT-001`** — câu hỏi **mất chủ ngữ** vì banner không phủ màn MC. Xem §Clarifications 2026-08-04.
- **`OQ-002`** — thứ tự chồng lớp khi banner tạm dừng và lớp công bố cùng bật: **đã phân xử** — hai lớp **loại trừ lẫn nhau**, tổ hợp không dựng được nên câu hỏi mất chủ ngữ. Xem §Clarifications 2026-08-04 và §Open Questions.
- **`OQ-003`** — lớp phủ dựng stream có nhận đáp án **Chướng ngại vật** không: **đã phân xử** — **CÓ**, cùng lúc và cùng điều kiện với màn khán giả; đứng ngoài `GR-037` chỉ đổi **thời điểm**, không đổi **người nhận**. Xem §Clarifications 2026-08-04 và §Open Questions.
- **`OQ-004`** — *"chủ đề hiển thị"* gồm những gì và áp cho những màn nào: **đã phân xử** — **ba trục tĩnh** *(bảng màu · logo · ảnh nền)*, áp cho đúng **hai kênh public**. Xem §Clarifications 2026-08-04 phiên hai và §Open Questions.
- **`OQ-005`** — danh sách **đầy đủ** các khe của nhóm *mốc thi đấu*: **đã phân xử** — dẫn xuất từ trang `Âm thanh` của wiki Fandom, lọc hai tầng, ra **52 khe**; biến thể độ dài đồng hồ gộp thành một khe *đếm giờ* mỗi vòng. Vế *kênh nào phát* đã đóng từ trước cùng `CONFLICT-002`. Xem §Clarifications 2026-08-04 phiên hai và §Open Questions.
...
- **Khe âm thanh của một thao tác can thiệp phát ra bản trộn phát sóng**: đây là ca mà một lệnh cấm **thị giác** bị lách qua đường **thính giác**. Đã đóng — năm khe sự cố thuộc nhóm **chỉ-admin** và không đi qua cấu hình nguồn phát (`FR-065a`, `FR-074a`).
...
- **`OQ-006`** — mốc câu khép gặp cú đóng hiển thị bằng tay của `PERM-044`: **đã phân xử** — **cú đóng tay thắng**, phạm vi câu, chấm dứt bằng cú mở lại của admin. Xem §Clarifications 2026-08-04 và §Open Questions.
- **Cú đóng tay còn hiệu lực khi sang câu mới**: **không dính** — hành vi hiển thị của câu mới quay về mặc định do cờ cấp trận quyết định (`FR-027a`, `NFR-25`).
- **`OQ-007`** — kết cục hiển thị của một lớp phủ khi trạng thái bên dưới không dựng được nội dung cho nó: **đã phân xử** — dựng **bình thường với dữ liệu hiện có**, không có nhánh trạng thái rỗng. Xem §Clarifications 2026-08-04 phiên hai và §Open Questions.
...
- **FR-074a**: Mỗi khe MUST thuộc đúng **một** trong hai nhóm, và ranh giới MUST là *khán giả có được nghe khe này không* *(§Clarifications 2026-08-04)*:
...
> Chín mục dưới đây được ghi theo Nguyên tắc II và III của `.specify/memory/constitution.md`. Mục nào **chưa** được chủ dự án phân xử thì **giữ nguyên, không tự trả lời**; mục đã phân xử giữ lại dưới dạng **bản ghi đã đóng** để truy vết. Mọi FR mang cảnh báo ⚠️ trỏ tới một mục **còn mở** MUST NOT vào task implementation trước khi `docs/` được sửa.
>
> **Tiến độ phiên làm rõ 2026-08-04**: **6/9 mục đã phân xử** — `CONFLICT-001` *(banner **không** phủ màn MC)* · `CONFLICT-002` *(khe âm thanh tách theo **khe**; nguồn phát `admin | overlay`, mặc định `overlay`)* · `OQ-001` *(**mất chủ ngữ** theo `CONFLICT-001`)* · `OQ-002` *(banner × lớp công bố **loại trừ lẫn nhau**)* · `OQ-003` *(lớp phủ **CÓ** nhận đáp án Chướng ngại vật)* · `OQ-006` *(**cú đóng tay thắng** cú đẩy tại mốc câu khép)*. **Còn ba mục**: `OQ-004` · `OQ-005` *(chỉ còn vế danh sách khe)* · `OQ-007`.
>
> ✅ **Nợ TRUY NGUYÊN của sáu mục đã phân xử: ĐÃ ĐÓNG (2026-08-04).** `docs/decisions.md` nhận **năm** `QĐ` mới — **`QĐ-116`** *(banner không phủ MC)* · **`QĐ-117`** *(banner × lớp công bố loại trừ)* · **`QĐ-118`** *(đáp án Chướng ngại vật tới cả overlay)* · **`QĐ-119`** *(thao tác tay thắng cả hai chiều)* · **`QĐ-120`** *(hai nhóm khe âm thanh + nguồn phát)*. Đã sửa theo ở `PRD.md` *(`PRD-REQ-049`, `073`, `075`, `076`, `077`, `078`, §22.3, bump 2.7.0)*, `game-rules.md` *(`GR-012`, `GR-037` **C10** mới)*, `game-state-machine.md` *(§F, `STATE-021`, `STATE-033`, `STATE-039`, `STATE-040`, `EVENT-032`, `EVENT-034`, `T-087`, `T-089`, §Invalid transitions, `INV-017`, `INV-021`)*, `permissions.md` *(`PERM-044`, `PERM-047`, `PERM-048`, vai MC)*, `glossary.md` *(`TERM-007`)*. Mọi FR neo vào sáu mục đó nay **truy nguyên được về `docs/`** và vào task implementation được.
>
> **Tiến độ phiên làm rõ 2026-08-04 phiên hai**: **9/9 mục ĐÃ PHÂN XỬ.** Ba mục còn lại đóng ở phiên này — `OQ-004` *(chủ đề = ba trục tĩnh, áp cho hai kênh public)* · `OQ-005` *(52 khe mốc thi đấu, dẫn xuất từ wiki Fandom + hai tầng lọc)* · `OQ-007` *(lớp phủ rỗng dựng bình thường, không nhánh trạng thái rỗng)*. **Không còn mục nghiệp vụ nào mở.**
>
> ✅ **Nợ TRUY NGUYÊN của ba mục phiên hai: ĐÃ ĐÓNG (2026-08-04).** `docs/decisions.md` nhận **ba `QĐ` mới** — **`QĐ-121`** *(chủ đề = ba trục tĩnh, áp cho hai kênh public)* · **`QĐ-122`** *(52 khe mốc thi đấu dẫn xuất từ wiki Fandom, lọc hai tầng)* · **`QĐ-123`** *(lớp phủ thiếu dữ liệu vẫn dựng bình thường)* — cộng **một nguồn mới** `docs/source/fandom-olympia-26-am-thanh.md`. Đã sửa theo ở `PRD.md` *(`PRD-REQ-075`, `PRD-REQ-078`, §22 ma trận, §22.3, bảng nguồn, bump **2.8.0**)*, `traceability.md` *(bảng nguồn gốc)*, `README.md` và `CLAUDE.md` *(đếm quyết định 120 → 123)*. Một con trỏ ⚠️ ngược đặt ở `QĐ-079` cho cụm *"ngoài các mốc thi đấu"*.
>
> **Không còn FR nào mang cảnh báo chặn implementation.** Toàn bộ **9/9** mục truy nguyên được về `docs/` và vào task được.

### CONFLICT

- **`CONFLICT-001` — ✅ ĐÃ PHÂN XỬ (§Clarifications 2026-08-04)**: **Màn MC có bị banner tạm dừng phủ không.** Phán quyết của chủ dự án: **KHÔNG phủ.** Banner tạm dừng tác động lên đúng **hai** loại màn — máy thí sinh *(che toàn bộ + chặn toàn bộ thao tác)* và hai kênh public *(báo hiệu thuần thị giác)*. Màn MC và màn admin đều **không bị phủ**; trên màn MC, câu hỏi và đáp án vẫn đọc được và prompt `STATE-039` vẫn bấm được. Hai lập luận: **(1)** màn `/mc` là **read-only** nên vế *"chặn thao tác"* của nhóm `F.3` **không có đối tượng** ở đó, chỉ còn vế *"che nội dung"* — mà che nội dung của MC thì phá đúng lý do màn MC tồn tại; **(2)** phương án ngược lại đẻ ra một **ca kẹt** không có trong nguồn. Đã tích hợp vào **FR-055**, **FR-055a** và **AC-050**, **AC-050a**. Kéo theo `OQ-001` **mất chủ ngữ**.

  <details><summary>Nội dung mâu thuẫn trước khi phân xử</summary>

  **Nguồn A** — `docs/game-state-machine.md` §F bảng nhóm lớp phủ, dòng `F.3`: cột *"Ai thấy"* ghi **"thí sinh · viewer · overlay · MC (không phủ admin)"**, tức màn MC **có** nằm trong tập bị lớp phủ chặn-toàn-cục tác động, và ngoại lệ duy nhất được nêu tên là admin.

  **Nguồn B** — `docs/PRD.md` §12 `PRD-REQ-076`: *"Banner tạm dừng MUST chặn toàn bộ thao tác trên máy thí sinh và báo tạm dừng trên màn khán giả cùng lớp phủ, **không chữ, không lý do**; MUST NOT phủ màn admin."* — liệt kê **ba** loại màn và **một** ngoại lệ; MC không có mặt ở cả hai vế. `docs/game-state-machine.md` `STATE-040` §Mô tả cũng chỉ mô tả *"**Hai** tác dụng trên **hai** loại màn"* — máy thí sinh, và viewer/overlay.

  Hai nguồn cho hai tập màn khác nhau. `QĐ-050` không nhắc MC. `QĐ-115` thu hẹp phạm vi của mình một cách tường minh — *"Chỉ **máy thí sinh**"* — nhưng cũng không nói MC.

  **Vì sao nó quan trọng chứ không phải chi tiết hiển thị.** `STATE-039` là **bề mặt quyền ghi duy nhất của MC trong toàn hệ thống**, và `STATE-039` §Cho phép nói *"mọi thao tác của trạng thái bên dưới **vẫn chạy**"*. Nếu banner **có** phủ màn MC theo Nguồn A, thì tổ hợp *(banner đang bật × phiên admin giữ quyền mất kết nối × một admin khác bấm giành)* làm cú giành **không ai duyệt được** — trận kẹt ở đúng tình huống mà `QĐ-093` được viết ra để cứu. Nếu banner **không** phủ màn MC theo Nguồn B, thì Nguồn A ghi sai một ô trong bảng phân nhóm.

  Phán quyết chọn **Nguồn B**: Nguồn A ghi sai một ô. Cột *"Ai thấy"* của bảng §F trộn hai câu hỏi khác nhau — *ai nhìn thấy lớp phủ* và *lớp phủ chặn thao tác của ai* — mà nhóm `F.3` chỉ có nghĩa với vế thứ hai; MC không có thao tác nào để chặn.

  </details>

  **Cổng truy nguyên — ✅ ĐÃ THOẢ (2026-08-04).** Ghi **`QĐ-116` — Banner tạm dừng KHÔNG phủ màn MC**. Đã sửa theo: `game-state-machine.md` §F bảng nhóm dòng `F.3` *(bỏ MC)*, `STATE-040` §Mô tả và §Vào, `STATE-039` §Cho phép, `T-089`, sơ đồ mermaid `S041`; `PRD-REQ-076` bổ sung vế *không phủ màn MC*; `permissions.md` `PERM-048` và dòng vai **MC**; `PRD.md` §22.3. Ảnh hưởng: **FR-055**, **FR-055a**. *(`game-state-machine.md` §F bảng nhóm, `STATE-039` §Cho phép, §Cấm, `STATE-040` §Mô tả, §Cho phép, §Cấm · `PRD-REQ-076`, `PRD-REQ-108` · `QĐ-050`, `QĐ-093`, `QĐ-115`)*

- **`CONFLICT-002` — ✅ ĐÃ PHÂN XỬ (§Clarifications 2026-08-04)**: **Khe âm thanh cho sự kiện điều khiển vs lệnh cấm báo cho khán giả.** Phán quyết của chủ dự án: **tách theo KHE, không theo nhóm.** Khe *mở màn công bố* **hướng khán giả**; năm khe còn lại — *hoàn nguyên · bỏ vòng · chạy lại vòng · kết thúc sớm · ép qua cảnh báo* — **chỉ phát trên máy admin**. Cơ sở văn bản: danh sách cấm của `QĐ-076` gồm đúng **bốn** thao tác **sự cố**, và *"mở màn công bố"* **không** nằm trong đó — nó là một **mốc trình diễn có chủ đích**. Nhờ vậy `QĐ-076` giữ nguyên và **không cần ngoại lệ nào**, đồng thời lý do tồn tại của `QĐ-079` *(có sẵn khe để khỏi sửa engine về sau)* cũng giữ nguyên. Kèm theo, **nguồn phát** cho nhóm hướng khán giả là cấu hình `admin | overlay`, mặc định **overlay**, **không** áp cho nhóm chỉ-admin, và **không** tự lui khi thiếu lớp phủ. Đã tích hợp vào **FR-074**, **FR-074a**, **FR-074b**, **FR-074c**, **FR-065a** và **AC-069**, **AC-069a**, **AC-069b**, **AC-069c**. Kéo theo `OQ-005` **đóng vế kênh phát**.

  <details><summary>Nội dung mâu thuẫn trước khi phân xử</summary>

  **Nguồn A** — `docs/decisions.md` `QĐ-079` và `docs/PRD.md` §12 `PRD-REQ-078`: danh sách khe âm thanh MUST phủ **cả sự kiện điều khiển** — *"hoàn nguyên · bỏ vòng · chạy lại vòng · kết thúc sớm · ép qua cảnh báo · mở màn công bố"*. `PRD-REQ-078` §Acceptance intent nói thẳng: *"**Bỏ một vòng phát đúng khe âm thanh đã gán**."*

  **Nguồn B** — `docs/decisions.md` `QĐ-076` và `docs/PRD.md` §12 `PRD-REQ-077`: *"Bỏ vòng, chạy lại vòng, sửa danh sách đề, gỡ lệnh cấm MUST NOT sinh thông báo nào tới khán giả; điểm và bàn cờ MUST đổi **đột ngột, không hiệu ứng, không giải thích**."* `QĐ-076` nêu lý do: *"mọi thông báo kiểu 'admin vừa bỏ vòng 2' đều biến sự cố hậu trường thành sự kiện trên sân khấu"*.

  Một khe âm thanh phát trên kênh khán giả **là** một thông báo về can thiệp của admin — nó chỉ đổi phương tiện từ mắt sang tai. Bốn trong sáu khe mà `QĐ-079` liệt kê nằm **đúng** trong danh sách mà `QĐ-076` cấm.

  **Cả hai nguồn đều im lặng về kênh nào phát âm thanh**, nên mâu thuẫn này chỉ thành thật khi câu hỏi đó được trả lời — xem `OQ-005`. Spec này ghi nhận nó **ngay bây giờ** vì `OQ-005` không có cách trả lời nào mà không chạm vào mâu thuẫn: chọn *"kênh khán giả có phát"* thì phá `PRD-REQ-077`; chọn *"chỉ máy admin phát"* thì phần lớn giá trị trình diễn của `QĐ-079` mất, và câu *"Bỏ một vòng phát đúng khe âm thanh đã gán"* cần một chủ ngữ mới.

  Phán quyết cho thấy **cả hai phía đều đang tranh trên một trục sai**. Câu hỏi đúng không phải *"nhóm sự kiện điều khiển có phát cho khán giả không"* mà là *"**từng khe** có hướng khán giả không"*. Đọc lại `QĐ-076`: nó cấm bốn thao tác **sự cố** — bỏ vòng, chạy lại vòng, sửa danh sách đề, gỡ lệnh cấm — chứ không cấm *"mọi thứ admin bấm"*. `QĐ-079` gộp *mở màn công bố* chung một danh sách với năm thao tác sự cố chỉ vì cả sáu đều là **cú bấm của admin**, nhưng *mở màn công bố* là cú bấm **trình diễn**, không phải cú bấm **sửa lỗi**. Câu *"Bỏ một vòng phát đúng khe âm thanh đã gán"* vẫn đúng nguyên văn — chủ ngữ của nó là **máy admin**.

  </details>

  **Cổng truy nguyên — ✅ ĐÃ THOẢ (2026-08-04).** Ghi **`QĐ-120` — Khe âm thanh chia HAI NHÓM; nguồn phát cấu hình được**. Đã sửa theo: `QĐ-079` nhận con trỏ ⚠️ *(danh sách phẳng bị tách)*; `PRD-REQ-077` bổ sung vế **mọi phương tiện, gồm âm thanh**; `PRD-REQ-078` nhận hai nhóm khe, nguồn phát và vế không-lui; `PRD.md` §22.3. §Acceptance intent của `PRD-REQ-078` giữ **nguyên văn** — chủ ngữ của nó là máy admin. Ảnh hưởng: **FR-065a**, **FR-074**, **FR-074a**, **FR-074b**, **FR-074c**. *(`QĐ-076`, `QĐ-079`, `QĐ-102`, `QĐ-001` · `PRD-REQ-077`, `PRD-REQ-078` · `GR-030` §Ở phía viewer)*

### NEEDS CLARIFICATION

- **`OQ-001` — ✅ MẤT ĐỐI TƯỢNG (§Clarifications 2026-08-04)**: **Thao tác của MC dưới banner tạm dừng.** Câu hỏi này chỉ tồn tại nếu `CONFLICT-001` được phân xử theo hướng *"banner CÓ phủ màn MC"*. Phán quyết chọn hướng ngược lại, nên **không có lớp nào** để prompt `STATE-039` phải xuyên qua và **không có gì** che câu hỏi cùng đáp án trên màn MC. `STATE-039` §Cho phép giữ nguyên hiệu lực mà **không cần** một ngoại lệ mới. Đã tích hợp vào **FR-055a** và **AC-050a**. *(`STATE-039`, `STATE-040`, `INV-021`, `PRD-REQ-074`, `PRD-REQ-108`)*

- **`OQ-002` — ✅ ĐÃ PHÂN XỬ (§Clarifications 2026-08-04)**: **Thứ tự chồng lớp khi nhiều lớp phủ cùng bật trên kênh public.** Phán quyết của chủ dự án: **không có thứ tự — hai lớp LOẠI TRỪ LẪN NHAU**, ràng buộc hai chiều, hạng **invalid state** *(nút không bật, toast, không ép được)*. Tổ hợp *(banner × lớp công bố)* trở thành **không dựng được**, nên câu hỏi mất chủ ngữ — cùng khuôn lập luận `QĐ-050` đã dùng cho cặp *banner × đồng hồ*, tức **dùng lại một pattern đã có** thay vì mở một trục thứ tự z mới. Trên hai kênh public, `STATE-033` và `STATE-040` là **hai lớp phủ duy nhất** dựng được, nên phán quyết này đóng câu hỏi **trọn vẹn** cho phạm vi epic này. `INV-021` bị **thu hẹp, không bị phủ định**: mọi tổ hợp lớp phủ khác vẫn hợp lệ, và ràng buộc này **không** là chỗ chặn cứng thứ tư của `INV-014`. Đã tích hợp vào **FR-044**, **FR-058a** và **AC-041**, **AC-058a**.

  **Cổng truy nguyên — ✅ ĐÃ THOẢ (2026-08-04).** Ghi **`QĐ-117` — Banner tạm dừng và lớp công bố kết quả LOẠI TRỪ LẪN NHAU**. Đã sửa theo: `STATE-033` §Vào, `STATE-040` §Vào và bảng ràng buộc hai chiều thứ hai, `EVENT-032` và `EVENT-034` §Hợp lệ ở / §Không hợp lệ ở, `T-087`, `T-089`, §Invalid transitions *(hai dòng mới)*, `INV-021` *(thu hẹp bằng một ngoại lệ đã liệt kê)*; `PRD-REQ-075` và `PRD-REQ-076`; `permissions.md` `PERM-047`, `PERM-048`; `PRD.md` §22.3. Ảnh hưởng: **FR-044**, **FR-058a**. *(`INV-021`, `INV-014`, `STATE-033`, `STATE-040`, `T-087`, `T-089`, `EVENT-032`, `EVENT-034` · `QĐ-050`)*

- **`OQ-003` — ✅ ĐÃ PHÂN XỬ (§Clarifications 2026-08-04)**: **Lớp phủ dựng stream có nhận đáp án Chướng ngại vật không.** Phán quyết của chủ dự án: **CÓ** — cùng lúc và cùng điều kiện với màn khán giả. Lập luận phân định: `QĐ-080` vế 4 loại Chướng ngại vật khỏi **trục THỜI ĐIỂM** *(mốc câu khép + cờ reveal)*, **không** loại nó khỏi **trục NGƯỜI NHẬN**, vốn đã được vế 2 chốt bằng câu *"lệnh cấm tuyệt đối với overlay bị gỡ"*. `GR-012` C2 viết *"cho viewer"* là **viết tắt của hai kênh public**. Đã tích hợp vào **FR-026**, **FR-026a** và **AC-023**.

  <details><summary>Nội dung câu hỏi trước khi phân xử</summary>

  `GR-037` C9 đẩy đáp án Chướng ngại vật ra ngoài phạm vi của mình và trỏ sang `GR-012`. `GR-012` C2 khai kết quả là *"Mở mọi miếng ghép và hiện Chướng ngại vật **cho viewer**"* — chỉ nêu **viewer**, không nêu **overlay**. Ở đường công bố **thường**, `QĐ-080` vế 2 đã gỡ lệnh cấm với overlay một cách tường minh và `TERM-007` khai overlay nhận *"cùng lúc và cùng điều kiện với viewer"*; nhưng chính `QĐ-080` vế 4 lại **loại Chướng ngại vật ra khỏi cơ chế đó**, nên câu *"cùng điều kiện với viewer"* không tự động phủ sang đường `GR-012`.

  Phán quyết cho thấy `GR-012` C2 **thiếu chữ**, không phải khai một lệnh cấm. Nó được viết trước `QĐ-080` — thời điểm mà overlay **vẫn** đang mang lệnh cấm tuyệt đối — nên chữ *"viewer"* ở đó là chính xác **tại thời điểm viết** và trở thành thiếu sót **sau** khi vế 2 gỡ lệnh cấm.

  </details>

  **Cổng truy nguyên — ✅ ĐÃ THOẢ (2026-08-04).** Ghi **`QĐ-118` — Đáp án Chướng ngại vật tới CẢ overlay**. Đã sửa theo: `GR-012` §Điều kiện *(thêm vế hai trục)*, C2 *(thêm **overlay**)*, §Nguồn; `GR-037` C9; `glossary.md` `TERM-007`; `PRD-REQ-073`; `PRD.md` §22.3. Ảnh hưởng: **FR-026a**. *(`GR-037` C9, `GR-012` C2, C3, §Điều kiện · `QĐ-080` vế 2, vế 4 · `TERM-007`, `QĐ-102`)*

- **`OQ-004` — ✅ ĐÃ PHÂN XỬ (§Clarifications 2026-08-04 phiên hai)**: **Chủ đề hiển thị gồm những gì và áp cho những màn nào.** Phán quyết của chủ dự án: chủ đề gồm đúng **ba trục tĩnh** — **bảng màu · logo giải · ảnh nền** — và áp cho đúng **hai kênh public**. **Không** tài sản động *(hình hiệu, clip, animation)*: chúng thuộc **module animation** và **khe âm thanh**, hai trục vốn đã tách riêng, nên nhét vào chủ đề là đẻ một loại media thứ ba phải preload và phải có ngưỡng dung lượng riêng mà không đổi lấy gì. **Không** phông chữ: `CLAUDE.md` khoá cứng *"Be Vietnam Pro"*. **Không** áp cho màn thí sinh, màn MC và màn admin: ba màn đó là bề mặt **vận hành**, và giữ chúng ngoài chủ đề loại hết đúng cái rủi ro tương phản mà mục này nêu — một chủ đề đặt sai chỉ hỏng phần trình diễn, không hỏng khả năng đọc của người đang thao tác. Đã tích hợp vào **FR-072**, **FR-072a**, **FR-072b** và **AC-067**, **AC-067a**.

  <details><summary>Nội dung mục trước khi phân xử</summary>

  `PRD-REQ-078` nói *"Contest MUST cấu hình được chủ đề hiển thị"* và `PERM-023` gộp nó vào `contest.edit`. **Không nguồn nào** trong `docs/` định nghĩa một chủ đề chứa gì *(bảng màu, logo, ảnh nền, hình hiệu?)*, áp cho những màn nào *(chỉ hai kênh public, hay cả màn MC, màn thí sinh và màn admin?)*, hay có ràng buộc nào để nó không phá tương phản và khả năng đọc trên màn thi đấu. `CLAUDE.md` §Quy ước code khoá cứng phông *"Be Vietnam Pro"*, nên ít nhất trục phông chữ **không** thuộc chủ đề — nhưng đó là loại trừ duy nhất suy ra được.

  </details>

  **Cổng truy nguyên — ✅ ĐÃ THOẢ (2026-08-04).** Ghi **`QĐ-121` — Chủ đề hiển thị gồm BA TRỤC TĨNH, áp cho ĐÚNG hai kênh public**. Đã sửa theo: `PRD-REQ-078`; `PRD.md` §22 ma trận, §22.3. Ảnh hưởng: **FR-072**, **FR-072a**, **FR-072b**. *(`PRD-REQ-078`, `PERM-023`, `CLAUDE.md` §Quy ước code)*

- **`OQ-005` — ✅ ĐÃ PHÂN XỬ TRỌN VẸN.** **Vế kênh nào phát** đóng ở phiên một cùng `CONFLICT-002`: hai nhóm khe, nguồn phát `admin | overlay` mặc định `overlay` chỉ áp cho nhóm hướng khán giả, màn khán giả không bao giờ là nguồn phát, không lui tự động — **FR-074a** → **FR-074c**. **Vế danh sách đầy đủ** đóng ở phiên hai (§Clarifications 2026-08-04 phiên hai). Phán quyết của chủ dự án: **dẫn xuất từ wiki Fandom, lọc hai tầng**. Tập khe = cột *Tên* ở trang [`Âm thanh`](https://duong-len-dinh-olympia.fandom.com/vi/wiki/%C3%82m_thanh) — cùng nguồn mà `docs/source/fandom-olympia-26-luat-choi.md` đã lấy luật — trừ **(1)** biến thể ngoài O26 *(`Ô mạo hiểm`; `Bộ 8/12/16 câu hỏi` của cấu trúc Khởi động O23)* và **(2)** khe truyền hình không ứng với sự kiện engine *(nhóm ngoài trận và nhóm tổng kết, vì `FR-077` đòi mỗi khe ứng một tín hiệu ngữ nghĩa, và khe *mở màn công bố* đã phủ mốc tổng kết)*. Biến thể theo **độ dài đồng hồ** gộp thành một khe *đếm giờ* mỗi vòng, vì `timeSeconds` là metadata của từng câu. Kết quả **52 khe**, cộng `6` khe điều khiển ra **58**. Đã tích hợp vào **FR-074d**, **FR-074e** và **AC-069d**.

  **Cổng truy nguyên — ✅ ĐÃ THOẢ (2026-08-04), qua HAI bước.** **(a)** Trang `Âm thanh` đã được lưu thành **`docs/source/fandom-olympia-26-am-thanh.md`** — bản **trích lược có chủ đích**: giữ cột `Tên` *(danh sách khe ngữ nghĩa)*, bỏ cột file và cột khoảng ngày phát sóng, và nói rõ điều đó ở §Phạm vi trích lược. Lý do bỏ: sản phẩm **không có bộ âm mặc định**, admin tự tải file, nên các bản phối theo mùa không là đầu vào của requirement nào. **(b)** Ghi **`QĐ-122` — Danh sách khe *mốc thi đấu* DẪN XUẤT từ wiki Fandom, lọc hai tầng**. Đã sửa theo: `PRD-REQ-078`; `PRD.md` §22 ma trận, §22.3, bảng nguồn; `traceability.md` bảng nguồn gốc; `QĐ-079` nhận con trỏ ⚠️. Ảnh hưởng: **FR-074d**, **FR-074e**. *(`QĐ-079`, `PRD-REQ-078`, `CLAUDE.md` §Quy ước code, `docs/source/fandom-olympia-26-luat-choi.md`)*

### Missing boundary / evaluation order / invalid-transition outcome

- **`OQ-006` — ✅ ĐÃ PHÂN XỬ (§Clarifications 2026-08-04)**: **Thứ tự đánh giá khi mốc câu khép gặp một cú đóng hiển thị bằng tay của `PERM-044`.** Phán quyết của chủ dự án: **cú đóng tay THẮNG** — engine không đẩy đè; hai kênh public vẫn kín tới khi admin tự mở lại; hiệu lực ở **phạm vi câu** và không dính sang câu sau. Cơ sở: cú đẩy của engine tại mốc câu khép bị gác bởi **chính cờ `revealAnswerAfterJudge`**, mà `PERM-044` đã được khai là **thắng cờ đó**; đọc theo hướng ngược lại làm `PERM-044` thắng cờ nhưng thua cái chạy **bằng** cờ. `GR-037` §Bấm trùng **không** bị phá — *"một chiều"* nói về **engine**, và chiều đó vẫn nguyên: engine đi một chiều, admin đi được cả hai. Đã tích hợp vào **FR-027**, **FR-027a** và **AC-024**, **AC-024a**.

  **Cổng truy nguyên — ✅ ĐÃ THOẢ (2026-08-04).** Ghi **`QĐ-119` — Thao tác tay thắng ở CẢ HAI CHIỀU**, dạng *Thay cho* vế *"engine không bao giờ tự mở gì"* của `QĐ-074` — vế đó đã cũ từ khi có `QĐ-080`. Đã sửa theo: `GR-037` **C10** *(mới)*, §Thứ tự đánh giá **bước (5b)**, §Bấm trùng, §Không đổi gì, §Nguồn; `STATE-021` §Công bố đáp án *(ba ngoại lệ)*; `INV-017`; `QĐ-074` nhận con trỏ ⚠️; `permissions.md` `PERM-044`; `PRD-REQ-049`; `PRD.md` §22.3. Ảnh hưởng: **FR-027a**. *(`GR-037` §Thứ tự đánh giá, §Không đổi gì, §Bấm trùng · `PERM-044`, `PERM-045` · `QĐ-048`, `QĐ-001` · `NFR-25`)*

- **`OQ-007` — ✅ ĐÃ PHÂN XỬ (§Clarifications 2026-08-04 phiên hai)**: **Kết cục hiển thị của một lớp phủ khi trạng thái bên dưới không dựng được nội dung cho nó.** Phán quyết của chủ dự án: **dựng bình thường với dữ liệu hiện có**; **không** có nhánh trạng thái rỗng, **không** để trống phần dữ liệu, và **không** từ chối cú mở. Ca mẫu — mở lớp công bố ở `LOBBY` — không phải một ca chưa khai: mọi ghế `0` cho ra **cả bảng đồng hạng 1**, đúng thứ mà `GR-025` C5 và `QĐ-049` đã định nghĩa. Hai lập luận: **(1)** một trạng thái rỗng riêng đẻ thêm một loại nội dung cộng một ngưỡng *"thế nào là rỗng"* phải định nghĩa cho **từng** lớp phủ; **(2)** từ chối cú mở là máy tự phán rằng thứ admin muốn đưa lên sóng *"không đáng hiển thị"* — trái `QĐ-001`. Đã tích hợp vào **FR-085a** và **AC-079a**.

  <details><summary>Nội dung mục trước khi phân xử</summary>

  `PRD` §9.4 định nghĩa hạng **invalid state** bằng *"toast phía admin"*, và `STATE-037` nói thẳng *"phía **thí sinh** thì không có toast — nút không render"*. Hai kênh public không có toast, không có nút, và **không có đường ghi**, nên về cấu trúc chúng không sinh được thao tác nào — vế *invalid transition* của chúng là rỗng. Câu hỏi còn lại là **hiển thị**: khi một lớp phủ đang bật mà trạng thái bên dưới không cho nó nội dung nào có nghĩa *(ví dụ mở lớp công bố ở `LOBBY` trước vòng đầu tiên, khi mọi ghế đều `0` và chưa có thứ hạng nào)*, hai kênh public hiện gì. Nguồn im lặng; `EVENT-032` §Hợp lệ ở khai *"mọi trạng thái cấp trận"* nên ca này **dựng được**, chỉ là nội dung chưa khai.

  </details>

  **Cổng truy nguyên — ✅ ĐÃ THOẢ (2026-08-04).** Ghi **`QĐ-123` — Lớp phủ thiếu dữ liệu vẫn dựng BÌNH THƯỜNG; không có nhánh trạng thái rỗng**. Đã sửa theo: `PRD-REQ-075`; `PRD.md` §22 ma trận, §22.3. Ảnh hưởng: **FR-085a**. *(`PRD` §9.4, `STATE-037`, `STATE-033`, `EVENT-032`, `QĐ-004`, `GR-025` C5, `QĐ-049`)*
...
| `GR-012` | C1 *(vòng kết thúc — **EPIC-006**)* · **C2** *(admin bấm công bố ⇒ mở mọi miếng ghép và hiện Chướng ngại vật cho **cả hai kênh public** — `OQ-003` đã phân xử)* · **C3** *(không bấm ⇒ không tắc)* · C4 *(hàng ngang chưa hỏi ⇒ trả lại kho — **EPIC-006**)* · C5 *(bấm hai lần — **EPIC-007**)* · C6, C7, C8 *(**EPIC-006**)* | *(EPIC-006)* · AC-008, AC-023 · AC-008 · *(EPIC-006)* · *(EPIC-007)* · *(EPIC-006)* |

**Trạng thái, sự kiện và transition**

| Nguồn | Nội dung | Acceptance scenarios |
|---|---|---|
| §F bảng nhóm lớp phủ | ba nhóm `F.1` / `F.2` / `F.3` khác nhau ở *ai thấy* và *có chặn thao tác bên dưới không*; dòng `F.3` **bỏ MC** *(`CONFLICT-001` đã phân xử)* | AC-039, AC-050, AC-050a |
...
| §*Invalid transitions* | mở banner khi đồng hồ chạy ⇒ toast · start timer khi banner bật ⇒ toast · **mở lớp công bố khi banner bật và ngược lại ⇒ toast** *(`OQ-002` đã phân xử)* · sự kiện ghi từ kênh public ⇒ **không có đích đến** | AC-052 · AC-053 · AC-058a · AC-002, AC-003 |
| `INV-006`, `INV-014`, `INV-016`, `INV-017`, `INV-018`, `INV-021` | tín hiệu luôn có outcome · **ba** chỗ chặn cứng, cặp loại trừ lớp phủ **không** là chỗ thứ tư · đồng hồ không đóng băng · đáp án chỉ rời server tới `PERM-045` · điểm được phép âm · lớp phủ không đổi trạng thái bên dưới, **thu hẹp một ngoại lệ** | AC-056 · AC-058a · AC-037 · AC-015, AC-025 · AC-005 · AC-047, AC-048, AC-058a, AC-059 |
| `permissions.md` `PERM-044` vs `PERM-045` | đọc trên màn của chính mình ≠ công bố cho mọi màn; `PERM-044` **thắng cờ**, nên cú đóng tay **chặn** cú đẩy tại mốc câu khép *(`OQ-006` đã phân xử)* | AC-036 · AC-024a |
...
**When** rà danh sách,
**Then** nó chứa khe cho **cả sự kiện điều khiển**: hoàn nguyên · bỏ vòng · chạy lại vòng · kết thúc sớm · ép qua cảnh báo · mở màn công bố; sáu khe này có mặt **kể cả khi** chưa gán file nào; và mỗi khe mang đúng **một** nhãn nhóm — *hướng khán giả* hoặc *chỉ-admin*. *(Danh sách đầy đủ của nhóm* mốc thi đấu *— `OQ-005`, không chặn kịch bản này.)*
