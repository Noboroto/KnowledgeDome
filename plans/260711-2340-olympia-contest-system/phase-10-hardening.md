...
- Non-functional: chịu tải mục tiêu ✅ D11 (12/07): **<50 viewer/trận, ~5 trận LIVE song song** (trần kiến trúc 500 viewer/trận chỉ là headroom); pass security checklist; backup DB tự động; một lệnh deploy.
...
- Create: `deploy/` (prod.compose.yml, **nginx** reverse proxy + TLS certbot, backup script pg_dump + MinIO mirror)
...
1. Load test (k6 + socket.io client) theo mục tiêu ✅ D11 (12/07): **5 trận LIVE song song × (12 thí sinh + 50 viewer)** + buzzer storm 50 đồng thời trên 2 trận cùng lúc (điểm nhấn = nhiều match song song, không phải fanout; single-writer per match phải giữ ordering từng trận); đo p95 latency event < 200ms LAN / < 500ms Internet; tìm bottleneck. Trần kiến trúc 500 viewer/trận chỉ smoke-test 1 lần làm headroom. Bổ sung: đo băng thông encrypted-preload (D12b) với video 200MB (trần D6) trên 12 client đồng thời.
...
3. Monitoring + alert đơn giản (lag/error spike hiện trên admin UI) + **disk usage với ngưỡng cảnh báo** (kiến trúc persist-trước-broadcast nghĩa là disk đầy = trận đứng hình — gap 5.2) + logging chuẩn hoá **pino** qua `nestjs-pino` (request-context trong mọi log; chạy tốt trên Express adapter — D9) + `pino-roll` rotation (đã check Context7 12/07 — cả 3 reputation High) + trạng thái backup gần nhất (backup fail phải NHÌN THẤY, không âm thầm — gap 3.4); chaos drill bổ sung: disk 95%.
...
6. Deploy — **2 profile (user chốt 12/07, chi tiết `research/queue-decision.md`)**: (a) **`prod.compose.yml`** — api ×2 + BullMQ/Redis adapter + MinIO + **proxy nginx là port DUY NHẤT expose (80/443, TLS qua certbot/lego)**, mọi service khác internal; backup cron; (b) **portable Windows không Docker cho LAN**: bundle 1 thư mục (node runtime + app build + **Postgres portable binaries zip** + pg-boss + in-process queue + filesystem storage — qua abstraction layer Phase 1), script `start.bat`, không cần cài đặt. Runbook sự cố cho cả 2 profile.
