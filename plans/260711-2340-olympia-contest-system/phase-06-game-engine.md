...
**Tổng quát hoá 12/07 (đã chốt với user):** không còn chuỗi 4 vòng cứng — orchestrator chạy `rounds[]` playlist tuỳ ý; **v1: 1-12 ghế CÁ NHÂN** (lộ trình D18 — semantics ĐỘI kích hoạt ở Phase 12/v2, nhưng schema `Team`/`seat.teamId`/`scoringUnit` + reducer team-aware dựng SẴN từ phase này: mỗi seat v1 = trường hợp suy biến team size 1); khởi động 4 kiểu lượt + rút đề ngẫu nhiên theo lĩnh vực (event `QUESTIONS_DRAWN`, noRepeatInMatch, server-side để replay được); VCNV 4-8 hàng ± gợi ý ký tự; tăng tốc 2 format (ranked-speed / clue-buzz "3-4 dữ kiện"); về đích gói preset hoặc custom-build; **`timeSeconds` là metadata TỪNG CÂU, ĐỘC LẬP với mức điểm** (✅ D8 12/07 — cùng mức 20đ có thể câu 15s và câu 40s; hệ thống chọn câu theo mức điểm, thời gian theo câu; preset chỉ đặt default); sound cue slot theo THÀNH PHẦN × round + background tracks. Phổ điểm là mảng theo số đơn vị điểm. Preset-driven để kìm test matrix: engine hỗ trợ mọi tổ hợp Zod-hợp-lệ, test/UAT chạy trên preset chuẩn. **Preset `O26_DEFAULT@1` build theo Fandom wiki (✅ D8 source of truth — bảng giá trị: `rules-2026.md`); contest builder có nút "Áp dụng luật 2026" áp preset này.**

## Requirements
- Functional: chạy trọn round playlist bất kỳ (Zod-hợp-lệ) — preset `O26_DEFAULT@1` là kịch bản chuẩn; admin đổi timer/điểm per-contest (RuleConfig v2) và can thiệp giữa trận (cộng/trừ điểm tay, skip câu, thay câu dự phòng, pause/resume, undo, CONFIG_PATCH có re-preflight); sound-cue events phát cho client; preload media MÃ HOÁ phân tầng audience (✅ D12b). **Nguồn đề (✅ D22): trận chỉ dùng danh sách câu người tạo contest đã CHỌN + snapshot trước khi start; mỗi câu mang mask "phần được phép dùng" (checkbox lúc gán, hợp lệ theo pool D24 + điều kiện điểm); pool được phép DƯ; draw service random TRONG pool đó, loại câu đã `usedInContest` (no-repeat toàn contest — event `QUESTION_USED` ghi lúc câu được hỏi), không bao giờ tự lấy từ kho.** Engine chỉ emit **semantic event** — animation là module client độc lập (✅ D22, mapping event→animation là config phía client, đổi rule không đụng animation).
...
- **Chấm**: câu nhập text → normalize (**TRIM đầu/cuối — user chốt 12/07**, bỏ dấu cách thừa giữa từ, không phân biệt hoa thường, tuỳ chọn bỏ dấu tiếng Việt) so `acceptedAnswers` (đã trim khi lưu ở phase-03). **Toggle `autoJudge` per-contest — DEFAULT TẮT (✅ D4 bổ sung 12/07)**: tắt → auto-match chỉ là GỢI Ý trên JudgingPanel, điểm chỉ chốt khi người điều khiển bấm Đúng/Sai; bật → câu gõ tự chốt theo auto-match + admin override được. Submission rỗng/toàn whitespace sau trim → SKIP giữ bản trước (✅ D20.1); câu miệng → luôn admin bấm Đúng/Sai bất kể toggle (D4).
- **Media preload MÃ HOÁ phân tầng theo audience** (✅ D12b user chốt 12/07; red-team C2 — media CHÍNH LÀ đề ở Tăng tốc/VCNV): viewer/overlay preload câu N+1 sớm (URL thường); **thí sinh preload BLOB MÃ HOÁ qua service worker — server chỉ phát decryption key đúng lúc reveal (server time)** → mượt cả qua Internet mà bytes trên máy thí sinh là ciphertext. Pipeline: media encrypt khi serve cho contestant-namespace (AES-GCM per-question key) → SW cache → event `MEDIA_KEY {questionId, key}` lúc reveal → decrypt + play. `preloadPolicy: encrypted | reveal-only` — default `encrypted`; **fallback `reveal-only`** (nhận URL đúng lúc reveal như phương án a) khi SW không khả dụng/lỗi decrypt, engine tự hạ cấp + log. Media 404/lỗi → admin nhận cảnh báo + nút skip/thay câu dự phòng.
...
## Milestones nội bộ (red-team M9; phase này = critical path của v1 — teams đã tách sang Phase 12/v2 nên NHẸ hơn thiết kế cũ)

- **6a** (mở khoá Phase 7/8 integrate sớm): MatchEvent + reducer (**team-aware từ đầu** — v1 mỗi seat = đơn vị điểm riêng, trường hợp suy biến team size 1; KHÔNG code semantics đội) + timer + buzzer + single-writer/lease + **orchestrator-skeleton chạy playlist tuần tự** + engine KHOI_DONG kind `individual-count` + `common-count`.
- **6b**: 2 turn kinds còn lại + draw service (random TRONG danh sách đề đã gán — ✅ D22); VCNV (4-8 hàng, hintMap), TANG_TOC (2 format), VE_DICH (2 package modes), TIE_BREAK engines; encrypted-preload pipeline (D12b) + snapshot/restore + degraded mode + CONFIG_PATCH re-preflight. *(Teams semantics §2b → Phase 12.)*

## Implementation Steps
1. `RuleConfigSchema` **v2** (round playlist + seats/teams [schema đủ từ v1 — semantics đội Phase 12] + 4 turn types + VCNV variants + TT 2 formats + VĐ package modes + sound slots + drawConfig — full spec ở `ruleconfig-v2-spec.md`) + presets: **`O26_DEFAULT@1` theo Fandom (✅ D8: khởi động 3s, VCNV 60/50/40/30 + trung tâm 20, tăng tốc 20/20/30/30, về đích 2 mức 20/30 + `stealMode: 'transfer'` + cướp sai −½)**, `O21_CLASSIC@1`; `TEAM_12@1` định nghĩa sẵn (test ở Phase 12); kèm `buzzWindowSec` (O26: 3s), `dropoutPolicy`, `autoPauseOnHostDisconnect`, `preloadPolicy: encrypted|reveal-only` (default encrypted — D12b), `stealMode` (O26 default `transfer`), `revealAnswerAfterJudge` + `matchPurpose bundle (§13 — kích hoạt Phase 11)`, quy tắc làm tròn điểm lẻ — Zod ép số nguyên, validate mảng điểm theo số đơn vị điểm. **Về đích: đáp án người thi chính = bản CUỐI, người cướp = bản ĐẦU (Fandom) — 2 chế độ ghi nhận trong answer-matcher.**
...
5. Match orchestrator nối các engine + state **`INTERMISSION`** giữa các vòng (admin điều khiển; viewer/overlay hiện bảng điểm tổng + vòng kế tiếp + intro thí sinh đầu trận — 30-40% thời lượng buổi thi thật là "giữa các vòng", gap 4.2) + admin intervention (adjust/skip/substitute/pause/undo) + state-sync cho mọi client mới/reconnect. Chức năng **anonymize contestant** (thay displayName bằng mã trong dữ liệu trận cũ, không phá event log — privacy gap 1.1; retention job tự động chạy ở Phase 11: practice 3 tháng / official 12 tháng — ✅ D21.1). Emit `ANSWER_REVEALED` sau JUDGE khi revealAnswerAfterJudge bật.
6. Media preload pipeline **mã hoá** (D12b): encrypt-on-serve cho contestant namespace + event `MEDIA_KEY` lúc reveal + fallback reveal-only; sound-cue emitter (client pre-download SFX khi vào phòng — D10).
7. Snapshot/restore + kill-test: giết process giữa vòng, restart, trận tiếp tục đúng.

## Success Criteria
- [ ] Chạy trọn trận mô phỏng automated ra điểm đúng với bảng tính tay cho ≥3 kịch bản trên preset `O26_DEFAULT@1` theo Fandom (4 thí sinh, có kịch bản steal-transfer + NSHV) + ≥1 kịch bản playlist tuỳ biến (clue-buzz + VCNV 8 hàng + draw trong pool đã gán). *(Kịch bản `TEAM_12@1` → Phase 12.)*
- [ ] Thí sinh nhận blob mã hoá trước reveal — kiểm tra bytes cached KHÔNG decode được thành media; key chỉ tới sau `REVEAL` (test tự động D12b); SW bị tắt → tự fallback reveal-only, trận vẫn chạy.
