...
2. Cấu hình Better-auth: username plugin, Argon2id, cookie `HttpOnly; SameSite=Lax`; `Secure` bật ở profile compose (HTTPS). **Profile portable chạy HTTP LAN — ✅ user chốt 12/07, không cần HTTPS** (gap-sweep H-F1): `Secure` off theo `INFRA_PROFILE`, **api serve luôn web static cùng origin** (fastify-static) để khỏi vỡ cookie cross-origin, Better-auth `baseURL`/`trustedOrigins` đọc từ IP LAN máy lúc start (in ra console + QR). Session theo role: thí sinh **24h**, admin/setter 7 ngày; admin có nút "logout mọi phiên thí sinh" sau trận.
