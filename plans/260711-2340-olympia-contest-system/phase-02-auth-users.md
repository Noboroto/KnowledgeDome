...
4. REST admin: `POST/GET/PATCH /users` (tạo user + role, reset password, disable). Admin ĐẦU TIÊN qua CLI `pnpm create-admin` bắt nhập password mạnh ngay lúc chạy — **cấm default credentials** kiểu admin/admin (gap 3.3). **Bulk-create user + role phải là service dùng chung** (không chỉ REST 1-user) vì Phase 4 import roster (🟡 D26) dựng nhiều account cùng lúc: expose `UsersService.upsertFromRoster()` (merge theo username, regenerate password default / mang hash khi opt-in, dựng role custom nếu thiếu) — Phase 4 gọi qua service này, không chạm Better-auth trực tiếp.
