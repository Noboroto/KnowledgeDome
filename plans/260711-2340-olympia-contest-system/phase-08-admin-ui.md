...
**Bổ sung 12/07:**
- **Contest builder UI**: trình dựng round playlist (thêm/xoá/sắp xếp vòng, form config từng loại vòng theo RuleConfig v2 — thời gian/số câu/điểm đều CUSTOM được), **nút "Áp dụng luật 2026"** áp preset `O26_DEFAULT@1` theo Fandom wiki (✅ D8 — 1 click về luật chuẩn, sau đó vẫn tuỳ biến tiếp), setup ghế 1-12 cá nhân (teams kéo-thả vào đội → Phase 12/v2), **bước BẮT BUỘC chọn danh sách câu hỏi cho từng vòng trước khi start (✅ D22 — hệ thống không tự lấy từ kho, draw chỉ random trong danh sách này): QuestionPicker với full-text search kho đề + filter (loại vòng/lĩnh vực/mức điểm/độ khó/tags/người thực hiện/trạng thái duyệt) + sort (mới nhất/mức điểm/độ khó/lần dùng gần nhất/tần suất dùng), preview không lộ đáp án ngoài quyền**, theme editor (màu theo design tokens, upload logo/ảnh/video hình hiệu), sound editor (gán file cho từng cue slot theo thành phần, playlist nhạc nền — admin tự upload, slot trống = silent ✅ D10).
