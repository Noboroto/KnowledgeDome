...
**Bổ sung 12/07:**
- **Contest builder UI**: trình dựng round playlist (thêm/xoá/sắp xếp vòng, form config từng loại vòng theo RuleConfig v2 — thời gian/số câu/điểm đều CUSTOM được), **nút "Áp dụng luật 2026"** áp preset `O26_DEFAULT@1` theo Fandom wiki (✅ D8 — 1 click về luật chuẩn, sau đó vẫn tuỳ biến tiếp), setup ghế 1-12 cá nhân (teams kéo-thả vào đội → Phase 12/v2), **bước BẮT BUỘC chọn danh sách câu hỏi trước khi start (✅ D22 — hệ thống không tự lấy từ kho, draw chỉ random trong danh sách này): QuestionPicker với full-text search kho đề + filter (pool D24/lĩnh vực/mức điểm/độ khó/tags/người thực hiện/trạng thái duyệt) + sort (mới nhất/mức điểm/độ khó/lần dùng gần nhất/tần suất dùng), preview không lộ đáp án ngoài quyền; mỗi câu đã chọn có CHECKBOX "được phép dùng ở phần nào" (chỉ hiện phần hợp lệ theo pool + điều kiện điểm — câu KV tick Khởi động/Về đích, TT chỉ Tăng tốc); pool ĐƯỢC PHÉP DƯ (chọn nhiều hơn nhu cầu — dư = dự phòng); badge cảnh báo câu đã `usedInContest`**, theme editor (màu theo design tokens, upload logo/ảnh/video hình hiệu), sound editor (gán file cho từng cue slot theo thành phần, playlist nhạc nền — admin tự upload, slot trống = silent ✅ D10).
...
2. JudgingPanel: câu hỏi + media preview + đáp án + nút Đúng/Sai/hotkey. **Chế độ chấm theo toggle `autoJudge` (✅ D4 — default TẮT):** tắt → auto-match hiện dưới dạng GỢI Ý (badge đúng/sai mờ) chờ admin bấm chốt; bật → câu gõ tự chốt, admin override. VCNV: nút **"Mở toàn bộ miếng ghép + công bố CNV"** THỦ CÔNG cho tình huống cả 4 bị loại (✅ D13.1 12/07 — không auto, giữ nhịp dẫn). Riêng tăng tốc ranked-speed: **TangTocJudgingGrid** — sau TIME_UP hiện grid N bản-cuối (per đơn vị điểm) với kết quả auto-match, admin confirm/override hàng loạt; realtime trước TIME_UP chỉ hiện bản mới nhất per seat (collapse — gap-sweep H-F4d).
