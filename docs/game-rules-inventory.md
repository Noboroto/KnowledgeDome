...
- **Actor**: Server.
- **Rule mâu thuẫn**: **K-8 — độ phân giải so sánh khác nhau 3 nguồn**: `W26` = 2 chữ số thập phân (≈10ms); `F26` không nêu; `R26` §7 chốt **ms**. Chọn ms làm cửa sổ hoà **hẹp hơn 10 lần** so với `W26` → ít trường hợp hoà hơn thực tế chương trình. `SPEC` §6 còn có option `tieRule: 'microsecond'` phá vỡ hoàn toàn quy tắc này; không nguồn nào nói default.
- **Chưa định nghĩa**: khi share-high, hạng tiếp theo nhảy mấy bậc (40/40/**20**/10 hàm ý nhảy 1 bậc, không phát biểu thành luật).
...
| **K-8** | Độ phân giải "đồng thời gian" Tăng tốc | **ms** (server-received) | `DEF` **D13.3** ✅ 12/07 (*"đồng thời gian → cùng mức điểm"*) + `R26` §7 (*"độ phân giải ms server-received"*) | `W26`: **2 chữ số thập phân** (≈10ms). Loại theo D8 |
