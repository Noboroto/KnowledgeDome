...
| 4 | ~~`GRR-077`~~ | ⚖️ **PHÂN XỬ 2026-07-27 (`Đ-47`) — bác ở dạng STATE, giữ ở dạng NHÃN.** Không thêm `ABANDONED` vào enum (giữ **4** giá trị, `Đ-38` không mở lại); thay bằng **`matchClosedReason = "bỏ dở"`** trên `FINISHED`, do admin bấm **"Huỷ trận"** (`EVENT-044`). Điểm giữ nguyên, không phân định thứ hạng. Lý do: không transition nào rẽ nhánh theo giá trị này — thứ cần nó là **biên bản, `SM-11`, retention**, tức hạng **thuộc tính đóng sổ**, không phải **trạng thái** | Máy trạng thái trận · thống kê `SM-11` |
...
| 21 | ~~`Q-A2`~~ | ✅ **ĐÃ DUYỆT 2026-07-27 (`Đ-44`).** Hàng ngang admin **đánh dấu đã hỏi** bằng tay **CÓ tính** vào thang điểm CNV — vì đó là đường **khôi phục trạng thái**; bước **lộ đáp án** thì không tính. Thao tác **không tự cộng điểm** cho ai; ngoại lệ thì admin **tự cộng tay** (`EVENT-020`) |
...
| **C-11 Q-A2** — miếng ghép admin mở tay có tính vào thang điểm CNV không | ✅ **CÓ TÍNH — `Đ-44`, chủ dự án chốt 27/07.** Chính xác hơn đề xuất gốc: bước **đánh dấu đã hỏi** tính, bước **lộ đáp án** không ⇒ mở tay trọn một hàng chỉ tính **một lần**. Thao tác **không tự cộng điểm** cho ai | ✅ `ĐÃ CHỐT` | Căn cứ quyết định là **khôi phục trạng thái**, mạnh hơn lập luận *"đo lượng thông tin đã lộ"* của đề xuất gốc: đường mở tay tồn tại để admin **dựng lại bàn cờ sau sự cố**, nên nó **phải** nuôi đúng biến đếm mà băng điểm đọc — không thì trạng thái khôi phục trông đúng mà **tính điểm sai**. Vẫn đóng luôn lỗ hổng *"mở tay để giữ băng cao"* |
| **C-11 Q-A3** — đóng lại có đổi điểm không | **Không đổi điểm, chỉ đổi hiển thị.** Bước thang CNV **không lùi lại** khi đóng | `SUY RA` | Hệ quả trực tiếp của Q-A2: thông tin đã lộ thì không thu hồi được. Điểm 6: muốn đổi điểm phải qua event đảo ngược. `Đ-44` **không đụng** vế này — nó nói về chiều **mở**, còn đây là chiều **đóng** |
...
| **C-11 Q-A6** — AuditLog phân biệt mở-do-admin và mở-do-engine | **CÓ** | `SUY RA` | Sau `Đ-44` thì càng cần: thao tác mở tay **đổi băng điểm thật** ⇒ phải tra được **ai bấm, lúc nào** khi có khiếu nại về giá của Chướng ngại vật, bên cạnh khiếu nại *"vì sao đáp án hiện sớm"* |
