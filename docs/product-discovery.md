...
| ID | Vấn đề | Epic |
|---|---|---|
| ~~**S-21**~~ | ✅ **ĐÃ CHỐT 2026-07-27 (`QĐ-071`): CÓ áp.** Hàng rào `everPublic` gắn với **thao tác**, không gắn với mốc thời gian — chạy phát như nhau ở **mọi** cửa đưa câu vào danh sách gán của một trận official: pre-flight, sửa danh sách ở `LOBBY`, và **import**. Cờ `everPublic` **đi theo câu** qua import/export, khác `usedInContest` vốn đặt lại theo contest. Không viết nhánh mới, không thêm chỗ chặn cứng — cùng **một** hàng rào, gọi ở nhiều điểm hơn. Cũ: **Thêm câu vào danh sách gán có bị hàng rào `everPublic` chặn không?** `R-GEN-12` quy định câu `everPublic` **hard-block mọi match**, force cần confirm 2 bước + audit — nhưng hàng rào đó được đặc tả ở **pre-flight trước trận**. Nay có đường thêm câu **giữa trận**, cần chốt hàng rào có áp ở đó không. Nếu không áp thì đây là **đường vòng qua toàn bộ cơ chế chống rò đề** | E-5, E-11 |
